/**
* create by zyt on 2022/7/25 17:53
* 类注释：
* 备注：
*/
<template>
  <div class="selectOrgOrDept">
    <div
      @click.prevent.stop="showDialogTree"
      style="width: 100%"
      id="orgOrDeptInput"
    >
      <slot></slot>
...
export default {
  ...
  methods: {
        ...
        this.$api.setting.organization
          ...
          .then((res) => {
            ...
            const data = res.data.map((item) => ({
              ...
              id: item.unitCode,
              leaf: node.data ? true : item.lastUnit,
            }));
            return resolve(data);
          })
          .catch((err) => {
    ...
    clickBlur(e) {
      const self = this;
      if (
        self.$el.querySelector("#tree-div") &&
        !self.$el.querySelector("#tree-div").contains(e.target)
      ) {
        self.showTree = false;
      }
    },
