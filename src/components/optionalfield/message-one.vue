...
<template>
    <div class="message-one" :class="{'is-disabled': disabled}">
        ...
        <div class="message-one-info-wrap">
            <p>
                <span :class="infoOrgClassObj" @click="clickInfo('org')">{{org}}</span> 
                <span :class="infoUserClassObj" @click="clickInfo('user')">{{user}}</span>
            </p>
            <p>
                <span :class="infoTimeClassObj" @click="clickInfo('time')">{{dTime}}</span>
            </p>
        </div>
        <!-- <el-row>
            <el-col :span="8" :offset="12" :class="orgClassObj">
                <el-input 
                    v-model="dOrg" 
                    :disabled="disabled"
                    :readonly="org_isReadonly"
                    :placeholder="org_placeholder" 
                    @change="change('org', $event)"
                    @blur="blur('org', $event)"
                    @focus="focus('org', $event)">
                </el-input>
            </el-col>
...
export default {
    ...
    props: {
        ...
        message_modify: {
            type: String,
            default: 'enable'   // modify值有enable, hidden, disable, modify
        },
        message_placeholder: {
            type: String,
            default: ''   
    ...
    computed: {
        ...
        //         this.$emit('result', this.cIndex, newVal, this.cOrg, this.cUser, this.cTime);
        //     }
        // },
        ...
        infoOrgClassObj: function(){
            return {
                'message-one-info-default': true,
                'message-one-info-disable': this.org_modify === 'disable',
                'message-one-info-hidden':  this.org_modify === 'hidden',
                'message-one-info-enable':  this.org_modify === 'enable',
                'message-one-info-modify':  this.org_modify === 'modify',
            }
        },
        orgClassObj: function(){
            return {
                'message-short':  true,
                'message-hidden':   this.org_modify === 'hidden',
                'message-disable':  this.org_modify === 'disable',
                'message-enable':   this.org_modify === 'enable',
            }
        },
        infoUserClassObj: function(){
            return {
                'message-one-info-default': true,
                'message-one-info-disable': this.user_modify === 'disable',
                'message-one-info-hidden':  this.user_modify === 'hidden',
                'message-one-info-enable':  this.user_modify === 'enable',
                'message-one-info-modify':  this.user_modify === 'modify',
            }
        },
        ...
        infoTimeClassObj: function(){
            return {
                'message-one-info-default': true,
                'message-one-info-disable': this.time_modify === 'disable',
                'message-one-info-hidden':  this.time_modify === 'hidden',
                'message-one-info-enable':  this.time_modify === 'enable',
                'message-one-info-modify':  this.time_modify === 'modify',
            }
        },
        ...
        org_isReadonly: function(){
            return this.org_modify === 'disable';
        },
    ...
    watch:{
        ...
        org: function(newVal, oldVal){
            this.dOrg = newVal;
        },
        orgId: function(newVal, oldVal){
            this.dOrgId = newVal;
        },
        orgNm: function(newVal, oldVal){
            this.dOrgNm = newVal;
        },
        org_modify: function(newVal, oldVal){
            this.dOrg_modify = newVal;
        },
    ...
    methods: {
        ...
        clickInfo(flag) {
            ...
            let flagConfig = this[flag + '_modify'];    // 点击栏位的modify配置
            console.log('click', flag, flagConfig);
            // 修改模式，触发修改事件
            if (flagConfig === 'modify') {
                switch(flag) {
                    case 'org':
                        this.$emit( 'orgclick', this.getResults() );
                        break;
                    case 'user':
                        this.$emit( 'userclick', this.getResults() );
                        break;
                    case 'time':
                        this.$emit( 'timeclick', this.getResults() );
                        break;
                }
            }
        },
        ...
        blur(flag, event){
            ...
            this.$emit( 'result', this.getResults() );
        },
        // input值改变
        change(flag, event){
...
<style scoped lang="less" rel="stylesheet/less">
    ...
    .message-one {
        ...
        .message-one-info-wrap {
            text-align: right;
            font-size: 13px;
        }
        .message-one-info-default {
            padding: 0 5px 0 10px;
        }
        .message-one-info-disable {
            cursor: not-allowed;
        }
        .message-one-info-hidden {
            display: none;
        }
        .message-one-info-enable {
            cursor: default;
        }
        .message-one-info-modify {
            cursor: pointer;
        }
    }
</style>