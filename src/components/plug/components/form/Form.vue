...
<script>
    ...
    export default {
        ...
        watch: {
            ...
            columns: {
                handler(newValue, oldValue) {
                    // this.tempColumns = this.resetColumns(newValue)
                    // let columns = this.clone(newValue)
                    // let rules = {}
        ...
        methods: {
                    ...
                    if (util.isPlainObject(column)) {
                        ...
                        if (column.rule) {
                            if (column.type == 'input') {
                                column.suffixIcon = () => {
                                    if (this.rules[column.prop].loading) return 'el-icon-loading'
                                }
                            }
                            newItem.push(
                                {
                                    ...
                                    filter: () => this.rules[column.prop].message
                                }
                            )
                        }
                        ...
                        if (column.length && column[column.length - 1].rule !== void 0) {
                            column.splice(
                                column.length - 1, 0,
                                {
                                    type: 'span',
                                    class: 'el-form-item__error',
                                    filter: () => this.rules[column[column.length - 1].prop].message
                                }
                            )
                        }
            ...
            init(){
                let columns = util.clone(this.columns)
                let rules = {}
                this.resetRules(columns, rules)
                this.rules = rules
