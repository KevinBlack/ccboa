...
const rules = {
  ...
  phone: {
    label: '请正确输入手机号码',
    regular: /^1\d{1}[0-9]\d{4,8}$/
  },
  email: {
    label: '请正确输入email',
    regular: new RegExp("^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$")
  }, //ok
  url: {
    label: '请正确输入链接',
    regular: /^((ht|f)tps?):\/\/([\w\-]+(\.[\w\-]+)*\/)*[\w\-]+(\.[\w\-]+)*\/?(\?([\w\-\.,@?^=%&:\/~\+#]*)+)?/
  },
  number: {
    label: '请输入数值',
  ...
  intp: {
    label: '请输入正整数',
    regular: new RegExp("^([+]?)\\d*$")
  },
  intn: {
    label: '请输入负整数',
    regular: new RegExp("^-[1-9]\\d*|0$")
  },
  zipcode: {
    label: '请正确输入6位邮编',
    regular: /^\\d{6}$/
  ...
  age: {
    label: '请正确输入年龄',
    regular: /^(?:[1-9][0-9]?|1[01][0-9]|120)$/
  },
  idcard: {
    label: '请正确身份证号码',
    regular: /^\d{8,18}|[0-9x]{8,18}|[0-9X]{8,18}?$/
  },
  username: {
    label: '仅允许字母开头，字母数字下划线组合5-16长度',
