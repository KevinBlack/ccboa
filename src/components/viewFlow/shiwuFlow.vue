            ...
            <div class="block steps_w">
              <el-scrollbar style="height:100%;">
                <el-steps direction="vertical" description>
                  <el-step v-for="item in tableData" :key="item.NODE">
                    <template slot="description">
                      <el-row class="step-self">
                        <el-row>
                          <el-col><h3>{{ item.NODE }}</h3></el-col>
                        </el-row>
                        <el-row style="padding: 20px; background: #F2F6FC;">
                          <el-col :span="12" style="height:30px">{{ item.DSC }}</el-col>
                          <el-col :span="12" style="text-align: right;">{{ item.EXCNM }} {{ item.TM }}</el-col>
                        </el-row>
                      </el-row>
                    </template>
                  </el-step>
    ...
    export default{
        ...
        data() {
            return {
                tableCols:[
                    ...
                    { label: "操作时间", prop: "TM", align: "center"},
                    { label: "处理记录", prop: "DSC", align: "center" },
                ],
                activeName: 'second',
                dialogVisible: false,
            }
        ...
        watch:{
          tableData(newvalue,oldvalue){
            //目的：改正便函查看流程时间数据错误问题
            //原因：便函写法不同，在获取组到建数据前已经执行了created,
          
              this.tableData&&this.tableData.map(item=>{
                if(item.TM){
                    item.TM=formatBData(item.TM)
                }else{
                    item.TM=''
                }
            })
