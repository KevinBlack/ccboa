<template>
  <div class="file_wp">
    ...
    >
      ...
      <input type="hidden" :id="uploadConfig.formId+uploadConfig.hiddenJson" name="jsonData"/>
    ...
    export default {
        props: {
            uploadConfig: {
                ...
                textFileHas: {type: Array, default: () => []},
                sysFileHas: {type: Array, default: () => []}
            }
        },
        watch: {
            // filesHas:function(newValue,oldValue){
...
//获取配置
            ...
            uploadFilesChange(e) {
                if (this.uploadConfig.sysFileHas && this.uploadConfig.sysFileHas.length > 0) {
                    this.$message({
                        type: 'warning',
                ...
                if (this.uploadConfig.textFileHas && this.uploadConfig.textFileHas.length > 0) {
                    this.$message({
                        type: 'warning',
                        message: '请先删除原文件后再上传新文件'
                    })
...
nowFileName = '.'+item.name.split(".")[item.name.split(".").length-1];
        ...
        //大小校验
            ...
            uploadFileNow(e) {
                ...
                PJF.html.getDom(this.uploadConfig.formId + this.uploadConfig.hiddenJson).value = PJF.util.json2str(condition);
                console.log("---formFileUpload----" +
                    ...
                    if (JSON.parse(data.Data_Cntnt).code=="SUCCESS") {
                        ...
                        fileList.forEach(item => {
                            datas.push({
                                ...
                                createrName: JSON.parse(localStorage.getItem("userInfo")).humanName,
