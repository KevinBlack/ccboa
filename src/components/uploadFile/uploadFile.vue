...
export default {
  ...
  methods: {
    ...
    getConfigInfo (){
       this.$post.postData(
        "getAttachConfig",
        JSON.stringify({function:'getAttachConfig', unitId: this.uploadConfig.unitId}),
        this.whiteCode
      ).then((res) => {
        if(res&&res.success){
          let newA = [];
    ...
    uploadFileNow(fileSizes) {
      ...
      if (!this.uploadConfig.id) {
        this.$emit("preserve", "uploadfile",null,null,null,true);
        return;
      } else {
    ...
    callBackFn(data) {
      ...
      if ((data.BK_CODE = "000000000000" && data.Data_Cntnt)) {
        if (JSON.parse(data.Data_Cntnt).success) {
          this.nowFilesList = [];
          this.$refs.filesLists0.value = '';
          this.$emit("preserve",null,null,null,null,true);
