        ...
        <div class="uploader-drop">
          <label class="uploader-btn" @click.stop="lockTips">
            ...
            <input
              ...
              name="upload"
              ref="filesLists0"
              :disabled="uploadConfig.subTempAddF==true||(!hldIsLock && uploadConfig.isLock==1)||uploadConfig.subliceseTip||uploadConfig.zlwhCheck"
              @click="resetInput"
          ...
          >&nbsp;{{filesLimit.attachWhiteList?'目前系统只支持'+(uploadConfig.isSpecial == 6?'xls、xlsx':filesLimit.attachWhiteList)+'格式附件上传':'请选择文件上传'}}{{filesLimit.attachSize?'，附件大小合计不超过'+filesLimit.attachSize+'M':'。'}}</span>
          <el-button
            v-if="false"
...
export default {
  props: {
    ...
    hldIsLock: { type: Boolean, default: () => false },
  },
  data() {
    return {
  ...
  methods: {
    ...
    lockTips(){
      if(this.uploadConfig.isLock != 1){
        ...
        if (!this.hldIsLock && this.uploadConfig.isLock == 1) {
          this.$message({
            type: "error",
            showClose: true,
            offset: 400,
       ...
       if(!this.hldIsLock && (this.uploadConfig.isLock == 1 || this.uploadConfig.subTempAddF)){
           $("#filesLists0").attr("disabled",true)
        }else{
          $("#filesLists0").attr("disabled",false)
        }
      return;
      }
    ...
    doUpLoad(files,nowRules,fileSizes){
      ...
      for (var i = 0, item; (item = files[i]); i++) {
        ...
        //限制xls ,xlsx  isSpecial ==6为 批量导入
        if(this.uploadConfig.isSpecial == 6&&!rulesImport.includes(nowFileName.toLocaleLowerCase())){
           this.$message({
            type: "error",
            showClose: true,
            offset: 400,
        ...
        if(this.uploadConfig.isSpecial != 6&&this.filesLimit.attachWhiteList&&!nowNames.includes(nowFileName)&& this.filesLimit.isActivated!=0&&this.uploadConfig.isSpecial == 4||this.uploadConfig.isSpecial == 5&&!rulesFw.includes(nowFileName.toLocaleLowerCase())){
          this.$message({
            type: "error",
            showClose: true,
            offset: 400,
        ...
        if( this.filesLimit.isLimitAttSize == 1 && fileSizes>this.filesLimit.attachSize*1000*1024&&this.uploadConfig.isSpecial != 6){
           this.$message({
              type: "error",
              showClose: true,
              offset: 400,
    ...
    callBackFn(data) {
      ...
      if ((data.BK_CODE = "000000000000" && data.Data_Cntnt)) {
        if (JSON.parse(data.Data_Cntnt).success) {
          this.nowFilesList = [];
          this.$refs.filesLists0.value = '';
          this.$emit("preserve", null);
          // this.$emit("initsearch", data.Data_Cntnt);
          ...
          this.$message({
            ...
            showClose: true,
            offset: 400,
            message: this.uploadConfig.isSpecial == 6?(JSON.parse(data.Data_Cntnt).message?JSON.parse(data.Data_Cntnt).message:'上传失败'):"上传失败",
            duration: 500
          });
        }
      } else {
