<template>
  <div class="file_wp" v-if="specialDialog">
        ...
        <template>
          <el-scrollbar>
            ...
            <!-- <el-checkbox-group v-model="checkFilesList" @change="handleCheckedFilesChange">
              <el-checkbox
                v-for="(file,fileNum) in nowFilesList"
                :label="file.name"
                :value="file.name"
                :key="file.name"
              >
    ...
    <div>
      <h3 class="noFile">无附件</h3>
      <div class="noFile_wrap">{{uploadConfig.checkedAllName}}</div>
    </div>
  </div>
</template>

<script type="text/javascript" src="../../../public/static/script/jsloader.js"></script>
<script>
import {
  Message,
  Loading
      ...
      if ((data.BK_CODE = "000000000000" && data.Data_Cntnt)) {
        if (JSON.parse(data.Data_Cntnt).success) {
         JSON.parse(data.Data_Cntnt).data.map((node)=>{
           this.nowFilesList.map((list,fileNow)=>{
             if(node.file_name == list.name){
               this.nowFilesList[fileNow].fileId = node.id;
             }
           })
         })
         this.$forceUpdate();
