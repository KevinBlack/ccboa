        ...
        <div class="uploader-drop">
          <label class="uploader-btn" @click.stop="lockTips">
            ...
            <input
              ...
              name="upload"
              ref="filesLists0"
              :disabled="uploadConfig.zlwhCheck"
              @click="removeName"
          ...
          >&nbsp;目前系统只支持 {{ uploadConfig.isSpecial == 3 ? "png、jpg、jpeg、gif" : filesLimit.attachWhiteList }} 格式附件上传，附件大小合计不超过{{filesLimit.attachSize}} M。</span>
          <el-button
            v-if="false"
...
export default {
  props: {
    ...
    uploadConfig: {
      ...
      isSpecial: { type: String, default: "" },
      callFunction:{ type: Boolean, default: () => false },
  ...
  created() {
    ...
    this.whiteCode = this.uploadConfig.newcode?this.uploadConfig.newcode:(this.uploadConfig.code.substr(0,this.uploadConfig.code.length-1)+(this.uploadConfig.code.substr(this.uploadConfig.code.length-1)>1?this.uploadConfig.code.substr(this.uploadConfig.code.length-1)-1:9));
    this.getConfigInfo();
  ...
  methods: {
    ...
    getConfigInfo (){
       this.$post.postData(
        ...
        if(res&&res.success){
          ...
          //模板范文单独处理
          if(this.uploadConfig.isSpecial == 5){
            this.filesLimit.attachWhiteList ='doc,docx,wps';//只能上传doc,docx
          }
    ...
    doUpLoad(files,nowRules,fileSizes){
      ...
      let nowNames = [".png",".jpg",".jpeg",".gif"];
      let rulesFw = [".doc",".docx",".wps"];
      let nowRuleNames = this.filesLimit.attachWhiteList ? this.nowRulesName :'';
      for (var i = 0, item; (item = files[i]); i++) {
        let names = '.'+item.name.split(".")[item.name.split(".").length-1].toLocaleLowerCase();
        //校验文件名符号规范
        // if (
        ...
        //   return;
        // }
        if(!nowNames.includes(names.toLocaleLowerCase()) && this.uploadConfig.isSpecial == 3){
          this.$message({
            type: "error",
            showClose: true,
            offset: 400,
          ...
          return;
        }//限制doc ,docx  isSpecial ==5为范文
        if(this.filesLimit.attachWhiteList&&!nowRuleNames.includes(names)&& this.filesLimit.isActivated!=0&&this.uploadConfig.isSpecial == 4||this.uploadConfig.isSpecial == 5&&!rulesFw.includes(names.toLocaleLowerCase())){
          this.$message({
            type: "error",
            showClose: true,
            offset: 400,
        ...
        if(this.filesLimit.isLimitAttSize == 1 && fileSizes>this.filesLimit.attachSize*1000*1024){
           this.$message({
              ...
              showClose: true,
              offset: 400,
              message: "附件总文件大小超过"+this.filesLimit.attachSize+"M,请先删除多余附件",
              duration: 1500
            });
            return;
        }
    ...
    uploadFileNow() {
      ...
      if (!this.uploadConfig.id) {
        this.$emit("preserve", "uploadfile",this.uploadConfig.formId == "formImg"?1:2);
        return;
      }
      // if (!this.isPreserve && !this.uploadConfig.id) {
    ...
    callBackFn(data) {
      ...
      if ((data.BK_CODE = "000000000000" && data.Data_Cntnt)) {
        if (JSON.parse(data.Data_Cntnt).success) {
          this.nowFilesList = [];
          this.$refs.filesLists0.value = '';
          if(this.uploadConfig.callFunction){
            this.$emit("search");
          }else{
            this.$emit("getDetail");
            this.$emit("preserve");
