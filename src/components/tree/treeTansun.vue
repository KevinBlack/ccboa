<template>
  <div class="treeWrap" ref='treeDialog'>
...
export default {
  ...
  props: {
    ...
    needFirdLoad:{
      default: false, //默认false,即首次加载禁用loadNode
      type: Boolean
    },
    isMulti:{
      default:false,//是否检查环节为多人模式,默认不检查
      type:Boolean
    }
  },
  data() {
    return {
      ...
      treeForm: {
        function: "selectUserByUnitTreeTy",
        unitCode: "",
  ...
  methods: {
    ...
    //懒加载数据
    loadNode(node, resolve,funcName) {
      ...
      if (node.level === 0) {
        ...
        if(funcName=='loadNode'||this.needFirdLoad){
    ...
    handleLoadTree(obj, resolve,level) {
      let urlInfo = "selectUserByUnitTreeTy";
      ...
      if (this.isCY == true) {
        ...
        if(level == 'root'){
         // this.getTreeInfo();
          ...
          this.treeForm.function = "selWfNodeTy";
        }else{
         this.treeForm = {
          flag: this.subOrgFlag,
          function:"selectUserByUnitTreeTy",
        ...
        this.treeForm.unitCode = (obj && obj.id) || "";
        console.log('判断this.isFirstLoad',this.isFirstLoad,'this.funcName',this.funcName,
        'level==root',level=='root')
        // if(this.isFirstLoad&&level=='root'&&this.funcName=='getTreeInfo'){//避免无部门配置信息时，重复加载人员树
        //   return;
        // }
        ...
        this.$httptan
          .postData(
           "A08462053",
            this.treeForm,
            "FUNCTION",
            {PAGE_JUMP:'',REC_IN_PAGE:''},
            )
          .then(res => {
            this.funcName='handleLoadTree';
    ...
    getTreeInfo(flag, fileterParams) {
      ...
      console.log(this.isMulti,'this.isMulti')
        if(this.isMulti){
          this.$api.bianhan.checkMulti({multiTenancyId:obj.MULTITENANCYID,tplNo:obj.PTPLNO,pcsAvyId:obj.PCSAVYID}).then(res => {
           this.singelCheckF = !res.isMulti; // 单选isMulti为false 多选isMulti为true
        });
        }else{
         // 单选singelCheck为true 多选singelCheckF为false
      ...
      this.nowCheckList = [];
      this.checkedCities = [];
         console.log(parms.pcsAvyId,parms,'params1')
        this.$httptan
          .postData(
           "A08462053",
            parms,
        ...
        .then(res => {
          ...
          if(res&&res.success){
            ...
            if (res.message == "Loading") {
              ...
              console.log('loadNode',this.node_has, this.resolve_has)
              this.loadNode(this.node_has, this.resolve_has,'loadNode');
              this.$nextTick(() => {
