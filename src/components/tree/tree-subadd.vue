  ...
  methods: {
    ...
    checkTree(now, check, hascheck) {
      ...
      this.$nextTick(() => {
        ...
        if (this.singelCheckF) {
          this.$refs[this.getTreeName()].setCheckedKeys([nowNode.unitCode]);
        }
        ...
        if(node.jsonUser&&node.jsonUser.length){
          node.jsonUser.map((item)=>{
            item.id = item.unitCode;
             this.checkIds.map((cid,nid)=>{
                if(cid == item.unitCode&&this.checkData[nid]&&this.checkData[nid].hideType){
                  item.disabled = true;
                }
              })
          })
    ...
    transformData(newv,canDo){
      if(!canDo){
        ...
        if(!this.loadingTreeCopy&&newv&&newv[0]&&newv[0].unitCode){
