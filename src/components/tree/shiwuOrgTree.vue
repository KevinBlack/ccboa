            ...
            <div v-for="(item,index) in nowCheckList" :key="index">
              <template>
                <div class="command_wrap">
                  ...
                  <div class="one_btn" v-if="!isSingle">
                    <el-button type="primary" size="mini" @click.native="upRow(index)" round>上移</el-button>
                  </div>
                  <div class="one_btn" v-if="!isSingle">
                    <el-button type="primary" size="mini" @click.native="downRow(index)" round>下移</el-button>
                  </div>
                  <div class="one_btn">
    ...
    loadNode (node, resolve) {
      ...
      if (this.isSearchAll) {
        ...
        this.$api.setting.organization.getTree({
          ...
          let data = []
          res.data.map(item => {
            data.push({ unitName: item.unitName, id: item.unitCode, leaf: item.lastUnit, selectSelf: item.selectSelf, unitClass: item.unitClass })
          })
          return resolve(data);
        }).catch(err => {
          this.loading = false
