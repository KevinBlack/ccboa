...
function WebOffice2015(officeConfig) {
  ...
  console.log('this.iWebOfficeTempName');
  console.log(officeConfig)
  // *****************************************************************************************************************************
  ...
  this.EditType = officeConfig.EditType; //文档编辑类型
  this.DataBase = "";
  ...
  // 设置浏览器Cookie
  this.INetSetCookie = (url, cookie) => {
    this.obj.INetSetCookie(url, cookie);
  }

  // 设置控件标题
  this.SetCaption = (captionName) => {
  ...
  this.WebOpen2 = (downloadLink) => {
    this.Status = "成功";
    var httpclient = this.obj.Http; // 设置http对象
    this.FileName = downloadLink;
    this.GetFileType(downloadLink);
    this.GetSingleFileName(downloadLink);
    console.log(this.ServerUrl + downloadLink, "打开文档路径");
    if (httpclient.Open(this.HttpMethod.Get, this.ServerUrl + downloadLink, false)) {
      if (httpclient.Send()) {
        console.log("httpclient.ErrorCode", httpclient.ErrorCode)
        console.log("httpclient.ErrorText", httpclient.ErrorText)
  ...
  this.WebOpen = (mBoolean) => {
    ...
    httpclient.AddForm("FormData", this.WebSendMessage()); // 这里是自定义json
    // 传输格式。
  ...
  /** @description: 值班动态成刊保存 */
  this.dutyTrendsAsAPublicationSave = data => {
    apiIndex.duty.saveChengKan(data).then(res => {
      this.officeConfig.dutyTrendsAsAPublicationSavedData = data || {}
      console.log('saveChengKan>res:', res, this.officeConfig.formId)
    })
  }
  ...
  this.WebSave = (nameFlag) => {
    ...
    if (this.WebSetAllowEmpty()) {
      this.GetSingleFileName(this.FileName);
      var randomNumb = nameFlag ? (Math.random() * 100000 + nameFlag + this.SingleFileName) : Math.random() * 100000 + this.SingleFileName;
      var mSavePath = this.getFilePath() + randomNumb;
      var mSaveResult = this.WebSaveLocalFile(mSavePath);
      if (this.officeConfig.pageFlag == 'signquoteWaitEntity' && this.officeConfig.taoHongFlag) {
        this.Alert('签报正文将保存在本地 ' + this.getFilePath() + '目录下，文件名为' + randomNumb);
      }
      if (!(mSaveResult == 0)) {
        this.Status = "保存本地文档失败！错误代码为：" + mSaveResult;
        return false;
    ...
    if (this.WebSetMaxFileSize(mSavePath)) {
      ...
      // localStorage.setItem("SingleFileName",this.this.SingleFileName)
      if (this.SAVEFILE(httpclient, mSavePath)) {
        var pubDatas = httpclient.ResponseText //正文data

        console.log(pubDatas, '<------获取信息--this.SAVEFILE(httpclient, mSavePath)--httpclient.ResponseText')
        console.log(this.officeConfig, '获取传人的数据')

        var ISO = httpclient.GetResponseHeader("ISO"); // 获取返回值
        if (this.officeConfig.tanSun == '1') {
          if (typeof pubDatas == 'object') {
            let pubMgie = JSON.parse(pubDatas.Data_Cntnt)
            if (this.officeConfig.isnewPage) {
              apiIndex.checkLz.checkLzAlterV({
                isKh: this.officeConfig.isKh,
                'adminType': 'admin',
                id: this.officeConfig.formId,
                xksqFileName: pubMgie.fileName,
                xksqFilePath: pubMgie.filePath,
                handleButton: '保存'
              }).then(res => {
          ...
          } else if (typeof pubDatas == 'string') {
            try {
              let starts = pubDatas.replace('"{', '{')
              let overs = starts.replace('}"', '}')
              let lasts = overs.replace(/\\/g, '')
              console.log(lasts, '获取overs')
              let pubMgieString = JSON.parse(lasts)
              if (this.officeConfig.isnewPage) {
                apiIndex.checkLz.checkLzAlterV({
                  isKh: this.officeConfig.isKh,
                  id: this.officeConfig.formId,
                  'adminType': 'admin',
                  xksqFileName: pubMgieString.Data_Cntnt.fileName,
                  xksqFilePath: pubMgieString.Data_Cntnt.filePath,
                  handleButton: '保存'
                }).then(res => {
              ...
              var shiwu = JSON.parse(httpclient.ResponseText);
              let Data_Cntnt = JSON.parse(shiwu.Data_Cntnt)
              if (this.officeConfig.isnewPage) {
                apiIndex.checkLz.checkLzAlterV({
                  isKh: this.officeConfig.isKh,
                  id: this.officeConfig.formId,
                  xksqFileName: Data_Cntnt.fileName,
                  xksqFilePath: Data_Cntnt.filePath,
                  handleButton: '保存'
                }).then(res => {
        ...
        } else if (this.officeConfig.tanSun == '2') {
          var WebText = this.WebObject.ActiveDocument && this.WebObject.ActiveDocument.Content.Text.replace(/[\n,\s]/g, ""); //获取word文档内容
          let starts = pubDatas.replace('"{', '{')
          let overs = starts.replace('}"', '}')
          let lasts = overs.replace(/\\/g, '')
          let pubMgieString = JSON.parse(lasts)
          console.log(pubMgieString, '获取数据pubMgieString')
          apiIndex.paraMeter.ManuscriptSaveZw({
            content: WebText,
            gId: this.officeConfig.formId,
            pathName: pubMgieString.Data_Cntnt.fileName,
            pubMainFilePath: pubMgieString.Data_Cntnt.filePath,
          }).then(res => {
            console.log('res:', res)
          })
        } else if (this.officeConfig.tanSun === '3') {
          /* let formDataObject = JSON.parse(localStorage.getItem('row'))
          console.log('部门信息清稿', formDataObject.pubMainFilePath, )
          let cleanDraftData = ''
          let isClear = ''
          if (this.officeConfig.cleanDraftFlag === 'true') {
            cleanDraftData = '清稿' + this.transitionDate()
            isClear = '1'
          } else {
            cleanDraftData = '恢复清稿' + this.transitionDate()
            isClear = '0'
          } */
          try {
            if (typeof pubDatas == 'object') {
              let pubMgie = JSON.parse(pubDatas.Data_Cntnt)
              console.log(pubMgie.fileName + ',' + pubMgie.filePath, '部门信息金格的地址')
              /*formDataObject.pubMainFilePath = pubMgie.fileName + ',' + pubMgie.filePath
              localStorage.setItem("row", JSON.stringify(formDataObject))
              console.log('部门信息备份接口', formDataObject.pubMainFilePath, )
              apiIndex.pubInfo.backups({
                mainContext: '',
                gaojianId: '',
                backupTitle: cleanDraftData,
                mainFileName: pubMgie.Data_Cntnt.fileName,
                mainFilePath: pubMgie.Data_Cntnt.filePath,
                pdnId: this.officeConfig.formId,
                remark1: 'DepInfo',
                isClear: isClear
              ...
              apiIndex.pubInfo.updataBulletin({
                content: WebText,
                depInfoPubId: this.officeConfig.formId,
                pubMainFilePath: pubMgie.fileName + ',' + pubMgie.filePath,
                jbNoStr: this.officeConfig.jbNoStr,
              }).then(res => {
              ...
              console.log(lasts, '获取overs')
              let pubMgieString = JSON.parse(lasts)
              /* formDataObject.pubMainFilePath = pubMgieString.fileName + ',' + pubMgieString.filePath
              localStorage.setItem("row", JSON.stringify(formDataObject))
              console.log("打印这个ROW的数据", row)
              console.log('打印第二个路径', formDataObject.pubMainFilePath,)
              apiIndex.pubInfo.backups({
                mainContext: '',
                gaojianId: '',
                backupTitle: cleanDraftData,
                mainFileName: pubMgieString.Data_Cntnt.fileName,
                mainFilePath: pubMgieString.Data_Cntnt.filePath,
                pdnId: this.officeConfig.formId,
                remark1: 'DepInfo',
                isClear: isClear
              ...
              apiIndex.pubInfo.updataBulletin({
                content: WebText,
                depInfoPubId: this.officeConfig.formId,
                pubMainFilePath: pubMgieString.Data_Cntnt.fileName + ',' + pubMgieString.Data_Cntnt.filePath,
                jbNoStr: this.officeConfig.jbNoStr,
                isClean: this.officeConfig.isClean
              }).then(res => {
                localStorage.setItem('bumenxinxi', JSON.stringify(res.data))
                console.log('_+_+_+_+_+_+_+_+_+__________', JSON.stringify(res.data))
              })
        ...
        } else if (this.officeConfig.tanSun === 'pubManuscript') {
          let starts = pubDatas.replace('"{', '{')
          let overs = starts.replace('}"', '}')
          let lasts = overs.replace(/\\/g, '')
          let pubMgieString = JSON.parse(lasts)
          apiIndex.pubStatistics.JointPublicationText({
            backupTitle: this.officeConfig.backupTitle,
            gaojianId: JSON.parse(this.officeConfig.gaojianId),
            mainContext: JSON.parse(this.officeConfig.mainContext),
            mainFileName: pubMgieString.Data_Cntnt.fileName,
            mainFilePath: pubMgieString.Data_Cntnt.filePath,
            pdnId: this.officeConfig.id
          }).then(res => {
            console.log('res:', res)
          })

        } else if (this.officeConfig.tanSun === 'clzw') {
          let starts = pubDatas.replace('"{', '{')
          let overs = starts.replace('}"', '}')
          let lasts = overs.replace(/\\/g, '')
          let pubMgieString = JSON.parse(lasts)
          ...
          let lasts = overs.replace(/\\/g, '')
          let pubMgieString = JSON.parse(lasts)
          let coff = JSON.parse(this.officeConfig.isSynthesisFormat)
          coff.depInfoPubId = this.officeConfig.id
          coff.pubMainFilePath = pubMgieString.Data_Cntnt.fileName + ',' + pubMgieString.Data_Cntnt.filePath
          apiIndex.pubInfo.getJbDetail(coff).then(res => {
            console.log(res)
          })
        } else if (this.officeConfig.tanSun == '4') {
          console.log(this.officeConfig)
          var WebText = this.WebObject.ActiveDocument && this.WebObject.ActiveDocument.Content.Text.replace(/[\n,\s]/g, ""); //获取word文档内容

          // 这里获取到返回值
          console.log(typeof pubDatas, '------>这里获取shiwu的数据类型')
          console.log(pubDatas, '------>这里获取shiwu的数据类型')
          if (typeof pubDatas == 'object') {
            let pubMgie = JSON.parse(pubDatas.Data_Cntnt)
            this.dutyTrendsAsAPublicationSave({
              id: this.officeConfig.formId,
              ckId: this.officeConfig.formId,
              fileName: pubMgie.fileName,
              filePath: pubMgie.filePath,
              qfIdeaIn: this.officeConfig.qfIdeaIn,
              remarkIn: this.officeConfig.remarkIn
            })
          } else if (typeof pubDatas == 'string') {
            try {
              let starts = pubDatas.replace('"{', '{')
              ...
              let lasts = overs.replace(/\\/g, '')
              console.log(lasts, '获取overs')
              let data = JSON.parse(lasts)
              const pathData = data && data.Data_Cntnt || {}
              if (Object.keys(pathData) <= 0) {
                data.BK_DESC && this.$message.error(`${data.BK_DESC}`)
                return false
              } else {
                this.dutyTrendsAsAPublicationSave({
                  id: this.officeConfig.formId,
              ...
              this.dutyTrendsAsAPublicationSave({
                id: this.officeConfig.formId,
                ckId: this.officeConfig.formId,
                fileName: Data_Cntnt.fileName,
                filePath: Data_Cntnt.filePath,
                qfIdeaIn: this.officeConfig.qfIdeaIn,
                remarkIn: this.officeConfig.remarkIn
              })
            }
          ...
          let lasts = overs.replace(/\\/g, '')
          let pubMgieString = JSON.parse(lasts)
          let file = pubMgieString.Data_Cntnt.fileName + ',' + pubMgieString.Data_Cntnt.filePath
          let data = {
            id: this.officeConfig.id,
            demoNoteId: file
          }
          apiIndex.sealManage.updateIntroduction(data).then(res => {
            console.log(res)
          })
        } else if (this.officeConfig.tanSun == 'schyd') {
          ...
          let data = {
            id: this.officeConfig.id,
            fileName: pubMgieString.Data_Cntnt.fileName,
            filePath: pubMgieString.Data_Cntnt.filePath,
          }
          apiIndex.secreat.saveMain(data).then(res => {
            console.log(res)
          })

        } else if (this.officeConfig.tanSun == "lijin") {
          if (typeof pubDatas == 'object') {
            let pubMgie = JSON.parse(pubDatas.Data_Cntnt)
            if (this.officeConfig.isnewPage) {
              apiIndex.checkLj.checkLzAlterV({
                isKh: this.officeConfig.isKh,
                'adminType': 'admin',
                id: this.officeConfig.formId,
                xksqFileName: pubMgie.fileName,
  ...
  /* 保存上传到服务器的数据*/
  this.WebSetMsgByName = (FieldName, FieldValue) => {
    this.ht.Add(FieldName, FieldValue);
  }

  /* 获取从服务器得到的数据*/
  this.WebGetMsgByName = (FieldName) => {
  ...
  this.WebUseTemplate = () => {
    // this.WebSave('beforeclear');//此处先保存清除痕迹 前正文
    ...
    // this.ServerUrl = 'http://127.0.0.1:8080/ecpweb/ecpJson.action';


    this.WebClearMessage(); //清除所有WebSetMsgByName参数

    if (this.LOADFILE(httpclient)) {
      if (this.VBAInsertFile("正文", this.getFilePath() + this.iWebOfficeTempName)) {
        this.Status = "套红成功";
        return true;
      } else {
  ...
  this.WebSaveImage = (ImageType, ImageName) => {
  ...
  this.WebInsertImage = (BookMark, ImageName, Transparent, ZOrder) => //插入远程图片：BookMark插入的书签的位置;ImageName远程图片的名称
    {
  ...
  this.WebGetFile1 = (LocalFile, LocalName, vName) => {
    this.ServerUrl = location.origin
    let downloadLink = '/ecpweb/getLocalFile.action?relativePath=' + LocalFile + '&fileName=' + LocalName
    this.Status = "成功";
    var httpclient = this.obj.Http; // 设置http对象
    this.FileName = downloadLink;
    this.GetFileType(downloadLink);
    ...
    if (httpclient.Open(this.HttpMethod.Get, this.ServerUrl + downloadLink, false)) {
      if (httpclient.Send()) {
        if (this.hiddenSaveLocal(httpclient, this, false, false)) {
          console.log(this.DownloadedFileTempPathName, '获取本地文档路径')
          this.VBAInsertFile(vName, this.DownloadedFileTempPathName)
          // this.WebAddBookMarks('draftInfo12', '这是12')
        }
      }
    }
  }
  /*下载服务器上的文件并保存在本地 */
  this.WebGetFile = (LocalFile, RemoteFile) => {
    var httpclient = this.obj.Http; //设置http对象
    httpclient.Clear();
    this.WebSetMsgByName("RECORDID", this.RecordID); //加载RecordID
  ...
  this.WebPutFile = (LocalFile, RemoteFile) => {
    var httpclient = this.obj.Http; //设置http对象
    httpclient.Clear();
    this.WebSetMsgByName("REMOTEFILE", RemoteFile); //取得远程文件名称
  ...
  /* 删除指定的本地文件或服务器上的文件 */
  this.WebDelFile = (LocalFile, RemoteFile) => {
    if (LocalFile == "") {
      if (RemoteFile != "") {
        var httpclient = this.obj.Http; //设置http对象
        httpclient.Clear();
  ...
  this.WebSaveLocal = () => {
    ...
    fileNamePath = this.obj.FileSystem.FileSaveAs(exts, this.officeConfig.saveName ? (this.officeConfig.saveName + this.FileType) : this.FileName); //this.FileName 显示名称
  ...
  this.WebSaveLocalTs = () => {
    ...
    if(this.officeConfig.saveName.indexOf('/')) {
      this.officeConfig.saveName = this.officeConfig.saveName.replace(/\//g,'、')
    }
    fileNamePath = this.obj.FileSystem.FileSaveAs(exts, this.officeConfig.saveName ? (this.officeConfig.saveName + this.FileType) : this.FileName); //this.FileName 显示名称
    if (fileNamePath == "") {
      return true;
  ...
  this.WebOpenLocal = () => {
    console.log('打开本地文档')
    console.log(this,this.FileType,'this.FileType打开本地文档');
    if(this.FileType=='.xls'||this.FileType=='.xlsx'){
      this.setShowDialog(this.ShowDialog.DialogOpen, this.getOpenDocSuffix(".xls"));
    }else{
      this.setShowDialog(this.ShowDialog.DialogOpen, this.getOpenDocSuffix(".doc"));
    }
    this.Status = "打开本地文档成功";
    this.SetCaption(this.UserName + "正在编辑文档");
  }


  /* 文档保存到本地 */
  this.WebSaveLocalFile = (FileName) => {
    try {
  ...
  /* 打开本地文档	*/
  this.WebOpenLocalFile = (filePath) => {
    console.log('打开本地文档filePath',filePath)
  ...
  /* 取得书签的值	*/
  this.WebGetBookMarks = (BMarksName) => {
    console.log(BMarksName, '获取书签的值')
    if (this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) // 判断是否存在该书签
    {
      const vName = this.obj.ActiveDocument.Bookmarks.Item(BMarksName).Range.Text;
      console.log(vName, '获取￥name')
  ...
  /* 光标处添加书签(新增功能)	*/
  this.WebAddBookMarks = (BMarksName, BMarksValue) => {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        if (!this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) // 判断是否存在该书签
        {
  ...
  /* 删除书签(新增功能) */
  this.WebDelBookMarks = (BMarksName) => {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        if (this.obj.ActiveDocument.BookMarks.Exists(BMarksName)) // 判断是否存在该书签
        {
  ...
  this.WebSetBookmarks = (vbmName, vbmValue) => {
    if (this.FileType == ".doc" || this.FileType == ".docx" || this.FileType == ".wps") {
      if (this.obj.ActiveDocument.ProtectionType == "-1") {
        try {
  ...
  //设置书签字体
  this.SetBookMarksFont = (bMarksName, fColor, fSize, fName, fBold, fItalic) => {
  ...
  /* 设置显示或隐藏工具栏				（需要修改控件代码） */
  this.WebToolsVisible = (ToolName, Visibled) => {
    this.Status = "该功能暂未提供!";
    return false;
  }

  /* 设置工具栏按钮是否有效			（需要修改控件代码） */
  this.WebToolsEnable = (ToolName, ToolIndex, Enabled) => {
    this.Status = "该功能暂未提供!";
    return false;
  }
  ...
  this.ClearRevisions = function () {
    // this.obj.ActiveDocument.Unprotect("123");
    try {
      this.obj.Activate(true);
      ...
      this.Alert("痕迹接受失败，错误原因：" + e.description);
      return false;
    }
  }
  ...
  this.VBAShowRevisions = function (mValue) {
    if (this.getDocType(this.FileType) == this.DocType.WORD &&
      ...
      this.Status = mValue ? "显示痕迹成功" : "隐藏痕迹成功";
      return true;
    } else {
  ...
  this.VBAProtectDocument = function (docType, password) {
    ...
    {
      this.obj.ActiveDocument.Protect(3, false, password); //第一个参数： 3可以复制  2不能复制
  ...
  this.GetDocumentPages = function () {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      return this.obj.ActiveDocument.Application.Selection.Information(4);
    } else if (this.FileType == ".xls" || this.FileType == ".xlsx") { //获取excel文件内容
      return 1;
    }
    return ''; //不是word
  }
  ...
  this.WebShow = function (blnValue) {
    if (this.getDocType(this.FileType) == this.DocType.WORD) {
      ...
      StatusMsg(this.Status)
    }
  }
  /*显示隐藏痕迹 */
  this.WebGetRevisions = function () {
    var Rev = this.obj.ActiveDocument.Revisions; //获取痕迹对象
    var Text = "";
    for (var i = 1; i <= Rev.Count; i++) {
  ...
  this.WebSetAllowEmpty = function () {
    console.log(this.FileType,'this.FileTyupe---------',this.officeConfig.fileName)
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      var WebText = this.WebObject.ActiveDocument && this.WebObject.ActiveDocument.Content.Text; //获取word文档内容
      if (WebText.length != 1) {
        return true; //文档有内容
      }
      return false; //文档没容
    } else if (this.FileType == ".xls" || this.FileType == ".xlsx") {
      var WebText = '123456qwertysdfghj'
      if (WebText.length != 1) {
        return true; //文档有内容
      }
      return false; //文档无内容
  ...
  this.WebDownloadFile_int = function (fn) {
    ...
    this.WebSetMsgByName("RECORDID", this.RecordID); // 加载RecordID
    this.WebSetMsgByName("OPTION", "LOADFILE"); // 发送请求LOADFILE
    httpclient.AddForm("FormData", this.WebSendMessage()); // 这里是自定义json传输格式。
    this.WebClearMessage(); // 清除所有WebSetMsgByName参数
  ...
  {
    this.ServerUrl = location.origin + location.pathname + 'ecpJson.action';
    var words = this.WebGetDocumentCont().length;
    var fileSize = this.WebFileSize(FileName);
    var documentPages = this.GetDocumentPages();
    var condition = {
      requestBody: JSON.stringify({
        "function": "doHandleMainWord",
        "id": this.officeConfig.formId || 'jksdi2w323df', //操作指南无id,
        "st": localStorage.getItem("st"),
        "multiTenancyId": this.officeConfig.multiTenancyId,
        "cleanDraftFlag": this.officeConfig.cleanDraftFlag,
        "taoHongFlag": this.officeConfig.taoHongFlag,
        "isHasRedHead": this.officeConfig.isHasRedHead,
        "unitId": this.officeConfig.unitId,
        "isQC": this.officeConfig.isQC,
        "maindept": this.officeConfig.maindept,
        "uniondept": this.officeConfig.uniondept,
        "sizeFact": words,
        "length": fileSize,
    ...
    httpclient.AddForm("_fw_service_id", "simpleTransaction");
    httpclient.AddForm("transaction_id", this.officeConfig.fwTranId);
    httpclient.AddForm("savePath", window.uploadSetFileConfig.savePath);
    httpclient.AddForm("transFlag", "p8_rp");
    httpclient.AddForm("remotePath", window.uploadSetFileConfig.remotePath);
    httpclient.AddForm("function", "doHandleMainWord");
    // httpclient.AddForm("fmtPrintId", this.officeConfig.fwTranId);
    httpclient.AddForm("adapterId", "ecpJson");
    httpclient.AddForm("jsonData", PJF.util.json2str(condition));


    httpclient.AddFile("upload", FileName); // 需要上传的文件
  ...
  this.LOADFILE = function (httpclient) {
    try {
      // this.ServerUrl = "http://localhost:8080/ecpweb/page/iWebOffice2015";
      // 用来保存文件的Server
      //改测试
      var downloadLink = location.origin + "/ecpweb/getLocalFile.action?relativePath=" + this.officeConfig.filePathTh + "&fileName=" + this.officeConfig.fileNameTh;
      console.log(this.officeConfig.filePathTh, this.officeConfig.fileNameTh)
      // var downloadLink = (location.host.indexOf('8080') > 0 ? location.origin : "http://128.32.96.143:8080") + "/ecpweb/getLocalFile.action?relativePath=" + this.officeConfig.filePathTh + "&fileName=" + this.officeConfig.fileNameTh;
      SetGraySkin.call(this); //设置控件皮肤
      if (this.WebOpen3(downloadLink)) // 文件在服务器上的相对路径 FileName
      {
        StatusMsg(this.Status);
        return true;
      } else {
    ...
    // {
    // 	if (httpclient.Send()) {
    // 		var aaa = httpclient.GetResponseHeader("MsgError");
    // 		if (aaa == "404") {
    // 			this.CreateFile();
    // 			this.getOfficeVersion();// 打开文档后，判断当前office版本
    // 			httpclient.Clear();
  ...
  this.Save = (FileName, is2003, FileType) => {
    if (this.getDocType(this.FileType) == this.DocType.PICTURE) {
  ...
  this.getOfficeVersion = () => {
    ...
    try {
      if (this.setVersion == -1) {
        getVersion = this.obj.ActiveDocument && parseFloat(this.obj.ActiveDocument.Application.Version) || '14';
  ...
  /* 获取文档类型 */
  this.getDocType = (fileType) => {
    let fileOfType = fileType.toLowerCase();
    if (fileOfType == ".doc" || fileOfType == ".docx" || fileOfType == ".wps") {
      return this.DocType.WORD;
    }
    if (fileOfType == ".xls" || fileOfType == ".xlsx" || fileOfType == ".et") {
      return this.DocType.EXECL;
    }

    if (fileOfType == ".tif" || fileOfType == ".jpg") {
      return this.DocType.PICTURE;
    }
    if (fileOfType == ".ppt" || fileOfType == ".pptx") {
  ...
  // 获取打开文档类型名称
  this.getOpenSuffixName = (fileType) => {
    var openSuffixName;
    switch (fileType) {
      case this.DocType.WORD:
  ...
  /* 设置打开窗口的类型 */
  this.setShowDialog = (thisType, exts) => {
    switch (thisType) {
      case this.ShowDialog.DialogOpen:
        console.log('setShowDialog',thisType,exts)
  ...
  /* --------------------------------------------水印功能 --------------------------------------------*/
  /* 添加水印 */
  this.AddWaterMark = (WaterMarkNmae) => {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.blnIE()) {
  ...
  /* 插入图片水印 */
  this.AddGraphicWaterMark = (WaterMarkNmae) => {
    if (this.FileType == ".doc" || this.FileType == ".docx") {
      if (this.blnIE()) {
        var intPageTotal = this.WebObject.ActiveDocument.Application.ActiveDocument.BuiltInDocumentProperties(14);
      } else {
  ...
  /* 退出打印预览 */
  this.PrintPreviewExit = () => {
    this.obj.PrintPreviewExit();
    this.obj.ActiveDocument.ActiveWindow.View.ShowFieldCodes = false;
  }

  /* 增加自定义工具栏按钮 */
  this.AppendTools = (Index, Caption, Icon) => {
    parseInt(Index);
  ...
  /* 按钮是否有效 */
  this.DisableTools = (Caption, Flag) => {
    var customtoolbar = this.obj.CustomToolbar;
  ...
  /* 自定义工具栏按钮是否显示 */
  this.VisibleTools = (Caption, Flag) => {
    var customtoolbar = this.obj.CustomToolbar;
  ...
  /* 更加书签插入图片 */
  this.InsertImageByBookMark = (Transparent, ZOrder) => {
    if (this.obj.ActiveDocument.BookMarks.Exists(this.BookMark)) {
  ...
  /* 下载文档	*/
  this.DownloadToFile = (DownFileName, SavePathName) => {
    var httpclient = this.obj.Http;
    var URL = this.WebUrl.substring(0, this.WebUrl.lastIndexOf("/"));
    httpclient.ShowProgressUI = this.ShowWindow; // 隐藏进度条
  ...
  /* 手写签批	*/
  this.HandWriting = (penColor, penWidth) => {
  ...
  /* 取消上一次签批 */
  this.RemoveLastWriting = () => {
    var handwritting = this.obj.Handwritting;
    handwritting.RemoveLast();
  }

  /* 显示某用户的签批 */
  this.ShowWritingUser = (bVal, username) => {
  ...
  this.GetDataToSend = () => {
    ...
    httpclient.Clear();
    var ReturnValue = httpclient.GetResponseHeader("RName"); // 获取返回值
    console.log("获取返回值ReturnValue", ReturnValue)
    var jsonObj = eval('(' + ReturnValue + ')');
    for (var i in jsonObj) {
  ...
  /* 解决打印预览里打印按钮没反应问题 */
  this.OleDialogPrint = (OLEFlag, bCancel) => {
    if (OLEFlag == 1) {
...
// 金格MultiPart方式在线打开文档
function Load(officeConfig) {
  console.log('--officeConfig--');
  console.log(officeConfig)
  try {
    // this.ServerUrl = this.officeConfig.serverUrl;
    var docArry = decodeURIComponent(this.officeConfig.fileName).split("/");
    ...
    this.UserName = localStorage.getItem("userInfo") && JSON.parse(localStorage.getItem("userInfo")) && JSON.parse(localStorage.getItem("userInfo")).humanName || '';
    this.FileName = "sample." + docType;
    this.FileType = "." + docType; //FileType:文档类型  .doc  .xls
    this.ShowWindow = true; //true显示进度条//false隐藏进度条
    console.log(this.obj, "初始化的判断");

    this.obj.style.ShowOpenProgress = true; //开启、关闭打开文档时的进度条
    //this.obj.WebCreateProcess(); //创建空进程避免打开慢
    this.SetCaption("正在编辑文档...");
    this.ShowMenuBar(officeConfig.ShowMenu); //控制菜单栏是否可以显示
    this.NewShowToolBar(officeConfig.ShowToolBar); //控制Office工具栏和自定义工具栏

    //  this.setEditType(officeConfig.EditType);
    SetGraySkin.call(this); //设置控件皮肤
    if (browser == "chrome") {
      ...
      var MyCookie = "JSESSIONID=" + "628AC4ED5E5AF75079DD877EF101F4B9";
      this.INetSetCookie(this.ServerUrl, MyCookie);
      //this.INetSetCookie(this.ServerUrl, document.cookie);
      ...
      // this.INetSetCookie(this.ServerUrl, varMyCookie);
...
function LoadURL() {
  try {
    // 全局搜索localhost ip改为测试 本地可看金格
    this.ServerUrl = location.origin; //服务器地址
    // this.ServerUrl = location.host.indexOf('8080') > 0 ? location.origin : "http://128.32.96.143:8080"; //服务器地址
    SetGraySkin.call(this); //设置控件皮肤
    if (this.WebOpen2("/ecpweb/getLocalFile.action?relativePath=" + this.officeConfig.relativePath + "&fileName=" + this.officeConfig.fileName)) // 文件在服务器上的相对路径 FileName
    {
      StatusMsg(this.Status);
      var docArry = decodeURIComponent(this.officeConfig.fileName).split("/");
      var docName = docArry[docArry.length - 1];
      var docType = docName.split('.')[docName.split('.').length - 1] || 'doc';
      docType = docType == 'undefined' ? 'doc' : (docType == 'doc' ? 'doc' : docType);
      this.FileName = "sample." + docType;
    }
  } catch (e) {
    StatusMsg(e.description);
...
function LoadServlet() {
  try {
    // this.ServerUrl = "http://localhost:8080/ecpweb/page/iWebOffice2015";               // 用来保存文件的Server
    // var downloadLink = (location.host.indexOf('8080') > 0 ? location.origin : "http://128.32.96.143:8080") + "/ecpweb/getLocalFile.action?relativePath=" + this.officeConfig.relativePath + "&fileName=" + this.officeConfig.servletName;
    var downloadLink = location.origin + "/ecpweb/getLocalFile.action?relativePath=" + this.officeConfig.relativePath + "&fileName=" + this.officeConfig.servletName;
    SetGraySkin.call(this); //设置控件皮肤
    if (this.WebOpen3(downloadLink)) // 文件在服务器上的相对路径 FileName
    {
      StatusMsg(this.Status);
...
//保存文档
function SaveDocument(iweb2015, needClose) {
  this.FileName = "sample" + iweb2015.FileType;
  if (iweb2015.WebSave()) { //交互OfficeServer的OPTION="SAVEFILE"
    if (needClose) {
      // iweb2015.WebClose();
      // window.close();
      iweb2015.Alert('保存成功');
    ...
    iweb2015.Alert(iweb2015.Status);
    StatusMsg(iweb2015.Status);
  }
}
...
function StatusMsg(mValue) {
  try {
    document.getElementById('StatusBar').innerText = mValue || '暂无信息';
  } catch (e) {
    return false;
  }
}
...
function SetGraySkin() {
  ...
  if (!this.WebSetSkin(0xdbdbdb, 0xeaeaea, 0xeaeaea, 0xdbdbdb, 0xdbdbdb, 0xdbdbdb, 0x000000))
...
function SendMessage() {
  ...
  this.WebSetMsgByName("TESTINFO", info); //USERNAME在后获取
  if (this.WebSendMessage()) { // 交互信息为INPORTTEXT
    this.Alert(this.WebGetMsgByName("RETURNINFO")); //USERNAME值为对应后台的key
  } else {
    this.Alert("客户端Web发送数据包命令没有合适的处理函数");
  }
  this.HidePlugin(1);
}

function isModify(state) {
  // if(this.obj.IsModify()){
  // 	this.Alert(this.obj.IsModify());
  // }
  if (!this) {
    return false;
  }
  console.log('this.obj515', this.obj)
  if (state) {
    typeof this.obj.IsModify === 'function' && this.obj.IsModify(0);
  }
  return this.obj.IsModify;
}

function setUserName(name) {
  if (this.obj.ActiveDocument) {
    this.obj.ActiveDocument.Application.UserName = name;
  }
}
//文档痕迹
function setDocState() {
  if (this.obj.ActiveDocument) {
    this.obj.ActiveDocument.ShowRevisions = false;
  }
}

function setXiuDingState(type) {
  if (this.obj.ActiveDocument) {
    this.obj.ActiveDocument.TrackRevisions = type; // 显示/隐藏修订标记
  }
}
//获取文档修订模式
