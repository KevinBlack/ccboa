    ...
    <div class="elementForm">
      <el-form ref="addForm" :rules="rules" :model="formData" label-width="160px">
        <el-form-item label="栏目类别" prop="columnCategory">
          <el-input v-model="formData.columnCategory" maxlength="80" :disabled="isColumTypeDisabled"></el-input>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      formData: { columnCategory: '' },
      options: [],
      isSave: false,
      btnType: '',
      tabsType: '',
      rules: {
        columnCategory: [{ required: true, message: '请输入栏目类别', trigger: 'blur' }],
      },
    }
  },
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          ...
          if (this.tabsType === '00002') {
            this.$api.duty.saveFormData({ ...this.formData, route: this.tabsType }).then((res) => {
