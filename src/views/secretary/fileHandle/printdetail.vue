      ...
      >
        <tbody>
          ...
          <tr>
            ...
            <td id="title" colspan="5" style="padding: 10px;line-height: 24px;">{{dataForm.title}}</td>
          </tr>
          <!-- <tr>
          ...
          <td id="title" colspan="5" style="padding-left: 10px; height: auto;">
            <ul>
              <li v-for="(item, indx) in dataForm.receiveIdeaList">
                <span style="font-weight: bolder; margin-right: 10px;">{{item.content}}</span>
                <span>{{dataForm.draftDepartment}}</span>
                <span style="display: inline-block; width: 68px; height: auto;">{{item.leaderName}}</span>
                <span>{{item.createTime}}</span>
          ...
          <td id="title" colspan="5" style="padding-left: 10px; height: auto;">
            <ul>
              <li v-for="(item, indx) in dataForm.processIdeaList">
                <span style="font-weight: bolder; margin-right: 10px;">{{item.content}}</span>
                <span>{{dataForm.draftDepartment}}</span>
                <span style="display: inline-block; width: 68px; height: auto;">{{item.leaderName}}</span>
                <span>{{item.createTime}}</span>
          ...
          <td colspan="4">
            <div class="qianF" v-for="item in dataForm.processIdeaList" :key="item.id">
              <div class="p1">{{item.content}}</div>
              <div class="p2">
                <span>
                  {{item.deptName}}&nbsp;
          ...
          <tr style="height: 70px;" v-for="(item, index) in dataForm.deptIdeaList" :key="item.id">
            <td
              v-if="!index"
              :rowspan="dataForm.deptIdeaList.length"
              style="width：20%;text-align:center;vertical-align:middle"
            >部门办理情况</td>
            <td
              colspan="4"
              :style="{borderBottom: index == dataForm.deptIdeaList.length - 1 ? null : '0px',borderTop: index == 0 ? null : '0px'}"
            >
              <div class="qianF">
                <div class="p1">{{item.content}}</div>
                <div class="p2">
          ...
          <tr style="height: 70px;" v-if="!(dataForm.deptIdeaList && dataForm.deptIdeaList.length)">
          ...
          <!-- <tr>
          <td style="text-align: center">部门办理情况</td>
          <td id="title" colspan="5" style="padding-left: 10px; height: auto;">
            <ul>
              <li v-for="(item, indx) in dataForm.processIdeaList">
                <span style="font-weight: bolder; margin-right: 10px;">{{item.content}}</span>
          ...
          <td id="title" colspan="5" style="padding-left: 10px; height: auto;">
            <ul>
              <li v-for="(item, indx) in dataForm.remarkList">
                <span style="font-weight: bolder; margin-right: 10px;">{{item.content}}</span>
                <span>{{dataForm.draftDepartment}}</span>
                <span style="display: inline-block; width: 68px; height: auto;">{{item.leaderName}}</span>
                <span>{{item.createTime}}</span>
  ...
  methods: {
      ...
      if (_this.type == "1") {
                ...
                for (let i = 0; i < 6; i++) {
                  ...
                  switch (arr[i]) {
                    ...
                    case "receiveIdeaList":
                      newSug.content = res.entity.receiveIdea
                        ? res.entity.receiveIdea
                        : "";
                      if (newSug.content) {
                    ...
                    case "callContentList":
                      newSug.content = res.entity.callContent
                        ? res.entity.callContent
                        : "";
                      if (newSug.content) {
                      ...
                      break;
                    case "deptIdeaList":
                      if (res.entity.deptIdea) {
                        _this.dataForm.deptIdeaList.push({
                          ...newSug,
                          content: res.entity.deptIdea
                        });
                      }
                      break;
                  }
        ...
        apis.docHandlingDetail(data).then(res => {
          if (res.code === "SUCCESS") {
            ...
            res.data.callContentList = res.callContent;
              ...
              for (let i = 0; i < 6; i++) {
                ...
                switch (arr[i]) {
                  case "remarkList":
                    ...
                    break;
                  case "leaderIdeaList":
                    newSug.content = res.data.leaderIdea
                      ? res.data.leaderIdea
                      : "";
                    if (newSug.content) {
                      _this.dataForm.leaderIdeaList.push(newSug);
                    ...
                    break;
                  case "receiveIdeaList":
                    newSug.content = res.data.receiveIdea
                      ? res.data.receiveIdea
                      : "";
                    if (newSug.content) {
                      _this.dataForm.receiveIdeaList.push(newSug);
                    }
                  ...
                  case "processIdeaList":
                    ...
                    break;
                  case "callContentList":
                    newSug.content = res.data.callContent
                      ? res.data.callContent
                      : "";
                    if (newSug.content) {
                      _this.dataForm.callContentList.push(newSug);
                    }
    ...
    splitContent(con, page) {
      ...
      Array.from(children_node).forEach((item, index) => {
        ...
        // 判断是否为 数组的最后一个并且 分页
        console.log(index === children_node.length - 1, "132");
        if (index === children_node.length - 1 && splitNode.length > 0) {
          ...
          children_node.forEach((item1, index1) => {
            if (index1 < children_node.length - 1) {
              height7 += $(item1)
                .find("td:first")
                .height();
            }
          ...
          $(children_node[children_node.length - 3])
            .find("td:last")
            .css({
              height:
                a4_h -
                height7 -
                $(children_node[children_node.length - 2])
                  .find("td:last")
                  .height() +
                "px"
            });
