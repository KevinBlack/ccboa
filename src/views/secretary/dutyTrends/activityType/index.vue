...
export default {
  ...
  methods: {
    ...
    loadData(isRefresh, pagination) {
      this.commonLoadListData(this.$refs.activityTypeDataTable,pagination, {
          filterState: '4',
          /** @description: 办结传1，其他不传 */
          processState: this.actionTabsName === 'tabs_done' ? '1' : undefined,
          ...(this.searchData || {})
      }, null, this.$api.duty.activityTypeList, res => {            
    ...
    delete_btn() {
      if (this.deleteArry) {
        this.$confirm("是否确定删除当前选中数据?", "提示", {
          ...
          this.$api.duty
            .deleteActivityTableData({ ids: this.deleteArry })
            .then((res) => {
              if (res.code == "SUCCESS") {
                this.deleteArry = "";
                this.$message({
