                ...
                <div v-for="(item, index) in formdata" :key="item.id" class="comp_list">
                  <template>
                    <div class="command_file">
                      <span
                        class="name down"
                        @click="downFile(item,null,formData.documentNo+item.fileName)"
                      >{{item.fileName}}</span>
...
export default {
  ...
  methods: {
    ...
    //详情数据
    loadList() {
      let text = JSON.parse(this.$route.query.detail);
      console.log("打印text", text);
      let text1 = {};
      this.isSave = text.id;
  ...
  created() {
    if (this.$route.query.detail || this.$route.query.id) {
      console.log("打印这个数据", this.$route.query.id);
      if (this.$route.query.id) {
        this.$api.duty
          .addressDataQuery({ id: this.$route.query.id })
          .then(res => {
            this.$route.query.detail = JSON.stringify(res.data);
            this.loadList();
          });
      } else {
        this.loadList();
