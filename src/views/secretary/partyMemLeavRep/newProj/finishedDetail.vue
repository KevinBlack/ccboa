        ...
        >
          <el-row>
            <el-col :span="12">
              <el-form-item label="报告人" prop="name">
                <el-select v-model="dataForm.name" disabled placeholder="请选择">
                  <el-option
          ...
          <el-row>
            ...
            <el-col :span="24">
              <el-form-item label="批示意见" prop="comments">
                ...
                <div class="d_flex" v-if="options && options.length!=0">
          ...
          <el-row>
            <el-form-item label="备注" prop="remark">
              <el-button
                v-show="this.dataForm.currentNode == '办结' "
                type="primary"
                size="mini"
              ...
              <div class="d_flex" v-if="remarks && remarks.length!=0">
                <el-row class="d_b100" style="margin: 5px 0;">
                  <el-col
...
export default {
  ...
  methods: {
    ...
    buttonClick(params) {
      switch (params) {
        ...
        case "收藏":
          // let deptTableHead = "";
          // const that = this;
          // deptTableHead = `${this.dataForm.curBank}党委成员离京报告表`;
          // // exportTable.getPdf(deptTableHead, this.formData.attchmentFileInfo, "meetzhifa", "A08462002", "biaoti", this)
          // this.$nextTick(() => {
          //   exportTable.getPdf(
          //     deptTableHead,
          //     that.dataForm.attchmentFileInfo,
          //     "loadPdf",
          //     that.$businessCode.msgzfj,
          //     deptTableHead,
          //     that
          //   );
          // });
        ...
        case "打印处理单":
          ...
          // this.openNewWindow('/partyMemLeavRep', {id: this.dataForm.id, fileName:JSON.stringify(this.codeFileName), filePath:JSON.stringify(this.codeFilePath)})
          break;
        case "操作指南":
          console.log("操作指南");
          this.caozuozhinan();
        ...
        case "维护":
          this.isadbled = false;
          console.log("维护");
          const baocunIndex = this.buttonList.findIndex(item => item.type === 'baocun')
          this.$set(this.buttonList, baocunIndex, {
            name: "保存",
            type: "baocun",
            show: true
          });
          const zuofeiIndex = this.buttonList.findIndex(item => item.type === 'zuofei')
          this.$set(this.buttonList, zuofeiIndex, {
            name: "作废",
            type: "zuofei",
            show: true
          });
    ...
    invalid() {
      ...
      console.log(this.remarks[this.remarks.length - 1].content);
      if (!this.uodata && this.remarks[this.remarks.length - 1].content) {
        ...
        this.$confirm("是否确定作废?", "提示", {
          ...
          .then(() => {
            const remark = this.remarks[this.remarks.length - 1].content;
            this.$api.secreatary.partyMemLeavRep
              .invalidProcess({
                id: this.dataForm.id,
                remark: remark
        ...
        .then(resData => {
          if (resData.code === "SUCCESS") {
            this.$nextTick(() => {
              ...
              this.options = resData.exeList;
              this.remarks = resData.remList;
              // this.dataForm.draftTime = formatData(new Date(resData.data.draftTime).getTime(), 'YYYY-MM-DD')
              ...
              if (
                resData.data.isReadFlag == "0" &&
                !["办结", "已失效"].includes(this.dataForm.currentNode) &&
                userInfo.humanCode == this.dataForm.oldUserCode
              ) {
                this.$set(this.buttonList, shouhuiIndex, { ...shouhuiObj, show: true });
              } else {
                this.$set(this.buttonList, shouhuiIndex, { ...shouhuiObj, show: false });
              }
              console.log(resData, "获取接口信息");
              if (this.dataForm.currentNode == "办结" && resData.flag) {
                this.isadbled = true;
                const weihuIndex = this.buttonList.findIndex(item => item.type === "weihu");
                this.$set(this.buttonList, weihuIndex, {
                  name: "维护",
                  type: "weihu",
                  show: true
                });
