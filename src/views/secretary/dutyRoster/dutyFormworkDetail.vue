<template>
  <div class="selectDept">
    <div class="formHeader">
      <div class="title-text">值班表报送</div>
    </div>
    <div>
      <el-form style="width:850px;display:inline-block;margin:0 12%">
        <el-form-item label="值班表模板">
          <el-col :span="20">
            ...
            <div class="tree_w" v-show="formdata.length">
              <el-scrollbar>
                <div v-for="(item,index) in  formdata" :key="item.id" class="comp_list">
                  <template>
                    <div class="command_file">
                      <span
                        class="name down"
                        @click="downFile(item,null,formData.documentNo+item.title)"
                      >{{item.fileName}}</span>
...
export default {
  ...
  data() {
    return {
      ...
      isSave: "", //上传后的ID
      uploadIS: true, //浏览框显隐
      savedisabled: true, //上传按钮是否可用
      resetdisabled: false, //重置按钮是否可用
  ...
  methods: {
    ...
    fileList(files) {
      this.formdata = files;
      console.log(this.formdata, this.formdata.length);
      this.$forceUpdate();
    },
    // 点击下载模板
    submission() {
    ...
    deleteRow(index) {
      this.$confirm("是否要删除当前附件?", "提示", {
        ...
        .then(res => {
          this.formdata = [];
          this.uploadIS = true;
          this.$api.duty.OnDutyDelBook({ id: this.isSave }).then(res => {
            this.$message({
              type: "success",
              message: "删除成功"
            });
      ...
      if (this.formdata.length == 1) {
        this.$api.duty
          ...
          .then(res => {
            ...
            this.$router.push({ path: "/secretary/dutyRoster/dutyTable" });
          });
      } else {
        this.$message({
          type: "warning",
    ...
    //详情数据
    loadList() {
      if (JSON.parse(this.$route.query.detail).attName) {
        var text = JSON.parse(this.$route.query.detail);
      } else {
        var text = this.formArr;
      }
      console.log(text, "转换数据");
      let text1 = {};
      this.isSave = text.id;
      text1.fileName = text.attName;
      text1.createTime = text.uploadTime;
      text1.createrName = text.sendPle;
      text1.id = text.attId;
      text1.filePath = text.attPath;
      this.sendStateFlag = text.sendState;
      this.formdata.push(text1);
      console.log(this.formdata, "从详情找到哦啊的");
      this.uploadIS = false; //隐藏浏览框
      if (text.sendState == "草稿") {
        this.submitdisabled = false;
      }
    },
    downFile(file) {
  ...
  created() {
    if (this.$route.query.detail || this.$route.query.id) {
      //  从首页进来 从数据获取
      if (this.$route.query.id) {
        this.$api.duty.dutyDataQuery({ id: this.$route.query.id }).then(res => {
          this.formArr = res.data;
          this.loadList();
          console.log(this.formArr, "获取到的数据");
        });
      } else {
        this.loadList();
      }
