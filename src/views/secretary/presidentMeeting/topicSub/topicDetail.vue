    ...
    background: white;"
      >
        <el-form :rules="rules" :model="formData" label-width="120px" ref="elForm">
          ...
          <el-row>
            <el-col :span="24" v-if="topType == '1'">
              <el-form-item label="退回原因" prop="remark">
                <el-row v-if=" (formData.ytContent &&　formData.ytContent.length==0)">
                  <el-input type="textarea" v-model="formData.yt_thyy" resize="none" disabled></el-input>
                </el-row>
                <div class="d_flex" v-if="formData.ytContent && formData.ytContent.length!=0">
                  <el-row class="d_b100" style="margin: 5px 0;">
                    <el-col
                      v-for="item in formData.ytContent"
                      :key="item.id"
                      style="background: rgba(224, 224, 224, 0.34); padding: 5px 10px"
                    >
                      <div style="word-break: break-all;">{{ item.content }}</div>
                      <div class="adviceInfo">
                        <!-- <span style="margin-right:10px">{{ item.ReBackTime }}</span> -->
                        <span>{{item.luokuan}}</span>
                        <br />
                        <span>{{item.ReBackTime}}</span>
                      </div>
                    </el-col>
                  </el-row>
                </div>
...
export default {
  ...
  methods: {
          ...
          if (!this.id) {
            this.$api.secreatary.meetings
              ...
              .then(res => {
                ...
                this.fileId = res.data.id + "1";
                this.uploadConfig.id = this.id + "1";
    ...
    getFileList() {
      let data = this.id + "1";
      this.$api.setSysConfig.getTextemList({ id: data }).then(res => {
        this.$nextTick(() => {
  ...
  created() {
    ...
    switch (Number(type)) {
      ...
      case 1:
        ...
        this.uploadConfig.id = this.$route.query.id + "1";
        sessionStorage.setItem("fileId", this.fileId);
