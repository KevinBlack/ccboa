...
<template>
  <div class="Newactivity">
    ...
    <div class="formHeader">
      <div class="shiwuTitle">{{unitName}}党委成员活动安排</div>
      <div class="content">
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="开始时间" prop="startTime">
                <el-time-select v-model="form.startTime" :picker-options="startTime"></el-time-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="结束时间" prop="endTime">
                <el-time-select v-model="form.endTime" :picker-options="endTime"></el-time-select>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  methods: {
        ...
        case "加至领导资料库":
          ...
          let parsm = {
            ...
            attendMemCnId:
              ...
              this.form.leaderCheckedValue.toString() //主持行领导id
          };
    ...
    // searchKeyword(v) {
    //   console.log(v);
    //   this.mobanList = this.guestVal.map(item => {
    //     return {
    //       value: item
    //     };
    //   });
    ...
    moban() {
      this.$api.secreatary.activety
        .MbanList({ startDate: this.form.startDate })
        .then(res => {
    ...
    details(id) {
      this.$api.secreatary.activety.Details({ id: id }).then(res => {
        ...
        const Arrays = res.data.leaderName.split(",");
        const arraysData = [];
        const IdArrays = [];
        Arrays.forEach(item => {
          const arrays = item.split("_");
          arraysData.push({
            leaderCnName: arrays[1],
            leaderHumanId: arrays[0]
          });
          IdArrays.push(arrays[0]);
        });
        console.log("打印这个数组", arraysData);
        for (const i in arraysData) {
          this.flag = true;
          for (const index in this.options) {
            if (
              arraysData[i].leaderHumanId === this.options[index].leaderHumanId
            ) {
              this.flag = false;
            }
          }
          if (this.flag) {
            this.options.push(arraysData[i]);
          }
        }
        console.log("打印这个数组1", this.options);
        this.leaderActiveDataId = res.data.data.leaderActiveDataId;
        console.log(this.form, "sss");
        this.form.leaderCheckedValue = IdArrays; //行领导专属组回显
        this.LookId = res.data.data.leaderActiveDataId;
      });
    },
    // 获取当前时间
    getTime() {
