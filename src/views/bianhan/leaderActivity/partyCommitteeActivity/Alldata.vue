...
*/
<template>
  <div class="partCommitteeActivity setCurrentAlldataCss">
...
export default {
  ...
  computed: {},
  methods: {
    bht_now(row, event, cell, column) {
      let arr = event.label;
      let oldDate = arr.split("[");
      var newDate = oldDate[1].slice(0, 10);

      this.$intent.goNewPage(this, {
        path: "/secretary/leaderActivity/List",
        query: { day: newDate, userId: row.userId }
      });
    },
...
<style scoped>
.setCurrentAlldataCss >>> .el-table .cell {
