    ...
    <table class="table breakBefore print-margin table" id="sortTable">
      <tbody>
        ...
        <tr style="height: 32px;">
          ...
          <td colspan="4">
            <div class="guoCh" v-for="item in formdata.processList" :key="item.id">
              <div class="guoCh1">{{item.content}}</div>
  ...
  methods: {
    ...
    getFileList() {
      this.$api.setSysConfig
        ...
        .then(res => {
          this.attchmentFileInfoZhi = res.data;
        });
    ...
    loadData() {
      this.$api.bianhan.underDeal({ id: this.$route.query.id,numYear:this.numyear }).then(res => {
        ...
        this.formdata.sendList = res.sendList;
          try{
            this.fontFamily=res.bHCbOrderDo.content.match(/彩虹粗仿宋|彩虹楷体|彩虹黑体|彩虹小标宋/ig)[0];
          }catch(e){
            this.fontFamily='彩虹粗仿宋';
          }
          try{
              let fontSizeArray=Array.from(new Set(res.bHCbOrderDo.content.match(/(font-size: )\w\w(px)/ig).sort()));
              this.fontSize=fontSizeArray[0].split(': ')[1]<'21px'?'21px':fontSizeArray[0].split(': ')[1];
          }catch(e){
            this.fontSize='21px'
          }        
