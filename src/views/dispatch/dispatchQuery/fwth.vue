      ...
      <el-col>
        <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm transition_wrap" :class="{mtdown:down}">
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="退回日期" prop="backRangTime">
                <el-date-picker
                  @change="backRangeTimeChange()"
                  value-format="yyyy-MM-dd"
                  v-model="backRangTime"
                  style="width:100%"
                  type="daterange"
                  align="right"
                  unlink-panels
  ...
  data() {
    return {
      ...
      tableCols: [
        ...
        { label: "退回原因", prop: "backReason", align: "left", minwidth:"20%" }
      ]
    };
  },
  methods: {
    exportExc(){
      exportTable.exportExcel("发文退回查询表",".el-table");
    },
    /**
     * 列表数据查询
    */
    getTableList(val) {
      ...
      this.$post.postData("searchBackOrder", JSON.stringify(Object.assign({},this.bank_select, this.pagination)), this.$businessCode.fawgl).then(res => {
    ...
    cellClick(row, column, cell, event){
      let isBackTypt = row.backNode==="收文"
