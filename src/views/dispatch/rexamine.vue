...
export default {
  ...
  methods: {
    handleLoadData(tabindex="1") {
      ...
      if(tabindex=="0") {
        this.$post.postData("getAgainOrderList", JSON.stringify(Object.assign(this.bank_countersignature_all,this.ruleForm, this.pagination)), this.$businessCode.fawgl).then(res => {
          if (res && res.success) {
    ...
    cellClick(row, column, cell, event){
      ...
      if(this.currentTab == '0' || this.currentTab == '1') {
        if (this.currentTab == '3') {
          if (row.sendType == '实物') {
            this.handleOpenPage('/rexamineDone', row.ID, readType)
          ...
          // .then(res => {
            if(this.currentTab == '0') {
              if (row.isFlowEnd == '已办') {
                ...
                this.handleOpenPage('/rexamineDispach', row.id, readType, "bd")
              } else if (row.isFlowEnd == '办结') {
                this.handleOpenPage('/rexamineFinishRed', row.id, readType)
              }
  ...
  created() {
    ...
    let transIndex = this.$route.query.rexIndex
    this.currentTab = this.$route.query.rexIndex||'1'
