...
export default {
  ...
  methods: {
    async maintenance(){
      let data={
        ...
        let obj1=await this.$post.postData(
            "getWithdrawOrderInfoAdmin",
            JSON.stringify(Object.assign(data,{function :"getWithdrawOrderInfoAdmin",})),this.$businessCode.fawgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
    ...
    toEdit() {
      let that = this;
      this.qzhs_bank.unitId = this.unitId;
