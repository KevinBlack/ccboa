...
export default {
  ...
  methods: {
    ...
    cellClick(row, column, cell, event){
      ...
      if(this.currentTab === '0') {
        // if (row.isFlowEnd === '已办') {
          this.handleOpenPage('/bankRecountersignDoneCopy', row.id, readType, this.unitId)
        // } else if (row.isFlowEnd === '待办') {
        //   this.handleOpenPage('/bankRecountersign', row.id, readType)
        // }
      } else if (this.currentTab === '1') {
        this.handleOpenPage('/bankRecountersign', row.ID, readType, this.unitId, "bd")
      } else if (this.currentTab === '2') {
        this.handleOpenPage('/bankRecountersignDone', row.ID, readType, this.unitId)
      } else if (this.currentTab === '3') {
        this.handleOpenPage('/bankRecountersignFinishRed', row.ID, readType, this.unitId)
      }
    },
    ...
    handleBatchDelete () {
      if (!this.deleteIds) {
        ...
        this.$confirm("是否确定删除?", "提示", {
          ...
          .then(() => {
            this.$post.postData('doDeleteByIdAgainSignOrderRecycle', JSON.stringify({ function: 'doDeleteByIdAgainSignOrderRecycle', ids: this.deleteIds, unitId: this.unitId }), this.$businessCode.fawgl).then((res) => {
              this.$message({
                type: 'success',
                message: '删除成功!'
              })
