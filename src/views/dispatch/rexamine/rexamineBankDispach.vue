<template>
  <div id="rexamineBankDispach" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <el-button
            size="small"
            v-if="chakanzw&&fromdata.sendType=='电子'"
            plain
            @click="drafting('0')"
          >查看正文</el-button>
          <el-button
            size="small"
            v-if="chulizw&&fromdata.sendType=='电子'"
            plain
            @click="drafting('2')"
          >处理正文</el-button>
          <el-button
            size="small"
            v-if="qinggao&&fromdata.sendType=='电子'"
            plain
            @click="doClearDraft(false)"
          >清稿</el-button>
          <el-button
            size="small"
            v-if="huifuqg&&fromdata.sendType=='电子'"
            plain
            @click="doClearDraft(true)"
          >恢复清稿前正文</el-button>
          <el-button
            size="small"
            v-if="!qinggao||fromdata.sendType!='电子'"
            ...
            @click="handleSend(1)"
          >发送</el-button>
          <el-button size="small" v-if="!qinggao&&teshufasong" plain @click="handleSend(2)">特殊发送</el-button>
          <el-button
            size="small"
            plain
            class="bank_grid_comtent_active"
      ...
      <div class="bank_dispach_file">
        <div class="bank_dispach_file_part">
          ...
          >
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="业务分类" :span="8" prop="businessType" ref="businessType">
                    <!-- <el-select v-model="fromdata.businessType" disabled placeholder="请选择" style="width:100%">
                      <el-option
                ...
                <el-col>
                  <el-form-item label="主办部门" prop="hostDepartment" ref="hostDepartment">
                    <el-input v-model="fromdata.hostDepartment" disabled name="hostDepartment"></el-input>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="15" v-if="fromdata.sendType=='电子'">
                  <el-form-item label="会签部门" prop="signDepartmentName" ref="signDepartmentName">
                    <!-- <el-input
                        ...
                        :disabled="readFields.signDepartmentName"
                        placeholder="请选择会签部门"
            ...
            <!-- 附件 -->
            <h6 class="bank_dispach_file_tit" v-if="fromdata.sendType=='电子'">附件</h6>
            ...
            <!-- 专栏 -->
            <h6 class="bank_dispach_file_tit" v-if="fromdata.sendType=='电子'">专栏</h6>
            <div id="scfj" class="height_90" v-if="fromdata.sendType=='电子'"></div>
            <div v-if="fromdata.sendType=='电子'">
              <el-row>
                <el-col>
            ...
            <!-- 公告部门 -->
            <h6 class="bank_dispach_file_tit" v-if="fromdata.sendType=='电子'">公告部门</h6>
            <div id="scfj" class="height_90" v-if="fromdata.sendType=='电子'"></div>
            <div v-if="fromdata.sendType=='电子'">
              <el-row>
                <el-col>
    ...
    >
      ...
      <div class="send_content">
        ...
        <el-divider v-if="handleSendType==1&&fromdata.sendType=='电子'"></el-divider>
        <el-row v-if="handleSendType==1&&fromdata.sendType=='电子'">
          <el-col :span="4">
      ...
      <el-card class="dialog_regain">
        <el-checkbox-group v-model="checkedsh">
          <el-checkbox
            ...
            :label="item.signOrderDepartmentId"
            @change="handleHQBMChange(item)"
            :key="index"
...
export default {
  ...
  methods: {
    ...
    toEdit() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          that.fromdata.againId = this.$route.query.againId;
          if (
            this.fromdata.currentNodeId === "004" &&
            this.fromdata.signDepartmentName &&
            this.fromdata.signState === ""
          ) {
            this.showHQ = true;
          } else {
            this.showHQ = false;
          ...
          that.$post
            .postData(
              ...
              JSON.stringify({
                ...
                sendOrderId: that.bank_dispach.id
              }),
      ...
      if (status) {
        ...
        switch (type) {
          ...
          case "dosend":
            ...
            break;
          case "gonggao":
            this.fromdata.realInline = this.fromdata.sendOrderNoticeInfo = gongId
              ? gongId + ";" + gongName
              : "";
    ...
    completeSending() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          // this.$post.postData('', JSON.stringify(this.bank_send))
          this.$post
            .postData(
              "selWfNode",
    ...
    //         sessionStorage.setItem("pageid", res.data.id)
    ...
    processSheet() {
      let routeData = this.$router.resolve({
        path: this.fromdata.sendType == "电子" ? "/dzfwcld" : "/swfwcld",
        query: {
    ...
    handleSend(val) {
      ...
      this.fromdata.realMain = "";
      if (this.fromdata.sendType == "电子") {
        if (val == 2) {
          this.dialogSendState = true;
        } else {
          // this.showSend = true; // 本地测试时打开，可以短路天阳接口
      ...
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            let transMessage = res.message;
            console.log("0" + transMessage);
            // 1216 fu gai long
            // let beginIndex = transMessage.indexOf("\[")
            // transMessage = transMessage.substring(beginIndex+1,transMessage.length)
            // let lastIndex = transMessage.lastIndexOf("\]")
            // transMessage = transMessage.substring(0,lastIndex)
    ...
    // 退回-确定
    handleBackOff() {
      let obj = Object.assign(this.fromdata, this.backFormData);
      this.$post
    ...
    handleSendHQPtmt(newIds, newNames) {
      var obj = Object.assign(this.bank_sendHQPtmt, {
    ...
    //恢复清稿操作
    backDrafting() {
      this.bank_doRecoverFile.id = this.$route.query.againId;
    ...
    async findTemplete(file) {
      ...
      this.bank_findTemplete.id = this.bank_dispach.id;
      await this.$post
        .postData(
          "findTemplete",
  ...
  created() {
    ...
    this.bank_dispach.id = this.bank_rexamine.sendOrderId = this.$route.query.id
      ? this.$route.query.id
      : "";
    this.againSendType = this.$route.query.againSendType;
    this.bank_doSend.id = this.bank_dispach.id;
    this.uploadConfig.id = this.$route.query.againId;
