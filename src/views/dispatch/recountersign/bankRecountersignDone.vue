    ...
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent" v-if="fromdata.isMutil == 1">
          ...
          <el-button size="small" plain @click="shxbr('sh')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb">收回多人会签</el-button>
          <el-button size="small" plain @click="shxbr('cb')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb">催办多人会签</el-button>
        ...
        <div class="grid_comtent" v-else>
          ...
          <el-button size="small" plain @click="shxbr('sh')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb">收回多人会签</el-button>
          <el-button size="small" plain @click="shxbr('cb')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb">催办多人会签</el-button>
          <el-button size="small" plain @click="handleUrge" v-if="!(fromdata.isShowSendXb=='1'&&!fromdata.isLastXb)">催办</el-button>
          <el-button plain size="small" @click="handleShowRegain" v-if="!(fromdata.isShowSendXb=='1'&&!fromdata.isLastXb)&&fromdata.currentNodeId!=800&&readFlag !=1 &&allowBack==true">收回</el-button>
          <el-button plain @click="flowScheme">查看流程</el-button>
          <!-- <el-button size="small" plain @click="collect('againSign', bank_dispach.id)">收藏</el-button> -->
              ...
              <el-row>
                ...
                <el-col :span="24">
                  <el-form-item label="附件标题" prop="attachTitle">
                    <el-input v-model="fromdata.attachTitle" type="textarea" resize="none" autosize disabled>
                  ...
                  <el-form-item label="修改后附件标题" prop="newAttachTitle">
                    <el-input v-model="fromdata.newAttachTitle" type="textarea" resize="none" autosize disabled></el-input>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                    ...
                    <template>
                      ...
                      <div class="fix_height">
                        <el-input
                          ...
                          v-if="(fromdata.processOpinionId || !(fromdata.processOpinionList && fromdata.processOpinionList.length!==0))||fromdata.isMutil != 1"
                          disabled
                        ></el-input>
                        <!-- <button
    ...
    <el-dialog title="查看依据文件" :visible.sync="showImportList">
      <table style="width: 100%;">
        ...
        <tr>
          ...
          <td><el-button type="text" style="text-align:left;padding-left:10px;line-height:25px;word-break:break-all;white-space:pre-line;" @click="viewFile(bank_import)">{{bank_import.title}}</el-button></td>
        </tr>
        <tr>
  ...
  methods: {
    ...
    doFinishManyAssist(){
      if(this.fromdata.processOpinion&&this.fromdata.processOpinion!=''){
        ...
        let params = Object.assign(copyFrom, {function:"doFinishManyAssist"});
        this.$post
          .postData("doFinishManyAssist", JSON.stringify(params), this.$businessCode.fawgl)
          .then(res => {
            if (res && res.success) {
              this.$message({
                type: "success",
    ...
    handleRegain () {
      ...
      this.$post.postData('withdrawSubTask', JSON.stringify(this.bank_regain), this.$businessCode.ggfawgl).then((res) => {
    ...
    toEdit() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          .postData(
            "checkTakeBack",
            JSON.stringify({function:"checkTakeBack",processInstId:that.bank_doSend.processInstId}),
            this.$businessCode.ggfawgl
          )
          .then(res => {
