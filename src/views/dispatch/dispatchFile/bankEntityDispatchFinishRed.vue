<template>
  <div id="bankEntityDispatchFinishRed" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <el-button size="small" plain v-if="fromdata.isSendSuperriorBank&&fromdata.textType=='请示'" @click="flowScheme(2)">查看上级行处理情况</el-button>
          ...
          <el-button size="small" plain @click="collect('sendSw', bank_dispach.id)">收藏</el-button>
          ...
          >
            ...
            <div>
              <el-row>
                ...
                <el-col>
                  <el-form-item label="附件标题" prop="attachTitle" ref="attachTitle">
                    <el-input v-model="fromdata.attachTitle" type="textarea" resize="none" autosize disabled></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
            </div>
...
export default {
  ...
  methods: {
    ...
    handleShowForced() {
      ...
      this.checkedsh = [];
      this.shData = this.shData.filter(item => {
    ...
    handleForced() {
      ...
      this.$post.postData('doWithdrawSw', JSON.stringify(this.forcedForm), this.$businessCode.fawgl).then(res => {
        if(res.success) {
          this.$message({
            type: "success",
            showClose: true,
    ...
    toEdit() {
      ...
      this.bank_dispach.id = this.bank_dispach.id
        ...
        : getFormId(this.$route.query.id,'bankEntityDispachFinishRedId');
      ...
      this.$post
        ...
        .then(res => {
          ...
          if(res.data.sendTime) {
            let sendTimeYear = parseInt(res.data.sendTime.slice(0,4)) 
            let sendTimeMonth = parseInt(res.data.sendTime.slice(5,7)) 
            let sendTimeDay = parseInt(res.data.sendTime.slice(8,10)) 
            let sendTimeHour = parseInt(res.data.sendTime.slice(11,13)) 
            let sendTimeSecond = parseInt(res.data.sendTime.slice(14,16))
            let openTimeYear = that.openTime.getFullYear();
            let openTimeMonth = that.openTime.getMonth()+1;
            let openTimeDay = that.openTime.getDate();
            let openTimeHour = that.openTime.getHours();
            let openTimeSecond = that.openTime.getMinutes()
            console.log(res.data.sendTime);
            console.log(sendTimeYear,sendTimeMonth,sendTimeDay,sendTimeHour,sendTimeSecond);
            console.log(openTimeYear,openTimeMonth,openTimeDay,openTimeHour,openTimeSecond);
            if (openTimeYear>sendTimeYear||openTimeMonth>sendTimeMonth||openTimeDay>sendTimeDay) {
              that.oneHour = false
            } else {
              let aa = ((openTimeHour*60+openTimeSecond)-(sendTimeHour*60+sendTimeSecond))/60
              console.log(aa);
              if (aa>1) {
                that.oneHour = false
              } else {
    ...
    // 保存
    preserve(type) {
      this.bank_dispach.id = getFormId(this.$route.query.id,'bankEntityDispachFinishRedId');
      ...
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            this.bank_addShiwuFw.id = this.bank_dispach.id = this.bank_doSendShiwuFw.id = getFormId(this.$route.query.id,'bankEntityDispachFinishRedId', res.data.id);
  ...
  created() {
    ...
    //初始化加载数据
    this.bank_dispach.id = this.uploadConfig.id = getFormId(this.$route.query.id,'bankEntityDispachFinishRedId');
    this.bank_rexamine.sendOrderId = this.forcedForm.sendOrderId = this.bank_doSendShiwuFw.id = this.bank_dispach.id;
