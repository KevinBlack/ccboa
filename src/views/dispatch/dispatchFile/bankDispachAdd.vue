<template>
  <div id="bankDispach" class="bank_dispach">
      ...
      <el-col class="bank_dispack_side">
        ...
        <div id="bankDispach" class="grid_comtent">
          ...
          <el-button
            ...
            plain
            class="bank_grid_comtent_active"
            @click="preserve('save',null,null,null,true)"
          >保存</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="drafting('2')">处理正文</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="handleSend(1)">发送</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="handleDoFinish">办结</el-button>
          <el-button
            size="small"
            plain
            class="bank_grid_comtent_active"
            v-if="showTWSM"
            @click="selectBackInfoListBySendOrderId"
          >退回说明</el-button>
          <el-button
            size="small"
            plain
            class="bank_grid_comtent_active"
            @click="collect('send', bank_dispach.id)"
          >收藏</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="flowScheme">查看流程</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="processSheet">打印处理单</el-button>
          <el-button size="small" plain class="bank_grid_comtent_active" @click="instructions">操作指南</el-button>
          <!-- <el-button
            size="small"
                  ...
                  <el-row>
                    ...
                    <el-col>
                      <el-form-item label="发文依据">
                        <!-- <el-col>
                          <el-button size="small" v-if="fromdata.signState!='0'" plain @click="handleImport">引入</el-button>
                        </el-col>-->
                      </el-form-item>
                        ...
                        <div class="border_yiju">
                          <el-row v-for="(item) in fromdata.sendBasis" :key="item.id">
                            ...
                            <!-- <el-col :span="4" class="btn_delImport">
                              <el-button type="text" v-if="fromdata.signState!='0'" @click="handleDeleteImport(item)">删除</el-button>
                            </el-col>-->
                          </el-row>
                        </div>
                      </el-form-item>
                    ...
                    <el-col :span="8">
                      <el-form-item label="保管期限" prop="keepLimit" ref="keepLimit">
                        <el-row>
                          <el-col :span="10">
                            <el-button disabled>期限鉴定表</el-button>
                          </el-col>
                          <el-col :span="14">
                            <el-select
    ...
    <el-dialog title="期限鉴定表" v-dialogDrag :visible.sync="dialogVisibleQXJD" class="dialog_backed">
      ...
      ></m-table>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="dialogVisibleQXJD = false">确 定</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      tableColsQXJD: [
        { label: "编号", prop: "numberNo", width: "50px" },
        { label: "条目描述", prop: "mark" },
  ...
  methods: {
    ...
    showCompDialog(data, status, type, params, dtype, radio, newPeopInfo) {
      let judge = true;
      let that = this;
        ...
        if (data.length) {
          ...
          if (type == "zhusong" || type == "chao" || type == "ben") {
            for (var cname in newPeopInfo) {
              newPeopConcat +=
                newPeopInfo[cname].join(cname.split("/")[2]) +
                cname.split("/")[0] +
                (cname.split("/")[1] == "hld" || cname.split("/")[1] == "ggry"
                  ? "、"
                  : "，");
            }
            names = newPeopConcat;
        ...
        switch (type) {
          ...
          case "hui":
            let obj = Object.assign(that.createSighWf, {
              ...
              sequenceSign: that.fromdata.sequenceSign,
              id: that.fromdata.id,
              deptIds: newIds,
              deptNames: newNames
            });
    ...
    //初始值 //input框内容
    toEditAddSend(oneTip, loadingFlag, stateF, needOnce) {
      let that = this;
      let taoSecret = "";
      ...
      that.$post
        ...
        .then(res => {
          ...
          //   that.fromdata.exceedWordReasonList.length&&that.fromdata.isOutDept!=1// isOutDept:1 //出部门
          // ) {
          //     //当前超限原因是上一处理人时
    ...
    SetDocNoHandle() {
      ...
      this.doSaveDocumentNumFuc.multiTenancyId = this.fromdata.multiTenancyId;
      this.doSaveDocumentNumFuc.id = getFormId(
        this.$route.query.id,
        "bankDispachId"
      );
      this.$post
        ...
        .then(res => {
          this.fromdata.documentNo =
            ...
            this.wh +
            this.nowBack;
          if (this.fromdata.directRange && this.fromdata.directRange !== "") {
            this.fromdata.title =
    ...
    handleSetDocNo(type) {
      let that = this;
      // this.SetDocNoVisible = true; // 未来会放开注释
      let day = new Date();
      if (type == 1) {
        that.docNoYear = day.getFullYear();
      } else {
        that.docNoYear = day.getFullYear() - 1;
      }
      ...
      that.$post
        ...
        .then(res => {
          ...
          if (res && res.success) {
            ...
            that.doSaveDocumentNumFuc.multiTenancyId =
              that.fromdata.multiTenancyId;
            ...
            that.fromdata.documentNo =
              ...
              that.nowBack;
            if (
              that.fromdata.isDirect == "是" &&
              that.fromdata.directRange &&
              that.fromdata.directRange !== ""
            ) {
              that.fromdata.title =
    ...
    doBackNoHandle() {
      ...
      // this.doBackNoFuc.year = this.docNoYear;
    ...
    handleSetLastyearDocNo() {
      ...
      that.$post
        ...
        .then(res => {
          // 未来会删掉
          if (res && res.success) {
            // that.docNoNum = res.data
            // that.selectredhead()
            that.DocNoDataList = res.data;

            // 未来会删掉
            let numHao = Object.values(res.data);
            that.setDocNum = numHao && numHao[0];
            that.doSaveDocumentNumFuc.redHead = that.getNumListForm.redHead;
            that.doSaveDocumentNumFuc.year = that.getNumListForm.year;
            that.doSaveDocumentNumFuc.multiTenancyId =
              that.fromdata.multiTenancyId;
    ...
    // 保存
    async preserve(type, loadingFlag, stateF, doBack, needOnce) {
      //doBack 退号时 保存标识
      ...
      //   return;
      // }
      var obj = Object.assign({}, this.fromdata, this.doSaveForAddSend);
      ...
      await this.$post
        ...
        .then(res => {
          if (res && res.success) {
            this.doSaveForAddSend.id = this.bank_dispach.id = getFormId(
              this.$route.query.id,
              "bankDispachId",
              res.data.id
            );
            ...
            this.toEditAddSend(null, loadingFlag, stateF, needOnce);
            successState = true;
          } else {
            this.$message({
              type: "error",
      ...
      if (this.fromdata.isHasRedHead != "true") {
        ...
        this.preserve(null, null, null, null, true).then(save => {
          if (save) {
            this.nowCheckList = [];
            this.checkedCities = [];
            // if(val==1 && !this.showSpecial && this.fromdata.isSeparate=='是') {
            //   this.$alert('若确认不进行特殊发送，请将“附件分别发送”改为“否”！', '提示', {
            //     confirmButtonText: '确定'
            //   });
            // } else {
          ...
          this.$post
            ...
            .then(res => {
              ...
              this.$message({
                ...
                duration: 1500,
                onClose: () => {
                  localStorage.setItem("tabRecord", "4");
                  window.location.href = "about:blank";
                  if (!(localStorage.getItem("st").indexOf("smarto") > -1)) {
                    window.opener.location.reload();
                  }
    ...
    handleQXJDRadio(val) {
      this.fromdata.keepLimit = val.limitDate;
      // this.preserve()
    }
  },
  watch: {
