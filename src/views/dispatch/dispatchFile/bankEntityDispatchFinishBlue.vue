<template>
  <div id="bankEntityDispatchFinishBlue" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <el-button size="small" plain @click="collect('sendSw', collectId)">收藏</el-button>
                    ...
                    <div :style="{'width':item.width}">
                      ...
                      >
                        <el-checkbox
                          ...
                          :value="user.deptIds"
                          :key="user.deptIds"
                        ><span >{{user.deptNames}}</span></el-checkbox>
...
export default {
  ...
  methods: {
    ...
    handleSendObj() {
      ...
      this.$post
        .postData(
          ...
          JSON.stringify({function: 'getTcDeptTypeDicList'}),
    ...
    handleSendConfirm() {
      ...
      if(this.checkedCities.length<=0) {
        ...
        let obj = Object.assign({}, this.fromdata);
        obj.function = "checkUser";
        // 添加校验所选部门是否有发文岗
        this.$post.postData("checkUser", JSON.stringify(obj), this.$businessCode.fawgl).then(res => {
          if (res && res.success) {
            // obj.isSpecial = 0; //发送
            obj.function = "doSendOpenSendSw";
    ...
    handleCheckedCitiesChange(value, n) {
      ...
      if (value === "outcheck") {
        ...
        this.$nextTick(()=>{
          ...
          this.fromdata.realNewMain = this.checkedCities.length?this.checkedCities.join(",")+";"+names.join(","):'';
          this.fromdata.realMain = this.checkedCities.length?this.checkedCities.join(",")+";"+names.join(","):'';
    ...
    toEditShiwuFw() {
      ...
      this.bank_dispach.id = this.bank_dispach.id
        ...
        : getFormId(this.$route.query.id,'bankEntityDispachFinishBlueId');
      this.bank_dispach.choseYear = this.$route.query.choseYear||"";
      console.log(this.bank_dispach);

      that.$post.postData(
        'getByfawenId', 
        JSON.stringify({function: 'getByfawenId', sendOrderId: that.$route.query.redId}),
      ...
      this.$post
        ...
        .then(res => {
          ...
          // that.secretTypeBackvalue(res.data.secretType || "商业秘密");
          ...
          if(res.data.realNewMain) {
            that.checkedCities = res.data.realNewMain.split(';')[0].split(',')
    ...
    // 保存
    preserve(type) {
      this.bank_dispach.id = getFormId(this.$route.query.id,'bankEntityDispachFinishBlueId');
      ...
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            this.bank_addShiwuFw.id = this.bank_dispach.id = this.bank_doSendShiwuFw.id = getFormId(this.$route.query.id,'bankEntityDispachFinishBlueId',res.data.id);
    ...
    checkedCities:{
      handler(neww){
        ...
        this.$nextTick(()=>{
          ...
          this.fromdata.newMain = names.join(";")
          this.fromdata.realNewMain = neww.length?neww.join(",")+";"+names.join(","):'';
  ...
  created() {
    ...
    //初始化加载数据
    this.bank_dispach.id = this.uploadConfig.id = getFormId(this.$route.query.id,'bankEntityDispachFinishBlueId');
