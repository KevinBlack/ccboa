...
export default {
  ...
  methods: {
    ...
    handleRegain () {
      ...
      this.bank_regain.multiTenancyId = this.fromdata.multiTenancyId;
    ...
    toEditShiwuFw() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          .postData(
            "checkTakeBack",
            JSON.stringify({function:"checkTakeBack",processInstId:that.fromdata.processInstId}),
            this.$businessCode.ggfawgl
          )
          .then(res => {
          ...
          //处理会签 、主送、抄送、本行发送
          if (that.fromdata.mainIds) {
            that.zhuCheckData = that.concatTreeData(
              that.fromdata.mainIds,
              that.fromdata.main,
              'song'
            );
          }
          if (that.fromdata.signDepartmentId) {
            that.huiCheckData = that.concatTreeData(
              that.fromdata.signDepartmentId,
              that.fromdata.signDepartment
            );
          }
          if (that.fromdata.copyIds) {
            that.chaoCheckData = that.concatTreeData(
              that.fromdata.copyIds,
              that.fromdata.copy,
              'song'
            );
          }
          if (that.fromdata.inlineIds) {
            that.benCheckData = that.concatTreeData(
              that.fromdata.inlineIds,
              that.fromdata.inline,
              'song'
            );
          }
    ...
    instructions() {
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            this.getFileInfo(that,res.data).then(resF => {
              if (resF.state) {
                let data = this.$router.resolve({
                  path: "/openAndSave_Word",
                  query: {
