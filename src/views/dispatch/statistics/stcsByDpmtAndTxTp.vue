        ...
        <el-scrollbar style="width:100%">
          <div class="tableWrap">
            <el-table 
              ...
              class="el-table"
            >
              <el-table-column label="办结发文数量统计表（按部门/文种统计）" align="center">
                <el-table-column type="index" v-if="tableData.length" width="50" label="序号" align="center"></el-table-column>
                <el-table-column v-for="item in titleList " :prop="item.value" :label="item.value" :key="item.id"
...
export default {
  ...
  methods: {
    ...
    handlerSearch () {
      ...
      this.params.function = 'statistDeptTextType';
      this.$post.postData('statistDeptTextType', JSON.stringify(this.params),this.$businessCode.fawgl).then((res) => {
        if(res.success){
    ...
    exportExc(){
       exportTable.exportExcel("办结发文数量统计表（按部门/文种统计）",".el-table");
    },
    setTableWidth(){
