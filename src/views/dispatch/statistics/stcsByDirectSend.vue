    ...
    <el-form>
      ...
      <el-col :span="6">
        <el-form-item label="统计模式:">
          <el-radio-group v-model="params.type" @change="handleTitleChange">
            <el-radio label="1">按部门</el-radio>
            <el-radio label="2">按直发范围</el-radio>
            <el-radio label="3">按直发种类</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-col>
...
const tableByDmp = [
  { label: "制发单位名称", value: "unitName" },
  { label: "发文数量", value: "fwcou" },
  { label: "直发发文数量", value: "zfcou" },
  { label: "直发率", value: "zfl" }
];
const tableByScope = [
  { label: "直发范围", value: "tname" },
  { label: "直发发文数量", value: "cou1" }
];
const tableByType = [
  { label: "制发单位名称", value: "部门" },
  { label: "行领导讲话", value: "行领导讲话" },
  { label: "情况通报", value: "情况通报" },
  { label: "规章制度", value: "规章制度" },
  { label: "参阅资料", value: "参阅资料" },
  { label: "转发的行外公文", value: "转发的行外公文" }
];
export default {
  name: "StcsByType",
  data() {
    return {
      ...
      columnList: tableByDmp,
      tableTitle: "直发发文统计表（按部门统计）",
      excelTime:'',
    };
  },
  methods: {
    ...
    handleTitleChange(val) {
      ...
      if (this.params.type == "1") {
        this.tableTitle = "直发发文统计表（按部门统计）";
        this.columnList = tableByDmp;
      } else if (this.params.type == "2") {
        this.tableTitle = "直发发文统计表（按直发范围统计）";
        this.columnList = tableByScope;
      } else if (this.params.type == "3") {
        this.tableTitle = "直发发文统计表（按直发种类统计）";
        this.columnList = tableByType;
      }
      this.$forceUpdate();
    },
    handlerSearch() {
      ...
      this.params.function = "statistDeptDirect"
      this.$post
        .postData(
          "statistDeptDirect",
    ...
    summation(param) {
      ...
      if (this.params.type == "1") {
        ...
        if (!isNaN(decimals)) {
          sums[columns.length-1] = (decimals * 100).toFixed(2) + "%";
        } else {
