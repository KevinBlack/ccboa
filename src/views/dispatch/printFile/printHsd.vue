  ...
  <div class="tablePrint">
    ...
    >
      <tbody>
        ...
        <tr>
          <td style="width: 11%; text-align: center">收回单位</td>
          <td id="documentNo" style="width: 26%; padding-left: 10px;">{{fromdata.withdrawUnit}}</td>
          <td style="text-align: center">收回人</td>
          <td id="draftUser" style="padding-left: 10px;">{{fromdata.withdrawUser}}</td>
          <td style="text-align: center">收回时间</td>
          <td id="draftUser" style="padding-left: 10px;">{{fromdata.withdrawTime}}</td>
        </tr>
        <tr>
          <td style="text-align: center">收回原因</td>
          <td id="title" colspan="5" style="text-align:left;padding-left:10px;line-height:25px;word-break: break-all;white-space: pre-line;">{{fromdata.withdrawReason}}</td>
        </tr>
        <tr>
          <td style="text-align: center">被收回单位</td>
          <td id="title" colspan="5" style="text-align:left;padding-left: 10px;">{{fromdata.recycledUnit}}</td>
        </tr>
        <tr>
          <td style="text-align: center">被收回部门</td>
          <td id="title" colspan="5" style="text-align:left;padding-left: 10px;">{{fromdata.recycledDept}}</td>
        </tr>
        <tr>
          <td style="text-align: center">被收回公告部门</td>
          <td id="title" colspan="5" style="text-align:left;padding-left: 10px;">{{fromdata.tackbackedGgsend}}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    ...
    initData() {
      let that = this;
      console.log(888, this.$route.query.unitId)
      let receiveselect = {
      ...
      this.$post
        ...
        .then(res => {
          ...
          if(res.data.isSpecialHead && res.data.isSpecialHead == '1') {
            ...
            that.draftOrganTitle = res.data && res.data.withdrawUnit + res.data.sendType + "收回单";
          }
        })
        .catch(erro => {
          // this.$message({
