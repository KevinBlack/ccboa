      ...
      <el-form :model="formData" label-width="120px" :rules="rules">
        ...
        >
          ...
          <el-table-column label="会议材料" align="center">
            <template slot-scope="scope">
              <div v-if="scope.row.attchmentFileInfo && scope.row.attchmentFileInfo.length">
                <el-button type="primary" @click="getFile(scope.row, scope.$index)">编辑会议材料</el-button>
              </div>
              <div v-else-if="scope.row.attchmentFileInfo.length == 0">
                <el-button type="primary" @click="getFile(scope.row, scope.$index)">上传会议材料</el-button>
              </div>
          ...
          <el-table-column prop="isGzzd" label="是否是规章制度类议题" align="center" width="150">
            <template slot-scope="scope">
          ...
          <el-table-column prop="isLegal" label="是否已通过合规性检查" align="center" width="150">
            <template slot-scope="scope">
        ...
        <el-form>
          <el-form-item label="会议材料要求" style="color:red">
            <div style="display:flex;flex-direction:row;justify-content:flex-start;">
              ...
              <el-button style="margin-left:10px" @click="dowMoBan()" type="primary">下载模板</el-button>
            </div>

            <p>1、请根据模板调整汇报材料格式，并以"审议《XXXXXX》/听取XXX的汇报/研究XX有关事宜"等形式命名</p>
            <p>2、请以OA正式发文的格式调整会议材料格式</p>
            <p>3、请将整套会议材料 (含汇报材料、签报材料、其他附件等) 转成PDF格式并打包上传</p>
          </el-form-item>
        </el-form>
      </el-row>
      <upload-file-ts
...
export default {
  ...
  mixins: [minixs, shiwuLineWord, openOrDownLoadFile,viewDraft],
  props: {},
  data() {
    return {
      fileNameText: "上传会议材料",
  ...
  methods: {
    ...
    save_btn() {
      for (let i = 0; i < this.hYData.length; i++) {
        console.log(this.hYData[i], "this.hYData");
        this.$api.secreatary.meetings
    ...
    loadData() {
      this.$api.secreatary.meetings.getAdDetail({ id: this.id }).then(res => {
        ...
        this.moBanList.fileName = res.data.ytAttachTitle;
        this.moBanList.filePath = res.data.ytAttach;
        console.log(this.moBanList, "moBanList");
        ...
        console.log(this.formData.jzTime, "this.formData.jzTime");
