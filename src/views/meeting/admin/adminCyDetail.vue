<template>
  <div class="adminCyDetail">
    <div class="headerClick">
      <div class="right">
        <el-button @click="buttonClick('关闭')" plain class="bank_grid_comtent_active">关闭</el-button>
        <el-button @click="buttonClick('收藏')" plain>收藏</el-button>
        <el-button @click="buttonClick('操作指南')" plain>操作指南</el-button>
        <!-- <el-button @click="buttonClick('关闭')" plain class="bank_grid_comtent_active">关闭</el-button>
        <el-button @click="buttonClick('发传阅')" plain v-if="formData.cyCount=='1'">发传阅</el-button>
        <el-button @click="buttonClick('已阅')" plain v-if="formData.isReadflag=='0'">已阅</el-button>
        <el-button @click="buttonClick('填写传阅意见')" plain v-if="formData.isReadflag=='0'">填写传阅意见</el-button>
        <el-button
          @click="buttonClick('传阅收回')"
          plain
          v-if="formData.cyCount=='1'&&formData.twocyList.length>0"
        >传阅收回</el-button>
        <el-button @click="buttonClick('打印处理单')" plain>打印处理单</el-button>
        <el-button @click="buttonClick('收藏')" plain>收藏</el-button>
        <el-button @click="buttonClick('操作指南')" plain>操作指南</el-button>
        <el-button @click="buttonClick('维护')" plain>维护</el-button>-->
      </div>
    </div>
      ...
      <div class="bank_dispach_file">
        <div class="process_content">
          ...
          <el-col :span="24">
            ...
            <el-form-item v-if="formData.pbIdeaList" label="办理意见">
              <el-input
                type="textarea"
                :autosize="{ minRows:3}"
                resize="none"
            ...
            <el-form-item label="传阅说明">
              ...
              <template v-if="formData.cyExplainList && formData.cyExplainList.length > 0">
...
export default {
  ...
  methods: {
    ...
    buttonClick(params) {
      switch (params) {
        ...
        case "已阅":
          ...
          break;
          if (formData.curbankId == "111111111") {
            let deptTableHead =
              this.formData.curban +
              this.formData.draftDepartment +
              this.formData.type +
              "（参会单）";
          } else {
            let deptTableHead =
              this.formData.draftDepartment + this.formData.type + "（参会单）";
          }
          exportTable.getPdf(
            deptTableHead,
    ...
    hyConList1() {
      this.$api.meeting
        .hyConList({ id: this.formData.mainDocId })
        .then((res) => {
