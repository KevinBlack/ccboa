          ...
          <div v-if="suggessObj.length>0">
            <el-row v-for="(item,index) in suggessObj" :key="index">
              ...
              >
                <div>
                  <div>{{item.label}}</div>
                  <el-button @click="addSuggess(index,item.label)" :disabled="(finalYJ&&item.label=='最终意见')||item.delete == 1||(item.label=='最终意见'&&item.value&&item.value.length!=0)">新增意见</el-button>
                  <!--  -->
                </div>
              </el-col>
              <el-col
                ...
                "></el-input>
                  ...
                  <el-row class="m-10 flex_ac">
                    ...
                    <el-col :span="8">
                      ...
                      :disabled="item.disabled == 1"></el-input>
                    </el-col>
                     ...
                     <el-col :span="2">用户Id</el-col>
                      ...
                      <el-col :span="8" style="text-align:center">
                        <el-button type="primary" @click="scYj(innerItem.id,index,item.label)" 
                         :disabled="item.delete == 1">删除</el-button>
                      </el-col>
                   </el-row>
                </div>
              </el-col>
    ...
    async maintenance(){
      let data={
        ...
        let obj1=await this.$post.postData(
            "getSysSupervise",
            JSON.stringify({function :"getSysSupervise"
        ...
        let obj2=await this.$post.postData(
            ...
            JSON.stringify(
              {function :"selectSysSuperviseOpinion",superviseDirect:this.$route.query.sign}
              ),this.$businessCode.zxdb
          ).then(res => {
            return res.data;
          })
      ...
      if(!id){
        for (let i1 = 0; i1 < this.suggessObj.length; i1++) {
          if(label == this.suggessObj[i1].label){
            this.suggessObj[i1].value.splice(this.suggessObj[i1].value.length-1,1)
           // return
          }
        }
        this.$forceUpdate();
    ...
    handelsave(type) {
      if(type == 'fj'){
        this.$post
                ...
                JSON.stringify({    
                    function: "selectSysSuperviseFile",
                    superviseDirect: this.$route.query.sign,
                    id: this.$route.query.id,
                    manager:"",
                    unitId:this.$route.query.unitId||'',
                    superviseDeptId:this.$route.query.superviseDeptId||'',
     ...
     this.bcType=1;
      ...
      //修改后Log
     this.$post
        .postData(this.url, JSON.stringify(Object.assign(this.subData,{manager:'',unitId:this.$route.query.unitId||''})), this.tradeLine)
        .then(res => {
          if (res.success) {
            if(type==="uploadfile"){
            ...
            this.$message({type: "warning",message: res.message||'保存失败',});
            successState = false;
          }
