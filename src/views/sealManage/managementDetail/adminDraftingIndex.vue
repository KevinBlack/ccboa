...
export default {
    ...
    data(){
        return{
            ...
            tableCols: [
                {label: '申请时间', prop: 'Stmp_Aply_Crt_Tm'},
                {label: '编号', prop: 'Stmp_Engr_Aply_Bsn_ID'},
                {label: '申请单位', prop: 'Stmp_Blng_InsID'},
                {label: '刻制印章名称', prop: 'Stmp_Dspl_Inst_Nm'},
                {label: '刻制日期', prop: 'engravingDate'},
                {label: '当前环节', prop: 'Stmp_Aply_StCd'},
                {label: '操作人', prop: 'Stmp_Aply_Psn_Nm'},
            ],  // 表格的项
            tableData: [],  // 表格的数据
            btnsList: [
                {name: '查询', show: true},
    ...
    filters: {
        // 过滤办结表数据。
        filterDoneTable: function(tableData, stock){
            if ( !Array.isArray(tableData) ) { return tableData; }
            let newArr = [];
            let eacape_stcds = stock.eacape_stcds;  // 环节转移对象
            for (var i = 0, len = tableData.length; i < len; i++) {
                let temp = {}, key;
                for (key in tableData[i]) {
                    if ( tableData[i].hasOwnProperty(key) ) {
                        let keyVal = tableData[i][key];
                        temp[key] = keyVal;    // 用原来的值赋值

                        // 如果是环节字段，且有对应的转义值，则转义
                        if ( key === 'Stmp_Aply_StCd' && eacape_stcds.hasOwnProperty(keyVal) ) {
                            temp[key] = eacape_stcds[keyVal];
                        }

                        // 如果是申请单位字段，且有值。则给申请单位编号添加申请单位的名字
                        if ( key === 'Stmp_Blng_InsID' &&  keyVal) {
                            let orgId = keyVal;                             // 申请单位编号
                            let orgNm = tableData[i]['Stmp_Blng_Inst_Nm'];  // 申请单位名称
                            temp[key] = orgNm + '（' + orgId + '）';
                        }
                    }
                }
                newArr.push(temp);
            }
            console.log('filterDoneTable:', newArr, stock);
            return newArr;
        }
    },
    methods: {
        ...
        querySubmit(){
            ...
            const options = {warningLevel: 1, warningTitle: '查询印章刻制信息列表失败！'};
            this.$api.sealManage.markMgr.getMarkMgrList(params, options).then(res => {
        ...
        toSealDetailPage(type = 'see', selects){
            ...
            query.id = selects[0].Stmp_Engr_Aply_Bsn_ID;  // 印章刻制申请业务编号
            // 跳转到废止文档页面。 query相当于带查询参数?a=jk&b=2
            this.$intent.goNewPage(this, {
                path: '/sealManage/managementDetail/adminDrafting',
                query: query,
            });
        },
    },
    ...
    created: function(){
        ...
        window.jk = this;
        this.eacape_stcds = this.$sealUtil.allNodes;                // 所有的环节
