<template>
    <!-- 这是印章管理》管理员查看》（管理员）印章销毁首页 -->
    <div class="adminSealDestroyDraftIndex">
        <!-- 查询条件 -->
...
export default {
    name: 'adminSealDestroyDraftIndex',
    components: {
        tTable,
    },
    data(){
        return{
            ...
            tableCols: [
                ...
                { label: '编号', prop: 'Stmp_Dstr_Aply_Bsn_ID' },
                { label: '申请单位', prop: 'Stmp_Kep_Inst_Nm' },
                { label: '当前环节', prop: 'Node_Nm'},
                { label: '操作人', prop: 'Cur_Pcsg_Psn_Nm' }
            ],  // 表格的项
            tableData: [],  // 表格的数据
            btnsList: [
                {name: '查询', show: true},
    ...
    methods: {
        ...
        querySubmit(){
            ...
            const options = {warningLevel: 1, warningTitle: '查询印章销毁信息列表失败！'};
            this.$api.sealManage.sealDestroy.getSealDestroyApplyList(params, options).then(res => {
        ...
        toSealDetailPage(type = 'see', selects){
            ...
            query.id = selects[0].Stmp_Dstr_Aply_Bsn_ID;  // 印章销毁申请业务编号
            // 跳转到印章销毁文档页面。 query相当于带查询参数?a=jk&b=2
            this.$intent.goNewPage(this, {
                path: '/sealManage/managementDetail/adminSealDestroyDraft',
                query: query,
            });
        },
    },
