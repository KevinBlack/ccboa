<template>
  <div class="maintain">
    <div class="handleButton">
      ...
      <!-- <el-button v-if="!isEdit" type="primary" @click="saveNew_btn">保存并新建</el-button> -->
      <el-button v-else type="primary" style="margin-left: 20px" @click="deleteTemplate_btn">删除模板</el-button>
      <el-button type="primary" @click="$router.go(-1)">关闭</el-button>
    </div>
    <div class="formHeader">
      <div class="title-text">印章红头维护</div>
    </div>
    <div class="process_content" id="bank_dispach_content">
      ...
      <el-form :rules="rules" ref="ruleForm" :model="formData" label-width="140px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="序号" prop="xuhao">
              <el-input v-model="formData.xuhao"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="16">
            <el-form-item label="机构名称" prop="organ">
              <el-input v-model="formData.organ" :disabled="true"></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="文件字头" prop="header">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="字号种类" prop="numberType">
              <el-input v-model="formData.numberType"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="字数限制" prop="numbers">
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="模板标签" prop="mouldLable">
              <el-input v-model="formData.mouldLable" :disabled="isEdit"></el-input>
            </el-form-item>
            <el-form-item>
        ...
        <el-row>
          <el-col :span="18">
            <el-form-item label="模板" prop="attachNm">
              <el-input v-model="formData.attachNm" :disabled="isEdit" readonly="readonly"></el-input>
          ...
          <div v-for="(item,index) in pubTplPath" :key="index" style="display:inline-block;margin-left:15px;">
            <el-button class="btn" :span="2" @click="deleteRowSys(index,item.id)" :disabled="isEdit">删除</el-button>
            <el-button class="btn" :span="2" @click="downFile(item)" :disabled="isEdit">下载</el-button>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        header: "",	 //文件字头
        numberType: "",	 //字号种类
        numbers: "",	 //字数限制 （注：配置为“0”时，正文字数将不受限制）
        mouldLable: "",	 //模板标签
        attach: "",	 //附件ID
        attachNm: "",	 //附件名称
        attachPath: "",	 //附件地址
      ...
      rules: {
        ...
        // attachNm: [{ required: true, message: '请选择模板', trigger: 'change' }],
      },
      title: '选择部门',
      showOrgTree: false,
      isSingle: false,
  ...
  methods: {
    ...
    //选择模板赋值
    fileList(files) {
      this.formData.attachNm = files[0].fileName
      this.formData.attachPath = files[0].filePath
      this.formData.attach = files[0].id
      this.pubTplPath = []
    ...
    createFile(id) {
      let params = {
        id: id + '',
        fileId: this.formData.attach,
        fileType: '9000408',
      }
      if(this.formData.attachNm){
        this.$api.setSysConfig
          .saveFile(params)
          .then((res) => {
    ...
    getOrgList(org) {
      if (this.orgOrDeptId == '1') {
        ...
        this.$set(this.formData, 'organ', org.unitName)
        this.$set(this.formData, 'organId', org.id)
      } else {
        this.$set(this.formData, 'organ', org.unitName)
        this.$set(this.formData, 'organId', org.id)
      }
    ...
    save_btn() {
      this.$refs.ruleForm.validate((valid) => {
        ...
        if (valid) {
          // if (!this.id) {
            console.log(this.formData, '获取formDaata')
            this.$api.pubInfo.updateInfoTemplate(this.formData).then((res) => {
              console.log(res)
              if (res.code == 'SUCCESS') {
                this.$message({
                  message: '保存成功',
    ...
    getFileTopWordNm() {
      this.$api.setSysConfig
        .getTextemList({ id: this.$route.query.id + '', fileType: '9000408' })
        .then((res) => {
          this.$nextTick(() => {
    ...
    // this.$api.paraMeter.paraMeterList({ attachNm: 'depInfoBase' }).then((res) => {
    //   console.log(res.data[0])
    //   this.pubTplPath_another[0].fileName = res.data[0].tplTopNm
    //   this.pubTplPath_another[0].filePath = res.data[0].tplTopPath
    // })
    //   // this.$api.setSysConfig
    //   //   .getTextemList({ id: '10800040810327', fileType: '80004081' })
    //   //   .then((res) => {
    //   //     this.$nextTick(() => {
    //   //       this.pubTplPath_another = res.data
    //   //       this.uploadConfig.filesHas = this.pubTplPath_another
    //   //     })
    //   //   })
    //   this.$api.pubInfo
    //     .getJbList({ depInfoPubId: '-10000' })
    //     .then((res) => {
    //       if (res.data) {
    //         if (res.data.page.length === 1) {
    //           this.accessoryHide = false
    //         } else {
    //           this.accessoryHide = true
    //         }
    //       }
    //     })
    //     .catch(function (err) {
  ...
  created() {
    ...
    if (this.id) {
      this.$api.pubInfo.collocateTemplate({ id: this.id, organId: this.$route.query.organId}).then((res) => {
        console.log(res)
        this.formData = res.data
        this.getFileTopWordNm()
      })
    } else {
      this.$set(this.formData, 'organ', this.$route.query.parentUnitNm)
      this.$set(this.formData, 'organId', this.$route.query.parentUnitId)
    }
  },
}
</script>
