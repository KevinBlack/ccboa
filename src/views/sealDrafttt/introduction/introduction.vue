...
<template>
  <div class="introduction" id="introduction">
    ...
    <div class="headerClick">
      ...
      <el-button
        @click="$router.push('/sealManage/useSeal/uplode')"
        :style="{ display: 'none' }"
      >uplode</el-button>
    ...
    >
      ...
      <el-row>
        <el-col :span="12">
          ...
          <el-form-item
            label="申请部门"
            prop="departFirstDepartment"
            v-show="hanjie"
            ref="departFirstDepartment"
          >
            <el-input v-model="form.departFirstDepartment" :disabled="hanjie"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="印章类别" prop="markType" ref="markType">
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="经办人电话" prop="tel" ref="tel">
            <el-input v-model="form.tel" :disabled="readFields.tel" maxlength="11"></el-input>
        ...
        <el-col :span="12">
          <el-form-item label="用印/生效时间" prop="useTime" ref="useTime">
            <el-input v-model="form.useTime" :disabled="readFields.useTime"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="有效天数" prop="usedDay" ref="usedDay">
            <el-input v-model="form.usedDay" :disabled="readFields.usedDay"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-col>
          <el-form-item label="人员列表" prop="rtfAssetList1" ref="rtfAssetList1">
            <el-row></el-row>
            <el-row>
              <el-col style="text-align: right; margin-bottom: 5px">
                <el-button type="primary" @click="addUseSealInfo" :disabled="documentNoNum1=='2'">增加</el-button>
                <el-button type="danger" @click="delUseSealInfo" :disabled="documentNoNum1=='2'">删除</el-button>
              </el-col>
            </el-row>
            <el-table
              border
            ...
            >
              ...
              <el-table-column label="姓名" align="center">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.name" :disabled="documentNoNum1=='2'"></el-input>
                </template>
              </el-table-column>
      ...
      <el-row>
        <el-col>
          <el-form-item label="发往单位类型" prop="sendType" ref="sendType">
      ...
      <el-row>
        <el-col :span="23">
          <el-form-item label="有权签字人审批" prop="ldBj" ref="ldBj">
            <el-radio-group
              v-model="ldBj0"
              :disabled="editedIdeaFields.ldBj"
              v-if="
                  !editedIdeaFields.ldBj || !(form.ldBjss && form.ldBjs.length != 0)
                "
              @change="changeLdBj0"
            >
            ...
            <el-input
              ...
              :disabled="editedIdeaFields.ldBj"
              v-if="
                  !editedIdeaFields.ldBj || !(form.ldBjs && form.ldBjs.length != 0)
                "
            ></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.ldBj">
          <i class="icon-font el-icon-edit select-chang" @click="showWriteList('ldBj')"></i>
        </el-col>
      </el-row>

      <el-row>
        <el-col :span="23">
          <el-form-item label="部门主要负责人审批" prop="fzBj" ref="fzBj">
            <el-radio-group
              v-model="fzBj0"
              @change="changeFzBj0"
              v-if="
                  !editedIdeaFields.fzBj || !(form.fzBjs && form.fzBjs.length != 0)
                "
        ...
        <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.fzBj">
          <i class="icon-font el-icon-edit select-chang" @click="showWriteList('fzBj')"></i>
        </el-col>
      </el-row>
      <!-- <el-row>
        <el-col>
      ...
      <el-row v-if="form.sendType === '1'">
        <el-col :span="23">
          <el-form-item label="印章保管人意见" prop="flIdea" ref="flIdea">
            <el-input
              v-if="
                  !editedIdeaFields.flIdea || !(form.flIdeas && form.flIdeas.length != 0)
                "
              type="textarea"
              resize="none"
              v-model="form.flIdea"
        ...
        <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.flIdea">
          <i class="icon-font el-icon-edit select-chang" @click="showWriteList('flIdea')"></i>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="23">
          <el-form-item label="承办部门处理意见" prop="ldIdea" ref="ldIdea">
            <el-input
              type="textarea"
              resize="none"
              v-model="form.ldIdea"
              ...
              v-if="
                  !editedIdeaFields.ldIdea || !(form.ldIdeas && form.ldIdeas.length != 0)
                "
            ></el-input>
            <div class="d_flex">
              <el-row class="d_b100" style="margin: 5px 0;">
        ...
        <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.ldIdea">
          <i class="icon-font el-icon-edit select-chang" @click="showWriteList('ldIdea')"></i>
        </el-col>
      </el-row>
    ...
    <!--
      发送上级行弹窗
    -->
    <!-- <el-dialog title="请选择上级行单位" :visible.sync="heigherOrgDialog">
    ...
    <!-- <el-form-item label="行外单位">
          <m-table
            ...
            :isIndex="true"
            :isHandle="false"
            :tableData="heigherOrgTableData"
            :tableCols="heigherOrgTableCols"
            :sortable="false"
            :isPagination="false"
            :isSingle="true"
          />
    ...
    <!-- </el-form>
      <div slot="footer">
        <el-button @click="heigherOrgDialog = false">取消</el-button>
    ...
    <el-dialog
      ...
      width="50%!important"
    >
      <el-form ref="ThForm" label-width="100px">
        <el-row>
          <el-col :span="23">
...
export default {
  ...
  created() {
    // this.notOpenPage();
    /** 获取当前部门信息 */
    let JsxId = sessionStorage.getItem("jieshaoxinId");
    if (JsxId) {
      this.sendId = JsxId;
  ...
  methods: {
    ...
    scjsxBtn() {
      if (this.form.sendType == "1") {
        this.departFirstDepartment = this.form.sendDept;
      } else if (this.form.sendType == "0") {
        this.departFirstDepartment = this.form.sendDeptName;
      }
      let departFirstDepartment = this.departFirstDepartment;
    ...
    getDetailIntroduction() {
      this.$api.sealManage
        .detailIntroduction({ id: this.$route.query.id || this.sendId ,numYear:this.numYears})
        .then(res => {
          ...
          if (res.data.rtfAssetList) {
            this.form.rtfAssetList = JSON.parse(res.data.rtfAssetList);
          }
          if (this.form.ldBjs) {
    ...
    getLink() {
      ...
      this.$api.sealManage.getLink(params).then(res => {
        ...
        console.log(this.ifAnNiu, "权限。。。。。。。");
        console.log(this.form.currentNode, "qicao........");
        if (
          this.ifAnNiu ||
    ...
    showCompDialog(data, status, type, params, dtype) {
      ...
      if (status) {
        ...
        console.log("下一处理人ID", this.nextUserId);
        console.log("下一处理人数据", data[0].id);
    ...
    MessageReminder() {
      let param = {
        ...
        infoType: "介绍信用印", //类型（模块名称中文（例如 收文，发文））
        infoTitle: this.form.title, //业务标题
        infoUser: this.userInfo.humanName, //提交人（起草人）
        // ?iseve=1&type=0&draftOrgan=D32960442&orgLevel=1&draftOrganName=
        pcUrl:
          "sealManage/useSeal/draft/introduction?" +
          "iseve=" +
          this.$route.query.iseve +
          "&type=" +
          this.$route.query.type +
          "&draftOrgan=" +
          this.$route.query.draftOrgan +
          "&orgLevel=" +
          this.$route.query.orgLevel +
          "&draftOrganName=" +
      ...
      this.$api.sealManage
        ...
        .then(res => {
          if (res.code == "SUCCESS") {
            ...
            localStorage.setItem("openFlag", JSON.stringify(openFlag));
            sessionStorage.setItem("jieshaoxinId", res.Id);
            this.$message({
              type: "success",
              message: res.msg
            });
    ...
    updateData1() {
      let ldBj0 = this.ldBj0;
      let fzBj0 = this.fzBj0;
      console.log(this.form, "updateData更新数据222");

      let formParams = JSON.parse(JSON.stringify(this.form));
      console.log(formParams, "updateData更新数据");
      formParams.ldBj && (formParams.ldBj = `${ldBj0}=${formParams.ldBj}`);
      formParams.fzBj && (formParams.fzBj = `${fzBj0}=${formParams.fzBj}`);
      // formParams.deptTableHeader = this.form.deptTableHeader+"介绍信用印审批单"
      ...
      if (typeof formParams.yyType === "string") {
        formParams.yyType = JSON.parse(formParams.yyType);
      }
      // tableSealStateCols
      formParams.yyType = this.tableSealStateData;
      console.log(formParams.yyType, "获取盖章状态正常数量fjdkvbn");
      this.$api.sealManage
        .updateIntroduction({
          ...formParams,
          rtfAssetList: this.rtfAssetList,
      ...
      this.$refs["registerForm"].validate((valid, object) => {
        if (valid) {
          this.$api.sealManage
            .getSealNum({
              markOrderType: "2",
              unitId: JSON.parse(localStorage.getItem("userInfo")).unitId, // 机构id
              statusType: true,
              id: this.$route.query.id,
              fileNo: val
            })
    ...
    sureReback() {
      ...
      let param = {
        markOrderType: this.$route.query.markOrderType, // 用印类型
        thIdea: this.form.thIdea,
        id: this.sendId || this.$route.query.id,
        thType: "1" //退回类型  0 退回， 1退回修改
      };
      ...
      switch (item) {
        ...
        case "生成生效时间":
          this.$api.sealManage.getUseSealType({}).then(res => {
            console.log(res, "生成有效时间");
            // this.form.useTime = res.newDateYY;
            this.$set(this.form, "useTime", res.newDateYY);
            this.getLink();
          });
          break;
        case "生成用印时间":
              ...
              this.$confirm(
                ...
                .catch(() => {
                  ...
                  this.$nextTick(() => {
                    ...
                    // this.seletOptionsData = res.list;
                    console.log(this.seletOptionsData, "过滤");
                  });
                });
            } else {
              this.sendId = res.id;
    ...
    sendBankDocument() {
      ...
      formParams.fzBj && (formParams.fzBj = `${fzBj0}=${formParams.fzBj}`);
      this.$api.sealManage
        .sendDocument(Object.assign(formParams, sendHeigherparams))
        .then(res => {
     ...
     */
    toConclude() {
      this.sendId || this.$route.query.id ? this.updateData() : this.addData();
          ...
          this.$confirm("是否办结文件？", "提示", {
            ...
            .then(() => {
              this.$api.sealManage
                .sureHander({
                  // ...formParams,
                  documentNo: formParams.documentNo,
                  useTime: formParams.useTime,
                  id: this.$route.query.id,
                  markOrderType: "2",
                  over: "true"
                })
    ...
    sendSystemUnit() {
      ...
      let param = {
        ...
        id: this.sendId || this.$route.query.id,
        // unitId: this.userInfo.unitId // 机构id   ####
        unitId: this.form.sendDept // 联系单位id
      };
      this.$api.sealManage.sureHander({ ...param }).then(res => {
        // 走发送上级行接口
        if (res.code === "SUCCESS") {
