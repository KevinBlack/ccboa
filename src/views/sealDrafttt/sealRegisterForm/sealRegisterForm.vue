...
export default {
  ...
  data: function data() {
    return {
      ...
      rules: {
        ...
        userJg: [
          { required: true, message: "请输入使用机构", trigger: "blur" }
        ],
        sealStatus: [
  ...
  mounted: function() {
    ...
    if (this.$route.query.id) {
      ...
      /*
                if(sealStatus=='A03' || sealStatus=='A04' || sealStatus=='A05'){
                    this.statusDisabled = true;
                }
                */
      //印章登记其它信息
      ...
      //     .querySealRegisterDetail(jsonData2)
      //     .then(res => {
      //       console.log("M516返回的印章基本信息：", res);
      //       let formData = res;
      //       this.form.yzJiancheng = formData.yzJiancheng; //印章简称
      //       this.form.Stmp_ECD = formData.Stmp_ECD; //印章编码
      //       this.form.sealName = formData.sealName; // 印章名称
      //       this.form.yzType = formData.yzType; // 印章类别
      //       this.form.sdeptShowId = formData.sdeptShowId; // 保管使用部门
      //       this.form.sdeptShow = formData.sdeptShow; // 保管使用部门名称
      //       this.form.useJgId = formData.useJgId; // 使用机构
      //       this.form.userJg = formData.userJg; // 使用机构名称
      //       this.form.sealStatus = formData.sealStatus; // 印章状态
      //       this.form.markNo = formData.markNo; // 序号
      //       this.form.sealPersonC1id = formData.sealPersonC1id; //实际保管人1
      //       this.form.sealPersonC1 = formData.sealPersonC1; //实际保管人1名称
      //       this.form.sealPersonC2id = formData.sealPersonC2id; //实际保管人2
      //       this.form.sealPersonC2 = formData.sealPersonC2; //实际保管人2名称
      //       this.form.useDate = formData.useDate; // 启用日期
      //       this.form.isLocate = formData.isLocate; // 是否存放到用印机器中
      //       this.form.rpidFlag = formData.rpidFlag; //RFID标签
      //       this.selectStmpStCd(this.form.sealStatus);
      //     })
      //     .catch(err => {
      ...
      this.$api.sealManage.sealRegister
        .querySealRegisterDetail(jsonData4)
        .then(res => {
          if (res.LIST9) this.tableData = res.LIST9;
        })
        .catch(err => {
      ...
      //使用机构为当前登录人机构
      let that = this;
      // that.$sealUtil.getMappingUnitSync(
      //   this,
      //   JSON.parse(localStorage.userInfo).unitId,
      //   function(data, res) {
      //     console.log(
      //       "转换前机构ID:" +
      //         JSON.parse(localStorage.userInfo).unitId +
      ...
      //         data
      //     );
      // that.form.useJgId = data; //使用机构
      // that.form.sdeptShowId = data; //保管使用部门
      //   }
      // );
      // that.form.userJg = JSON.parse(
      //   localStorage.userInfo
      // ).shortUnitName;
      // that.form.sdeptShow = JSON.parse(
      //   localStorage.userInfo
      // ).shortUnitName;
    }
  },
  ...
  methods: {
    ...
    //     tempOrgID = data;
    //   });
    //   this.form.sdeptShowId = tempOrgID;
    //   this.form.sdeptShow = org.unitName;
    // },
    //制发机构
    selectRcvChkOrg(data) {
      ...
      //   tempOrgID = data;
      // });
      // this.form.issJgId = tempOrgID;
      // this.form.issJg = org.unitName;
    },
    getOrgList(org) {
    ...
    /*
            selectBlngOrg(org) {
                ...
                this.form.useJgId = tempOrgID;
                this.form.userJg = org.unitName;
                if(this.form.yzType != '' && this.form.useJgId !='' && this.form.markNo !=''){
                    let stmp_ecd = this.form.useJgId+'000'+this.form.yzType+this.form.markNo+'001';
    ...
    //   this.form.safeDept = org.unitName;
    // },
    //序号
    ...
    // 添加印模图片
    addAttachImg() {
      this.upFileConfig.info =
        "请选择要上传的印模图片！最大只支持小于30M的文件";
    ...
    getRegisterRecord(stmp_ECD) {
      ...
      this.$api.sealManage.sealRegister
        ...
        .then(res => {
          ...
          if (res.LIST13) {
            ...
            if (this.form.isLocate == "")
              this.form.isLocate = formData.isLocate; // 是否存放到用印机器中
            if (this.form.sealCh == "") this.form.sealCh = formData.sealCh; // 刻制单位
            if (this.form.sealGa == "") this.form.sealGa = formData.sealGa; // 批准刻制单位
            if (this.form.getSealP == "")
              this.form.getSealP = formData.getSealP; // 领用人名称
            if (this.form.sendBase == "")
              this.form.sendBase = formData.sendBase; // 启用文号
            if (this.form.deviceNameBak == "")
              this.form.deviceNameBak = formData.deviceNameBak; // 印章设备名称
            if (this.form.deviceSpnoBak == "")
              this.form.deviceSpnoBak = formData.deviceSpnoBak; // 印章设备型号
            if (this.form.deviceNoBak == "")
              this.form.deviceNoBak = formData.deviceNoBak; // 印章设备编号
            if (this.form.storeMark == "")
              this.form.storeMark = formData.storeMark; //印章在用印机中的位置
            if (this.form.Store_Plc_Cd == "")
              this.form.Store_Plc_Cd = formData.Store_Plc_Cd; //印章存放地点
            if (this.form.sealPersonA1id == "") {
              this.form.sealPersonA1id = formData.sealPersonA1id; //印章保管人A1角
              this.form.sealPersonA1 = formData.sealPersonA1; //印章保管人A1角名称
            }
            if (this.form.sealPersonA2id == "") {
              this.form.sealPersonA2id = formData.sealPersonA2id; //印章保管人A2角
              this.form.sealPersonA2 = formData.sealPersonA2; //印章保管人A2角名称
            }
            if (this.form.sealPersonB1id == "") {
              this.form.sealPersonB1id = formData.sealPersonB1id; //印章保管人B1角
              this.form.sealPersonB1 = formData.sealPersonB1; //印章保管人B1角名称
            }
            if (this.form.sealPersonB2id == "") {
              this.form.sealPersonB2id = formData.sealPersonB2id; //印章保管人B2角
              this.form.sealPersonB2 = formData.sealPersonB2; //印章保管人B2角名称
            }
            if (this.form.issJgId == "") {
              this.form.issJgId = formData.issJgId; //制发机构
              this.form.issJg = formData.issJg; //制发机构名称
            }
            //废止情况
            if (this.form.desDate == "") this.form.desDate = formData.desDate; //废止日期
            if (this.form.returnPer) this.form.returnPer = formData.returnPer; //收回人Nm
            if (this.form.desCause == "")
              this.form.desCause = formData.desCause; //废止原因
            if (this.form.jiaoPer == "") this.form.jiaoPer = formData.jiaoPer; //交回人Nm
            if (this.form.jiaoDate == "")
              this.form.jiaoDate = formData.jiaoDate; //交回日期
            if (this.form.safeDept == "")
              this.form.safeDept = formData.safeDept; //保管部门名称
            //销毁情况
            if (this.form.desDate1 == "")
              this.form.desDate1 = formData.desDate1; //销毁日期
            if (this.form.desCause1 == "")
              this.form.desCause1 = formData.desCause1; //销毁原因
            if (this.form.desPer1 == "") this.form.desPer1 = formData.desPer1; //销毁人NM
            if (this.form.desPer2 == "") this.form.desPer2 = formData.desPer2; //监销人Nm
            if (this.form.remark == "") this.form.remark = formData.remark; //备注
          } else {
      ...
      //   this.$api.sealManage.sealRegister
      //     .querySealRegisterDetail(jsonData1)
      //     .then(res => {
      //       console.log("M516返回的印章基本信息：", res);
      //       let formData = res;
    ...
    setMachineInfo(name) {
      for (var i = 0; i < this.sealMachines.length; i++) {
        if (name == this.sealMachines[i].title) {
          ...
          this.form.deviceNoBak = this.sealMachines[i].deviceNumber; // 印章设备编号
