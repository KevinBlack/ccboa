    ...
    <div class="process_content">
      <div class="bank_dispach_content" id="bank_dispach_content">
        ...
        <el-form ref="elForm" :rules="rules" :model="formData" label-width="185px">
          ...
          <el-row>
            <el-col :span="22">
              <el-form-item label="具体事项" prop="leavesType">
                <el-checkbox-group v-model="formData.leavesType" :disabled="readFields.jtsx || showFlag">
                  <el-checkbox
                    @change="changeTypes"
          ...
          <el-row>
            <el-col :span="22">
              <el-form-item label="目的地" prop="destination">
                <el-input
                  placeholder="请写明省份和地级市"
                  :autosize="{minRow:5,maxRow:15}"
                  v-model="formData.destination"
                  :disabled="readFields.destination || showFlag"
                ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="22">
              <el-form-item label="离京期间联系方式" prop="otherTel">
                <el-input
                  placeholder="联系方式..."
                  :autosize="{minRow:5,maxRow:15}"
          ...
          <el-row>
            <el-col :span="22">
              <el-form-item label="事由报告" prop="reason" ref="reason">
                <el-input
                  type="textarea"
                  placeholder="1、写明离京人员姓名、单位、职务、离京时间、地点、事由等信息 2、是否符合目的地防疫要求（请注明符合目的地防疫要求），是否符合返京防疫要求（请注明返京防疫要求）"
                ...
                <el-row>
                  <el-col :span="24">
                    <el-select
                      ...
                      :disabled="readFields.firstDepart || showFlag"
              ...
              <div class="d_flex">
                <el-row class="d_b100">
                  ...
                  <i
                    ...
                    v-show="item.createTime==lastOptionTime&&item.createUserId==localUserId&&($route.query.iswait == true||$route.query.iswait == 'true')"
                  ></i>
                </el-row>
              </div>
            </div>
          ...
          <!-- 意见域 -->
          <!-- 1 行领导批示 -->
          <!-- <el-form-item label="行领导批示" prop="hldps">
            <div v-for="item in formData.hldpsOpinionList" :key="item.id" class="opinion-wrapper">
              <div class="d_flex">
                <el-row class="d_b100">
                  <el-col :span="22" class="content-container">
                    <div>{{item.content}}</div>
            ...
            <el-row
              v-if="!editedIdeaFields.hldps||!(formData.hldpsOpinionList&&formData.hldpsOpinionList.length!=0)">
              <el-col :span="22">
                <el-input
                  type="textarea"
                  :autosize="{minRow:5,maxRow:15}"
                  :placeholder="hldpsPlaceholder"
                  maxlength="500"
                  show-word-limit
                  v-model="formData.hldps"
                  :disabled="editedIdeaFields.hldps || showFlag"
                ></el-input>
              </el-col>
              <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.hldps">
                <i :disabled="showFlag" class="icon-font el-icon-edit select-chang" @click="showWriteList('hldps')"></i>
              </el-col>
            </el-row>
          </el-form-item> -->
          <!-- 2 申请机构（部门）主要负责人意见 -->
          <el-form-item :label="'申请机构（部门）\n主要负责人意见'" prop="fzryj" class="fold_label">
            <div v-for="item in formData.zyfzrOpinionList" :key="item.id" class="opinion-wrapper">
              <div class="d_flex">
                <el-row class="d_b100">
                  <el-col :span="22" class="content-container">
                    <div>{{item.content}}</div>
            ...
            <el-row
              v-if="!editedIdeaFields.fzryj||!(formData.zyfzrOpinionList&&formData.zyfzrOpinionList.length!=0)">
              <el-col :span="22">
                <el-input
                  type="textarea"
                  :autosize="{minRow:5,maxRow:15}"
                  :placeholder="fzryjPlaceholder"
                  maxlength="500"
                  show-word-limit
                  v-model="formData.fzryj"
                  :disabled="editedIdeaFields.fzryj || showFlag"
                ></el-input>
              </el-col>
              <el-col :span="1" style="text-align: right" v-if="!editedIdeaFields.fzryj">
                <i :disabled="showFlag" class="icon-font el-icon-edit select-chang" @click="showWriteList('fzryj')"></i>
              </el-col>
            </el-row>
          </el-form-item>
                          ...
                          <div class="command_file">
                            ...
                            <div class="btns">
                              <el-button
                                ...
                                v-if="(formData.attchmentFileInfo.length>1)&&(!formData.approveNo)&&(formData.currentNode=='拟稿')"
                              ...
                              <el-button
                                ...
                                v-if="!formData.approveNo&&(formData.currentNode=='拟稿')"
...
export default {
  ...
  data() {
    return {
      ...
      FEIbuttonList: [
        ...
        { name: "填写申请", type: "tianxiesq", show: false }, //9 8
        { name: "处理申请", type: "chulisq", show: false },  //10 9
        { name: "查看申请", type: "chakansq", show: false }, //11 10
        { name: "修改重报", type: "btn_xgcb", show: false }, //12 11
      ...
      editedIdeaFields: {
        ...
        mark: true, //备注
        // hldps:true, //行领导批示
        fzryj:true,//申请机构（部门）主要负责人意见
        qjbaqjlxfs: true, //离京期间联系方式
  ...
  computed: {
    /*“标题”字段自动生成规则
      关于‘单位[表单中单位字段]’‘姓名[表单中姓名字段]’疫情期间离京的申请
    */
    curTitle: {
      get() {
        const a = `关于${this.formData.responsibleUnit}${this.curResponsiblePerson}疫情期间离京的申请`;
  ...
  methods: {
    ...
    tidIdea() {
      ...
      let editedIdeaFields_tipsObj = [
        ...
        //   name: "hldps",
        //   tips: "填写不可多于500字",
        //   placeholderName: "hldpsPlaceholder"
        // },
        {
          name: "fzryj",
          tips: "填写不可多于500字",
          placeholderName: "fzryjPlaceholder"
        }
      ];
    ...
    deleteCurMark(dataId) {
      this.deleteList("deleteRemark", dataId);
    },
    /**
    ...
    deleteList(funcName, dataId) {
      this.$api.checkLj[funcName]({ id: dataId }).then(res => {
        if (res.code == "SUCCESS") {
          this.$message({
            message: "操作成功",
            type: "success"
    ...
    initRule() {
      this.rules = {
        ...
        // hldps: [
        //   {
        //     required: this.requiredIdeaFields.hldps,
        //     message: "不能为空",
        //     trigger: "change"
        //   }
        // ],
        fzryj: [
          {
            required: this.requiredIdeaFields.fzryj,
            message: "不能为空",
            trigger: "change"
          }
        ],
        ...
        destination:[
          {
            required: this.requiredFields.destination,
            message: "不能为空",
            trigger: "change"
          }
        ],
      ...
      switch (params) {
        ...
        case "生成编号":
          this.$refs["elForm"].validate(valid => {
            if (valid) {
              if (this.formData.exaunitIdea || this.formData.applyOpinionList?.length > 0) {
                if(this.formData.isKh==0 && this.formData.currentNode=="申请部门综合处理"){
                  this.formData.approveType='apply'
                }else{
                  this.formData.approveType='approve'
                }
                this.$api.checkLj
                  .detailForNumVv({
    ...
    viewApply() {
      ...
      this.viewFile(
        ...
        (this.formData.approveNo
          ? this.formData.approveNo
          : this.formData.virtualApproveNo) + this.formData.title
      );
    },
    //下载文件
    ...
    writeApply() {
      ...
      if (this.formData.xksqFileName) {
        ...
        if (this.id) {
          this.formData.id = this.id;
          this.$api.checkLj.checkLzAlterV(this.formData).then(res => {
        ...
        if (valid) {
          if (!this.editedIdeaFields.spyj) {
            ...
            if (this.id) {
              ...
              this.formData.handleButton = "保存";
              this.$api.checkLj.checkLzCreateV(this.formData).then(res => {
                this.id = res.id;
    ...
    delete_btn() {
      this.$confirm("是否确定删除当前表单数据并退出?", "提示", {
        ...
        .then(() => {
          if (this.id) {
            this.$api.checkLj.checkLzDeleteV({ id: this.id }).then(res => {
              if (res.code == "SUCCESS") {
                this.$message({
                  message: "删除成功",
                  type: "success"
    ...
    loadqueryDeal() {
      ...
      this.$api.checkLj
        ...
        .then(res => {
          ...
          if(res.checkOpinionFlag && res.data.currentNode == "拟稿" ) {
            this.showFlag = true
          } else {
            this.showFlag = false
          }
          ...
          this.formData.hldpsOpinionList = res.hldpsOpinionList //行领导批示
          this.formData.zyfzrOpinionList = res.zyfzrOpinionList //申请机构（部门）主要负责人意见
          ...
          //筛选所有可删除意见最后一条意见添加时间
          this.lastOptionTime = []
          this.getLastOptionTime("lastOptionTime", res.hldpsOpinionList);
          this.getLastOptionTime("lastOptionTime", res.zyfzrOpinionList);
          ...
          this.getHuanJie();
          this.isImportant =
            this.formData.currentNode == "拟稿" ? false : true;
    ...
    queryDeal() {
      this.$api.checkLj
        ...
        .then(res => {
          ...
          this.getLastOptionTime("lastOptionTime", res.zyfzrOpinionList);
          this.getLastOptionTime("lastMarkTime", res.markList);
          console.log("ddddddddddssssssss", res.data,this.lastOptionTime)
          this.$forceUpdate();
          if (res.data.leaveBase && res.data.leaveBase != "{}") {
            this.Xjcheck = [];
            this.Xjcheck.push(JSON.parse(res.data.leaveBase));
    ...
    //退回原因
    backReason() {
      this.$api.checkLj.reasonBackC({ id: this.id }).then(res => {
        if (res.code == "SUCCESS") {
    ...
    backNum() {
      ...
      this.$api.checkLj
        .backNumV({ id: this.formData.id,approveType:this.formData.approveType })
        .then(res => {
    ...
    generateNum() {
      ...
      this.$api.checkLj.createNumV(this.formData).then(res => {
        if (res.code == "SUCCESS") {
          if (res.code1) {
           return  (this.$message({
              type: "warning",
    ...
    docompositeNo(yearNu, num) {
      ...
      //非跨行生成审批编号
      if (this.formData.isKh == 0) {
        if(this.formData.currentNode=="申请部门综合处理"){
          this.listCodeSq = `${this.resData.unitHeader}${this.resData.projectShortName}〔${yearNu}〕第${num}号`;
          this.formData.applyNo = this.listCodeSq;
        }else{
          this.listCodeSp = `${this.resData.projectShortName}〔${this.resData.year}〕第${num}号`;
          this.formData.approveNo = this.listCodeSp;
        }
        //跨行生成审批编号
      } else if (
    ...
    setConfig() {
      ...
      if(this.formdata.checkOpinionFlag && this.formdata.currentNode == "拟稿" ) {
      //   this.FEIbuttonList[10].show = false
  ...
  created() {
    ...
    setTimeout(() => {
      ...
      if (this.$route.query.dataForm && !this.id) {
        ...
        console.log("电话",rowData.telMobile)
        //赋值，并添加请假/备案选中值字段
        this.formData = {
          ...rowData,
          leavesType: [],
        ...
        // this.$set(this.formData, "otherTel", JSON.parse(localStorage.getItem("userInfo")).telMobile);
