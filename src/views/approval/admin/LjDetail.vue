        ...
        <el-form :rules="rules" :model="formData" label-width="130px">
          ...
          <el-row>
            <el-col :span="22">
              <el-form-item label="具体事项" prop="leavesType">
                <el-checkbox-group v-model="formData.leavesType" :disabled="isWeiHu">
                  <el-checkbox
          ...
          <!-- <el-form-item label="行领导批示">
            <el-button
              type="primary"
              size="mini"
              @click="addSucc('hldpsOpinionList')"
              :disabled="isWeiHu"
            >新增意见</el-button>
            <div class="suggStyle">
              <div
                v-bind:class="['d_flex',{'border_line':index!=(formData.hldpsOpinionList.length-1)}]"
                v-for="(item,index) in formData.hldpsOpinionList"
                :key="index"
              >
                <el-row class="d_b100">
                  <el-col :span="24">
                    ...
                    <el-col :span="2" style="margin-left:12px">
                      <el-button
                        type="danger"
                        @click="deleSugg('hldpsOpinionList',index,item.id)"
                        :disabled="isWeiHu"
                      >删除</el-button>
                    </el-col>
                  </el-col>
          ...
          <!--         2 申请机构（部门）主要负责人意见-->
          <el-form-item :label="'申请机构（部门）\n主要负责人意见'" class="fold_label">
            <el-button
              type="primary"
              size="mini"
              @click="addSucc('zyfzrOpinionList')"
              :disabled="isWeiHu"
            >新增意见</el-button>
            <div class="suggStyle">
              <div
                v-bind:class="['d_flex',{'border_line':index!=(formData.zyfzrOpinionList.length-1)}]"
                v-for="(item,index) in formData.zyfzrOpinionList"
                :key="index"
              >
                <el-row class="d_b100">
                  <el-col :span="24">
                    ...
                    <el-col :span="2" style="margin-left:12px">
                      <el-button
                        type="danger"
                        @click="deleSugg('zyfzrOpinionList',index,item.id)"
                        :disabled="isWeiHu"
                      >删除</el-button>
                    </el-col>
                  </el-col>
  ...
  methods: {
    ...
    deleMark(n, dataId) {
      this.$confirm("是否确定删除当前数据?", "提示", {
        ...
        .then(() => {
          if (dataId) {
            this.$api.checkLj.deleteRemark({ id: dataId }).then(res => {
              if (res.code == "SUCCESS") {
                this.formData.mark = "";
                this.$message({
                  message: "操作成功",
    ...
    deleSugg(sugg, n, dataId) {
      this.$confirm("是否确定删除当前数据?", "提示", {
        ...
        .then(() => {
          if (dataId) {
            this.$api.checkLj.delAskMsg({ id: dataId }).then(res => {
              if (res.code == "SUCCESS") {
                this.formData.mark = "";
                this.$message({
                  message: "操作成功",
            ...
            switch (sugg) {
              ...
              case (sugg = "checkOpinionList"):
                ...
                break;
              case (sugg = "hldpsOpinionList"):
                this.formData.hldpsOpinionList.splice(n, 1);
                break;
              case (sugg = "zyfzrOpinionList"):
                this.formData.zyfzrOpinionList.splice(n, 1);
                break;
            }
          }
        })
    ...
    addSucc(range) {
      ...
      switch (range) {
        case (range = "hldpsOpinionList"):
          this.formData.hldpsOpinionList.push({
            content: "",
          ...
          break;
          case (range = "zyfzrOpinionList"):
          this.formData.zyfzrOpinionList.push({
            content: "",
            departmentId: this.formData.depId,
            departmentName: AdepartmentName,
            createUser: AcreateUser,
    ...
    saveData() {
      ...
      if(!this.formData.hldpsOpinionList) {
        this.formData.hldpsOpinionList = []
      }
      if(!this.formData.zyfzrOpinionList) {
        this.formData.zyfzrOpinionList = []
      }
      ...
      this.$api.checkLj
        ...
        .then(res => {
          ...
          this.$nextTick(() => {
            ...
            this.$set(this.formData, "hldpsOpinionList", res.hldpsOpinionList);
            ...
            this.$set(this.formData, "zyfzrOpinionList", res.zyfzrOpinionList);
