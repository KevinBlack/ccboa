...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //字典表查询 ---:TODO
            getOptions(){
                let findModule = {function: "findModule"};
                this.$post.postData(
                    "findModule",
            ...
            deleteRow(n, id) {
                let that = this;
                this.bank_deleteFileById.id = id ? id : this.fromData.handbookVo.id;
        ...
        created() {
            if (JSON.parse(localStorage.getItem("tcHumanRole"))) {
                ...
                let humanArr = humanRole.filter(
                    el => el.codeName == "系统管理员" || el.codeName == "超级管理员" || el.codeName == "业务管理员"
                );
