...
export default {
  ...
  data() {
    return {
            /**自定义导出弹窗相关参数 */
      dialogVisible:false,
      stateList:['当前处理人','当前环节'],
      stateListData:[{label:'当前处理人',prop:'currentUser'},
                      {label:'当前环节',prop:'currentNode'}],
      elementList: ['申请日期','申请项目','申请编号','申请单位','审批编号','联系人','标题'],
      elementListData:[{label:'申请日期',prop:'applyTime'},
                    {label:'申请项目',prop:'applyProjectName'},
                    {label:'是否跨行',prop:'isKh'},
                    {label:'申请编号',prop:'applyNo'},
                    {label:'申请单位',prop:'draftDepartment'},
                    {label:'主管部门',prop:'mainDepart'},
                    {label:'审批编号',prop:'approveNo'},
                    {label:'联系人',prop:'contactName'},
                    {label:'事由提要',prop:'reason'},
                    {label:'标题',prop:'title'},                  
                    {label:'联系电话',prop:'contactTel'},                
                    {label:'附件标题',prop:''}],
      objList:[],
      objListData:[{label:'传阅人员未阅',prop:'readPerson'},
                    {label:'传阅人员已阅',prop:'unreadPerson'}],
      /***自定义导出弹窗相关参数end */
      ...
      formCfg:[
        ...
        {type: "input",prop: "approveNo",label: "审批编号",span: 8,value: "〔" + new Date().getFullYear() + "〕"},
        { type: "input",prop: "applyNo",label: "申请编号",span: 8,value: "〔" + new Date().getFullYear() + "〕"},
        {
          type:'date-daterange',
          prop: "$date",
          label: "查询日期",
