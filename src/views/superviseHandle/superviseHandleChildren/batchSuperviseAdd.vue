<template>
  <div id="batchSuperviseAdd" class="bank_dispach greenLight">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div>
          ...
          <el-button plain  @click="createNum">生成编号</el-button>
          <el-button plain @click="deleteThisClick">删除此文</el-button>
          
          <el-button plain  @click="doSendTmp">发送办理单位</el-button>
    ...
    <el-dialog 
    v-dialogDrag
    title="生成文号" width="30%" :visible.sync="SetDocNoVisible">
      ...
      <div class="bank_dispach_file">
        <div class="bank_dispach_file_part">
          ...
          >
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="立项依据" prop="projectBase" ref="projectBase">
                    <el-select :disabled="readFields.projectBase" v-model="fromdata.projectBase" placeholder="请选择" style="width:100%">
                      <el-option
                ...
                <el-col :span="24">
                  <el-form-item label="标题" prop="summary" ref="summary">
                    <el-input v-model="fromdata.summary" :disabled="readFields.summary" maxlength="500"></el-input>
                  </el-form-item>
                </el-col>
                <el-col>
                  <el-form-item label="批量督办事项" prop="type" ref="type">
                    <template>
                      <el-button @click="add" :disabled="readFields.add||fromdata.batchStatus != '0'">增加</el-button>
                      <el-button @click="dialogFormImport = true" :disabled="fromdata.doucumentNum.length>7||fromdata.batchStatus != '0'">导入</el-button>
                      <el-button @click="plsh" v-if="fromdata.batchStatus == 1&&state<2">批量收回</el-button>
                      <el-tooltip class="item" effect="dark" content="批量督办事项导入注意事项：
                      1，请勿修改模板格式。
                      2，根据模板中提供的信息格式添加数据。
                      3，确保承办部门，协办部门按照组织机构的部门名称进行正确填写。
                      4，督办事项添加多个协办单位时用英文分号;分隔。
                      " placement="top-start">
                        <span class="pad_lr10">说明：导入、导出格式为Excel表格，<a class="colorRed" @click='downPiLiang'>点击此处下载模板<span><i class="el-icon-warning"></i></span></a>;点击事由提要查看督办详细信息。</span>
                      </el-tooltip>
                      ...
                      <m-table                                                                 
                        ...
                        :isIndex="true"
                        :isPagination="true"
                        :isSelection="fromdata.batchStatus == 1"
                        @select="select"
                        :isDB="true"
                        :isHandle="false"
                  ...
                  <el-form-item label="批量督办办结情况" prop="setUpInfo" ref="setUpInfo">
                    <div class="fix_height">
                      <el-input
                        ...
                        :disabled="editedIdeaFields.finishDetail"
                        v-if="!editedIdeaFields.finishDetail  
                        || (editedIdeaFields.finishDetail && !(fromdata.opinionSuperviseSetupList && fromdata.opinionSuperviseSetupList.length))"
                      ></el-input>
                      <button
                        @click="writelistbox(clickTypes=6,
                        fromdata.finishDetail&&fromdata.finishDetail.length
                        )"
                        type="button"
                        class="el-button el-button--default dimi_btn"
                        v-show="!editedIdeaFields.finishDetail"
                      >
                        <i class="icon-font el-icon-edit select-chang"></i>
                      </button>
                    </div>
                  ...
                  <el-form-item label="备注">
                    <template>
                      <el-input type="textarea"
                        resize="none"
                        maxlength="500"
                        autosize v-model="fromdata.remarks" :disabled="editedIdeaFields.remarks"
                        v-if="!editedIdeaFields.remarks||(editedIdeaFields.remarks && !(fromdata.superviseRemarkList && fromdata.superviseRemarkList.length))"
                      ></el-input>
                      <opinionText 
            ...
            <!-- 附件v-if="fromdata.isUndertakeOver=='1'||fromdata.isUndertakeOver=='0'" -->
            <h6 class="bank_dispach_file_tit">附件</h6>
            <div id="scfj" class="height_90"></div>
            <el-row class="pr45">
              <!-- <uploadFiles ref="uploadComponent" @preserve="preserve" :uploadConfig="uploadConfig" v-if="state!=2&&state!=3&&fromdata.doucumentNum.length<8"></uploadFiles> -->
              <uploadFiles ref="uploadComponent" @preserve="preserve" :uploadConfig="uploadConfig" v-if="state!=2&&state!=3&&(fromdata.batchStatus =='0'||fromdata.batchStatus =='2'||fromdata.batchStatus =='')"></uploadFiles>
              <el-col>
                  <div
                    class="file_list_box"
                        ...
                        <template>
                          <div class="command_file">
                            ...
                            <el-button
                              ...
                              v-if="fromdata.doucumentNum.length<8"
                              round
                            >删除</el-button>
                            <el-button
                                type="primary"
                        ...
                        <div class="command_file">
                          <span class="order">{{index+1}}.</span>
                            ...
                            <span
                              ...
                              @click="viewDraftFile(item1.fileName,item1.filePath ,$businessCode.zxdbfj)"
                            >{{item1.fileName}}</span>
                            <span class="name" :title="item1.createrName">{{item1.createrName}}</span>
    ...
    <el-dialog width="30%" title="批量督办事项单"
    v-dialogDrag
     :visible.sync="dialogFormVisible">
      <el-form :model="ruleForm" ref="ruleForm" :rules="rulesDialog" class="demo_ruleForm" :disabled="cellcol">
        <el-row>
          ...
          <el-form-item label="事由提要" style="padding-left: 10px;" prop="summary" ref="summary">
            <el-col :span="12" class="allwidth" style="width:calc(80% + 10px)">
              <el-input 
              type="textarea"
              resize="none"
              autosize
              maxlength="500" 
              placeholder="填写事由提要可输入500字" 
              v-model="ruleForm.summary"></el-input>
            </el-col>
          </el-form-item>

          <el-form-item label="详情描述" style="padding-left: 10px;" prop="detailDescription" ref="detailDescription">
            <el-col :span="12" class="allwidth" style="width:calc(80% + 10px)">
              <div class="fix_height">
                      <el-input
                        type="textarea"
          ...
          <el-form-item label="是否跨行" prop="isOrg" ref="isOrg">
            <el-radio-group v-model="ruleForm.isOrg" @change="changekh"> 
              <el-radio :label="0">是</el-radio>
              <el-radio :label="1">否</el-radio>
            </el-radio-group>
          </el-form-item>
            <el-form-item label="承办单位" style="padding-left: 10px;" prop="undertakeOrganName" ref="undertakeOrganName">
              <el-col :span="12" class="allwidth" style="width:calc(80% + 10px)">
              <div style="display:flex">
                  <el-input
                    ...
                    autosize
                    readonly
                    :title="ruleForm.undertakeOrganName"
            ...
            <el-form-item label="协办单位" style="padding-left: 10px;" prop="assistOrganName" ref="assistOrganName">
              <el-col :span="12" class="allwidth" style="width:calc(80% + 10px)">
              <div style="display:flex;">
                  <el-input
                    type="textarea"
                    ...
                    autosize
                    readonly
                    :title="ruleForm.assistOrganName"
    ...
    <el-dialog width="30%" title="批量导入"
    v-dialogDrag
     :visible.sync="dialogFormImport">
      <uploadFiles1 @initsearch="initTabledata" ref="uploadNewComponent" @preserve="preserve" :uploadConfig="uploadNewConfig"></uploadFiles1>
    </el-dialog>
  </div>
</template>
...
import uploadFiles1 from "@/components/uploadFile/uploadSingelFile";
...
export default {
  ...
  methods: {
    ...
    disabledState(){
     ...
     //设置控制域
      ...
      // this.isCXGZ = this.readFields.isCXGZ;
      // let isCXGZ =  this.readFields.isCXGZ;
      this.isCXGZ = this.editedIdeaFields.isCXGZ;
      let isCXGZ =  this.editedIdeaFields.isCXGZ;
      if(this.fromdata.doucumentNum.length>7){
        for(let i in this.readFields){
          ...
          if(this.state!=2&&this.state!=3){
            if(this.fromdata.curNodeCode == '005'||this.fromdata.curNodeCode == '006'){
              this.tableCols= [
              { label: "事由提要", prop: "summary", align: "center" },
              { label: "详情描述", prop: "detailDescription", align: "center" },
              { label: "承办单位", prop: "undertakeOrganName", align: "center" },
              ...
              if(this.fromdata.batchStatus == '0'){
                  this.tableCols= [
                  { label: "事由提要", prop: "summary", align: "center" },
                  { label: "详情描述", prop: "detailDescription", align: "center" },
                  { label: "承办单位", prop: "undertakeOrganName", align: "center" },
      ...
      Object.assign(this.editedIdeaFields,{isCXGZ:isCXGZ})
    },
    setFormConfig() {
     this.$nextTick(()=>{
       this.rules = {
    ...
    select(rows,row){
     ...
     for(let i=0;i<rows.length;i++){
      ...
      console.log(this.plshList)
    },
    plsh(){
      if(this.plshList.length == 0){
        this.$message({
          type: "info",
          showClose: true,
          offset: 400,
        ...
        .then(() => {
          ...
          this.$post
            .postData(
              ...
              JSON.stringify({
                function:"doWithdrawReturnBatch",
    ...
    cellClick(row, column, cell, event) {
      if (column.label == "事由提要") {
        ...
        this.editflag = false;
        if (row.handleTime) {
          this.handleTime = this.fromdata.handleTime = row.handleTime;
        }else{
          this.handleTime = "";
        }
      }
      if (column.label == "查看流程") {
        if (row.isUndertakeOver == 1||row.isUndertakeOver == 2||row.isUndertakeOver == 4 ) {
            const routeData = this.$router.resolve({
              ...
              query: {
                ...
                isPL:row.isUndertakeOver,
                codeNode:!this.isCXGZ,
                batchStatus:this.fromdata.batchStatus,
                undertakeOrganId:row.undertakeOrganId
              }
            });
            window.open(routeData.href, "_blank");
          }else if(row.isUndertakeOver == 3){
            return
          } else {
            this.$message({
              message: "未反馈！",
    ...
    console.log(row)
      ...
      if (row.undertakeOrganId) {
        this.undertakeOrgData = this.concatTreeData(
          row.undertakeOrganId,
          row.undertakeOrganName
        );
      }else{
    ...
    changekh(){
      this.ruleForm.undertakeOrganName = '';
      this.ruleForm.undertakeOrganId = '';
      this.ruleForm.assistOrganId = '';
      this.ruleForm.assistOrganName = '';
      this.checkData=[];
      this.undertakeOrgData=[];
    },
    add() {
      this.ruleForm = {
          isOrg:1,
          expireDate:this.fromdata.expireDate||this.expireDate,
          summary:"",
          undertakeOrganName:"",
          assistOrganName:"",
      };
    ...
    handleAddData() {
      this.preserve().then(()=>{
        ...
        var dem = {
          function: "saveTmp",
          pid: id,
          handleTime:this.fromdata.handleTime,
          parentId: tempId
        };
    ...
    info() {
      this.$post
        .postData("doImportTmp", JSON.stringify({
          function:'doImportTmp',
          id:this.fromdata.id,
          tempId:this.fromdata.tempId
        }), this.$businessCode.zxdb)
        .then(res => {
          this.fromdata.tempId = res.data;
        })
        .catch(err => {
        });
    },
      ...
      switch (n) {
        case "chen":
          ...
          if(this.ruleForm.isOrg =='0'){
            Object.assign(this.bank_main,{openSub:true})
            Object.assign(this.bank_main,{openCurrFistDept:false})
          }else{
            Object.assign(this.bank_main,{openSub:false})
            Object.assign(this.bank_main,{openCurrFistDept:true})
          }
          break;
        case "xie":
          name = "协办部门";
          ...
          fun = 'unitTreeByFirst'; 
          this.bank_main.function="unitTreeByFirst"
    ...
    showCompDialog(data, status, type, params, dtype) { 
      ...
      if (status) {
        ...
        if (data.length) {
          ...
          nowIds = this.ruleForm.assistOrganId;
          nowNames = this.ruleForm.assistOrganName;
          nowNamesChen=this.ruleForm.undertakeOrganName;
          nowIdsChen=this.ruleForm.undertakeOrganId;
          names = names.join(";");
          ids = ids.join(";");
        } else {
          names = "";
      ...
      if(dtype == 'chen' || dtype == 'xie'){
        ...
        .then(res => {
          if(res&&res.success == false){
            if(dtype == 'xie'){
             this.ruleForm.assistOrganName = nowNames
             this.ruleForm.assistOrganId = nowIds
             this.preserve();
            }
            if(dtype == 'chen'){
             this.ruleForm.undertakeOrganName = nowNamesChen
             this.ruleForm.undertakeOrganId = nowIdsChen
             this.preserve();
            }
            this.$message({
              type: "error",
    ...
    completeSendingIsTrue(){
      if(this.tableData.length==0||!this.fromdata.tempId){
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
          message: "不存在批量督办事项，请添加。",
          duration: 1000
        });
        return
      }else if(this.fromdata.batchStatus == 1){
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
    ...
    initTabledata() {
      ...
      let id = this.uploadNewConfig.id = this.fromdata.id;
      let tempId = this.uploadNewConfig.tempId =this.fromdata.tempId;
      let obj = {
        id: id,
        tempId: tempId,
        function: "selTmpList",
        processInstId:this.fromdata.processInstId,
        page: this.pagination.page || 1, //当前页数
        limit: this.pagination.limit||10, //每页条数
      ...
      this.$post
        ...
        .then(res => {
          ...
          if (res.data) {
            ...
            if(res.data.rows[0].isUndertakeOver != '0')
            this.tableCols=[
              { label: "事由提要", prop: "summary", align: "center" },
              { label: "详情描述", prop: "detailDescription", align: "center" },
              { label: "承办单位", prop: "undertakeOrganName", align: "center" },
    ...
    //初始值 //input框内容
    toEdit() {
      this.uploadConfig.id = this.uploadNewConfig.id = this.bank_dispach.id = this.bank_dispach.id
        ...
        .then(res => {
          ...
          if(this.state!=2&&this.state!=3){
            if(res.data.processInstId){
              ...
              JSON.stringify({function:"getpcsTplStartNode",multiTenancyId:"CN017"}), 
              this.$businessCode.zxdbgg
              )
              .then(res => {
            ...
            this.$post
              ...
              .then(res => {
                ...
                if(!this.fromdata.expireDate){
                  this.fromdata.expireDate =this.expireDate= res.data.term;
          ...
          if(this.readFlag != 1&&this.state<2){
            //code change
            this.$post
              .postData(
                "doUpdateReadRecord",
      ...
      await this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            if(type==="uploadfile"){
              this.uploadConfig.id = this.uploadNewConfig.id = res.data.id;
              if(!this.dialogFormImport){
                this.$refs.uploadComponent.uploadFileNow();  
              }else{
                this.uploadNewConfig.tempId = res.data.tempId; 
                this.$refs.uploadNewComponent.uploadFileNow();
              }
            }
            this.toEdit();
            successState = true;
      ...
      this.$confirm("是否确定发送并退出当前页面?", "提示", {
        ...
        .then(() => {
          var obj = {
            id: this.fromdata.id,
            function: "doSendTmp",
            handleTime:this.fromdata.handleTime||'工作日'
          };
      this.$post
    ...
    writelistvalue(data) {
      if (data) {
        if (this.clickTypes == 1) {
          ...
          // 签发意见
          this.fromdata.finishDetail = this.fromdata.finishDetail
            ? this.fromdata.finishDetail + data
            : data;
            if(this.fromdata.finishDetail && this.fromdata.finishDetail.length>=500){
              this.fromdata.finishDetail = this.fromdata.finishDetail.substr(0,500)
            }
        }
        this.$forceUpdate();
      }
    ...
    downPiLiang(){
      downLoadFileReName("/ecpweb/page/ccboa/static/file/piliang.xlsx","督办批量导入单.xlsx")
    },
    downFile(file){
      this.bank_doDownloadFile.fileName = file.fileName;
      this.bank_doDownloadFile.filePath = file.filePath;
  ...
  created() {
    let a = localStorage.getItem('st');
    this.uploadConfig.code = this.uploadNewConfig.code = this.$businessCode.zxdbfj;
    //初始化加载数据
    ...
    this.uploadConfig.flowTranId  = this.uploadNewConfig.flowTranId = this.treeTradeCode = this.$businessCode.zxdb;  //完成并发送 树 流程交易线   chai
    this.state = this.$route.query.state ? this.$route.query.state : "";
