<template>
  <div class="dispatchparameterspage">
    <el-row>
      ...
      >
        ...
        <el-col :span="6" style="margin-left: 20px;">
          <el-button type="primary" size="small" @click="handleLoadData(val)">查询</el-button>
        </el-col>
        <el-col :span="9" style="text-align: right;margin-bottom: 10px;">
          <el-button type="primary" size="small" @click="handleAddData">新增本行发送配置</el-button>
    ...
    <el-dialog :visible.sync="dialogVisible" v-dialogDrag :modal-append-to-body="false" :before-close="handleCancel" title="本行发送配置">
      <el-form
        :model="ruleForm"
        :rules="rules"
        ref="ruleForm"
...
export default {
  ...
  data () {
    return {
      ...
      referForm: {
        function: 'getTcSendObjDicByPage',
      },
      unitForm: {
        function: 'getOrgInfo'
      },
      ruleForm: {
        function: 'addTcSendObjDic',
        unitName: '',
        unitId: '',
        organizationName: '',
        organizationId: '',
      ...
      rules: {
        ...
        // manySpace: [
        //   { required: true, message: '请输入中文“，”、“、”及英文“,”、“.”、“;”五种符号中的一种', trigger: 'blur' }
        // ],
        content: [
  ...
  methods: {
    handleLoadData (val) {
      ...
      this.$post.postData('getTcSendObjDicByPage', JSON.stringify(Object.assign({},this.ruleForm, this.referForm, this.pagination)), this.$businessCode.fawgl).then((res) => {
    ...
    cellClick(row, column, cell, event) {
      let that = this;
      this.$post.postData('getTcSendObjDicById', JSON.stringify({ function: 'getTcSendObjDicById', id: row.id, unitId: this.unitId }), this.$businessCode.fawgl).then((res) => {
        if(res&&res.success) {
          this.ruleForm = Object.assign({}, res.data)
          this.ruleForm.function = 'selectSendObjTypeConfigByParam'
          ...
          // this.ruleForm.isSelectAll = this.ruleForm.isSelectAll.toString()
        ...
        this.$post.postData('selectSendObjTypeConfigByParam', JSON.stringify(this.ruleForm), this.$businessCode.fawgl).then((res) => {
          if(res&&res.success) {
            that.paramList = res.data;
            // if(res.data[0].categoryName==that.ruleForm.typeName) {
            //   that.isEveryDeptTRUE = res.data.isEveryDept=='1';
    ...
    handleBatchDelete () {
      if (!this.deleteIds) {
        ...
        this.$confirm("是否确定删除?", "提示", {
          ...
          .then(() => {
            this.$post.postData('deleteTcSendObjDicEntityList', JSON.stringify({ function: 'deleteTcSendObjDicEntityList', id: this.deleteIds, unitId: this.unitId }), this.$businessCode.fawgl).then((res) => {
              this.$message({
                type: 'success',
                message: '删除成功!'
              })
    ...
    handleSave () {
      ...
      this.ruleForm.function = 'addTcSendObjDic'
      this.$post.postData('addTcSendObjDic', JSON.stringify(this.ruleForm), this.$businessCode.fawgl).then((res) => {
        this.$message({
          type: 'success',
          message: '保存成功!'
        })
  ...
  created () {
    ...
    this.isGly = isGlyId();
    this.treeTradeCode = this.$businessCode.fawglfj;  //完成并发送 树   流程交易线   chai
