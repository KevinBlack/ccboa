<template>
  <div>
    ...
    >
      ...
      <el-form-item label="今日完成工作" prop="finishWorkToday">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.finishWorkToday"></el-input>
      </el-form-item>
      <el-form-item label="未完成工作" prop="unfinishedWorkToday">
        <el-input
          type="textarea"
           :disabled="!formData.edit"
      ...
      <el-form-item label="需协调工作" prop="coordination">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.coordination"></el-input>
      </el-form-item>
      ...
      <el-form-item label="附件" prop="createname">
        <el-row  @click.native="clickNow(2)">
          <uploadFiles
            ref="uploadComponent"
            @getDetail="getDetail"
            :isPreserve="isPreserve"
...
export default {
  ...
  data() {
    return {
      ...
      bank_save: {
        ...
        id: "",
        shareId: "",
        unfinishedWorkToday: "", //未完成工作
        finishWorkToday: "", //今日完成工作
        coordination: "" //需协调工作
      },
      share_parmas: {
        function: "share",
    ...
    mixins: [viewDraft],
  created() {
    this.bank_dialyinfo.id = this.$route.query.id ? this.$route.query.id : "";
    console.log(this.bank_dialyinfo.id);
    this.uploadConfig.code = this.$businessCode.rwglfj;

    this.uploadConfig.newcode = this.uploadImgConfig.newcode = this.$businessCode.rwglfj;
    this.uploadImgConfig.code = this.$businessCode.rwglfj;
    this.title = sessionStorage.getItem("logTitle");
    if (this.bank_dialyinfo.id != "") {
      this.uploadConfig.id = this.bank_dialyinfo.id;
      this.uploadImgConfig.id = this.bank_dialyinfo.id;
  ...
  methods: {
    ...
    getInitData() {
      let that = this;
      this.bank_dialyinfo.id = this.$route.query.id;
      this.$post
        .postData(
          "getDetails",
          JSON.stringify(this.bank_dialyinfo),
        ...
        .then(res => {
          if (res && res.success) {
            ...
            that.sharePer = eval(res.data.shareId);
    ...
    preserve(type,fileTypeNow) {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$post
            .postData("save", JSON.stringify(obj), this.$businessCode.rwgl)
            .then(res => {
              if (res && res.success) {
                ...
                if (type === "uploadfile") {
                  ...
                  this.bank_dialyinfo.id = res.data.id;
                  this.isPreserve = true;  
                  if(fileTypeNow == 2){
                    this.$refs.uploadComponent.uploadFileNow();
                  }else{
