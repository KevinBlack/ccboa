<template>
  <div>
    ...
    >
      ...
      <el-form-item label="周" prop="weeks" ref="weeks">
        第
        <el-input
          ...
          :disabled="!formData.edit"
          style="width:80px"
          oninput="if(value>9999)value='';if(value<0)value=0;value=value.replace(/[^\d]/g,'')"
        ></el-input>周
      </el-form-item>
      <el-form-item label="本周回顾" prop="thisWeek">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.thisWeek"></el-input>
      </el-form-item>
      <el-form-item label="下周计划" prop="nextweek">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.nextweek"></el-input>
      </el-form-item>
      <el-form-item label="表扬/建议" prop="praise">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.praise"></el-input>
      </el-form-item>
      <el-form-item label="其他" prop="other">
        <el-input :disabled="!formData.edit" type="textarea" placeholder="请输入内容,最多600个文字" v-model="formData.other"></el-input>
      </el-form-item>
            ...
            <el-form-item :label="item" :key="item">
              <el-scrollbar>
                <div v-for="item1 in fkfjList[n]" :key="item1.id" class="tree_w">
                  <template>
                    <div class="command_wrap">
                      <span class="name down" @click="viewDraftFile(item.fileName,item.filePath ,$businessCode.rwglfj)">{{item1.fileName}}</span>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        weeks: [{ required: true, message: "周不能为空" }],
        thisWeek: [
      ...
      bank_save: {
        function: "taskWeekly_save",
        teamid: "",
        teamname: ""
      },
      //input框内容
      bank_weekinfo: {
        function: "taskWeekly_getDetails",
        id: ""
      },
      share_parmas: {
        function: "share",
  ...
  created() {
    this.bank_weekinfo.id = this.$route.query.id ? this.$route.query.id : "";
    ...
    this.getSelectData();
    if (this.bank_weekinfo.id != "") {
      this.uploadConfig.id = this.bank_weekinfo.id;
      this.uploadImgConfig.id = this.bank_weekinfo.id;
  ...
  methods: {
    ...
    getInitData() {
      this.$post
        .postData(
          "taskWeekly_getDetails",
          JSON.stringify(this.bank_weekinfo),
        ...
        .then(res => {
          if (res && res.success) {
            ...
            this.getSelectData(this.formData.teamid);
          ...
          this.$post
            ...
            .then(res => {
              if (res && res.success) {
                ...
                if (type === "uploadfile") {
                  this.uploadConfig.id = this.bank_weekinfo.id = res.data.id;
