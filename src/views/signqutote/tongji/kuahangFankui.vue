        ...
        <table class="stcs_table">
          <tr>
            <td>
              ...
              <br />（1）跨行签报数：审批行收到跨行签报的时间在统计时间段内的跨行签报数量。
              <br />（2）已办理件数：审批行收到跨行签报的时间在统计时间段内，跨行签报办结的数量。
              <br />（3）已办理率=已办理件数/跨行签报数*100%。
              <br />（4）办理天数：以工作日为标准，每跨过一次1:00或13:00，即加0.5天，不到1次即完成的计0天。
              <br />（5）平均办理天数=所有跨行签报的办理时长之和/跨行签报数。
            </td>
          </tr>
        </table>
      </el-col>
...
const finshData = [
  { label: "审批部门", prop: "bumen", align: "center" },
  { label: "跨行签报数", prop: "zs", align: "center" },
  { label: "已办理件数", prop: "ybjCount", align: "center" },
  { label: "已办理率", prop: "zd", align: "center" },
  { label: "平均办理天数", prop: "pj", align: "center" }
];
export default {
  components: {
    mTable,
  ...
  methods: {
    ...
    resetForm(formName) {
      ...
      this.bank_countersignature.function = "queryTJQianbaoShenpi";
      this.bank_countersignature.draftTimeStart = "";
      this.bank_countersignature.draftTimeEnd = "";
      this.bank_countersignature.page = this.pagination.page = 1;
    ...
    exportData(){
      exportTable.exportExcel("跨行签报反馈情况统计表");
    }
  },
  created() {
