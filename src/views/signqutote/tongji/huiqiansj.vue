        ...
        <table class="stcs_table">
          <tr>
            <td>
              ...
              <br />（1）统计范围：统计收到会签单时间在统计时间段内的办理时间情况。
              <br />（2）会签件数：收到会签单时间在统计时间段内的会签文档数。
              <br />（3）在办件数：收到会签单时间在统计时间段内会签状态为“正在会签”的文档数。
              <br />（4）办结件数：收到会签单时间在统计时间段内会签状态为“会签完毕”的文档数。
              <br />（5）平均会签办理时长=每份会签文件办理时长之和/会签件数。
              <br />（6）最长会签办理时长：各份文件会签办理时长中的最大值。
            </td>
...
const finshData = [
  { label: "会签部门", prop: "bumen", align: "center" },
  { label: "会签件数", prop: "zs", align: "center" },
  { label: "在办件数", prop: "wbjCount", align: "center" },
  { label: "办结件数", prop: "ybjCount", align: "center" },
  { label: "平均会签办理时长", prop: "pj", align: "center" },
  { label: "最长会签办理时长", prop: "zd", align: "center" }
];
export default {
  components: {
    mTable,
  ...
  data() {
    return {
      bank_countersignature: {
        function: "queryTJQianbaoHuiqian",
        page: 1, //当前页数
        limit: 20, //每页条数
        total:0,
        draftTimeStart:"",
  ...
  methods: {
    ...
    resetForm(formName) {
      ...
      this.bank_countersignature.function = "queryTJQianbaoHuiqian";
      this.bank_countersignature.draftTimeStart = "";
      this.bank_countersignature.draftTimeEnd = "";
      this.bank_countersignature.page = this.pagination.page = 1;
    ...
    exportData(){
      exportTable.exportExcel("签报会签办理时间统计表");
    }
  },
  created() {
