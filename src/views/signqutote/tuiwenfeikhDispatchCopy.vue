<template>
  <!-- 中国建设银行电子签报退文单 -->
  <div id="signquteFkhTuiGly" class="bank_dispach">
    <el-row>
...
<!--【关闭】【查看正文】【查看流程】【打印退文单】 -->
        <div class="grid_comtent">
          ...
          <el-button plain @click="collect">退文说明</el-button>
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="是否半电子">
                    <el-radio v-model="fromdata.isArtificial" label="是">是</el-radio>
                    <el-radio v-model="fromdata.isArtificial" label="否">否</el-radio>
                  </el-form-item>
                </el-col>
              </el-row>-->
              <el-row class="pr45">
...
directTitle:"中国建设银行电子签报退文单",
  ...
  methods: {
    ...
    async maintenance(){
      let data={
        ...
        let obj1=await this.$post.postData(
            "getQianbaoBackInfoById",
            JSON.stringify(Object.assign(data,{function :"getQianbaoBackInfoById", unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res&&res.data||[];
          })
          .catch(err => {
        ...
        let obj3=await this.$post.postData(
            ...
            JSON.stringify(Object.assign(data,{function :"selectSysQianbaoFile",tplbsnid: 'BHTW',id: this.$route.query.id, unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
        ...
        // if(obj1&&obj2){
          const routeData = this.$router.resolve({
              ...
              query: {
                ...
                type:'qb',
                sign: 4,
                selectUrl: 'getToQianbaoBackEntityById',
    ...
    // : sessionStorage.getItem("sTwFkhSignpageid") || "";
    //   this.$post
    //     .postData(
    //       "selectSWById",
...
setTimeout(() => {
    ...
    getToQianbaoBackEntityById() {
      if (!sessionStorage.getItem("sTwFkhSignpageid")) {
        this.bank_dispach.id = this.$route.query.id;
      } else {
        this.bank_dispach.id = sessionStorage.getItem("sTwFkhSignpageid") || "";
      }
      // let that = this;
...
//获取签报处理单主标题
          // this.directTitle = res.data && res.data.orgName + "电子签报退文单";
          if(res.data && res.data.isSpecialHead && res.data.isSpecialHead == "1"){
            this.directTitle = this.fromdata.draftDepartmentName + "电子签报退文单";
          }else{
            this.directTitle = res.data && res.data.orgName + "电子签报退文单";
          }
      ...
      await this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            sessionStorage.setItem("sTwFkhSignpageid", res.data.id);
...
// 预览 
    ...
    drafting(state) {
      ...
      if (!getFormId(this.$route.query.id, "sTwFkhSignpageid")) {
    ...
    doDraft(fileInfo,saveName) {
      ...
      let data = this.$router.resolve({
        ...
        query: {
          ...
          id: getFormId(this.$route.query.id, "sTwFkhSignpageid"),
