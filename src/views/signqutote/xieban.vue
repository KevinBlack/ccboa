<template>
  ...
  <el-tabs v-model="activeName" @tab-click="handleTabsClick" v-cloak>
    ...
    <el-row  style="margin-top: -20px;">
      <el-col v-if="getXiebanTableOrderAll.state == 0">
        <el-form
          :model="ruleForm"
          ref="ruleForm"
          label-width="100px"
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="签报字号" prop="qianbaoCode">
                <el-input v-model="getXiebanTableOrderAll.qianbaoCode"></el-input>
              </el-form-item>
            </el-col>
      ...
      <el-col style="margin: 10px 0;" :class="{'is10':(bank_countersignature.state == 1)||bank_countersignature.state>1}">
        <m-table
          size="medium"
          :isSelection="false"
          :isIndex="true"
...
const otherData = [
  ...
  { label: "文号", prop: "DOCUMENTCODE", align: "left"  ,width:"210"},
  { label: "标题", prop: "TITLE", align: "left" },
  { label: "当前环节", prop: "PCSAVYNM", align: "center"  ,width:"210" },
...
const allData = [
  ...
  { label: "标题", prop: "title", align: "left" },
  { label: "当前环节", prop: "curNodeName", align: "center"  ,width:"210" },
  { label: "当前处理人 ", prop: "currentUser", align: "center"   ,width:"150" },
  { label: "办理状态", prop: "orderStatus", align: "center"  ,width:"100" }
];
export default {
  name: "DispatchFile",
  components: {
  ...
  methods: {
    timeChange() { //时间获取
      if(this.ruleForm.searchTime && this.ruleForm.searchTime[0]){
        this.getXiebanTableOrderAll.startTime = dateFormate.date(Date.parse(this.ruleForm.searchTime[0]));
        this.getXiebanTableOrderAll.endTime = dateFormate.date(Date.parse(this.ruleForm.searchTime[1]));
      }else{
        this.getXiebanTableOrderAll.startTime = ""
    ...
    resetForm() {  //重置
       ...
       this.getXiebanTableOrderAll.qianbaoCode = '';
      //  this.getXiebanTableOrderAll.receive_type = '全部';
       this.getXiebanTableOrderAll.startTime = '';
       this.getXiebanTableOrderAll.endTime = '';
      //  this.getXiebanTableOrderAll.receive_type = '全部';
    ...
    initData(str) {
      ...
      this.bank_countersignature.state = this.getXiebanTableOrderAll.state = witchState(this.activeName)
    ...
    cellClick(row, column, cell, event) {
      ...
      if (
        ...
        if (
              ...
              this.handleOpenPage(
                "/xiebanWaitQbDispatch",
                this.idType,
                this.readType,row.orderStatus
              );
            } else if (
              this.bank_countersignature.state == "0" &&
              row.orderStatus == "已办"
