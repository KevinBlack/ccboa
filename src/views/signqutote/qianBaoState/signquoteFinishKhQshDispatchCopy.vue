<template>
  <!-- 签报电子处理单  （跨行请示行） 已办-->
  <div id="signquteWaitQshGly" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <el-button
            size="small"
            plain
            @click="superiorsenddialogbox"
            v-if="fromdata.curNodeName== '综合处理' || fromdata.curNodeName== '总收发'"
          >发上级行</el-button>
          ...
          >流程调度</el-button>
          <el-button size="small" plain @click="handleClearDocNo"  v-show="tuijiejia && historyTuijiejia && finishTuihao">退号</el-button>
          <el-button size="small" plain @click="handledeblock"  v-show="tuijiejia && historyTuijiejia">解锁</el-button>
          <el-button size="small" plain @click="handleLock"  v-show="tuijiejia && historyTuijiejia">加锁</el-button>
          <el-button size="small" plain @click="drafting('0')">查看正文</el-button>
          ...
          <el-button size="small" v-show="fromdata.curNodeCode==800&&fromdata.isSevendDays <= 7 && fromdata.handleTypeName !='1' && choseYear" plain 
          ...
          <el-button size="small" plain @click="collect('qianbao_qingshi',fromdata.id,fromdata.multiTenancyId, unitId)">收藏</el-button>
        </div>
      </el-col>
    </el-row>
              ...
              <el-col>
                ...
                >
                  ...
                  <el-input
                    ...
                    autosize
                    maxlength="500"
                    v-model="fromdata.agreementHandleSituationList"
                    disabled
                  ...
                  <el-col>
                    <div>
                      <el-row>
                        <el-col>
                          <div
                            class="file_list_box tree_zhuban file_lists"
...
export default {
  ...
  methods: {
    ...
    async maintenance() {
      ...
      let obj2 = await this.$post
        .postData(
          ...
          JSON.stringify(
            Object.assign(data, { function: "getOpinionSysInfoById", unitId: this.unitId })
          ),
          this.$businessCode.qbgl
        )
        .then(res => {
      ...
      let obj3=await this.$post.postData(
            ...
            JSON.stringify(Object.assign(data,{function :"selectSysQianbaoFile",tplbsnid: 'KHQBQS',id: this.$route.query.id, unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
      ...
      const routeData = this.$router.resolve({
        ...
        query: {
          ...
          type: "qb",
          sign: 1,
          selectUrl: "getQianbaoKhQingshiPoById",
    ...
    async processScheduling() {
      //查看会签状态
      if (this.isSignFinish == "0" && this.signState) {
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
    ...
    getQianbaoKhQingshiPoById() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          if (this.isSignFinish == "0" && this.fromdata.signState == "0") {
            //判断是否已经发了 signState
