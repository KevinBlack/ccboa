<template>
  <!-- 签报电子处理单  (非跨行) 已办-->
  <div id="signquteFinishFkhGly" class="bank_dispach">
    <el-row>
...
<!-- 【关闭】【维护】【流程调度】【退号】【解锁】【加锁】【查看正文】归档 重新流转【查看流程】【打印处理单】【收藏】 -->
        <div class="grid_comtent">
          ...
          <el-button
            size="small"
            plain
            v-show="bank_selectConfig.curNodeId != '800' && smoaButtonFlag"
            @click="processScheduling"
          >流程调度</el-button>
          <el-button size="small" plain @click="handleClearDocNo" v-show="tuijiejia && historyTuijiejia && finishTuihao && smoaButtonFlag">退号</el-button>
          <el-button size="small" plain @click="handledeblock" v-show="tuijiejia && historyTuijiejia && smoaButtonFlag">解锁</el-button>
          <el-button size="small" plain @click="handleLock" v-show="tuijiejia && historyTuijiejia && smoaButtonFlag">加锁</el-button>
          <el-button size="small" plain @click="drafting('0')" v-show="smoaButtonFlag">查看正文</el-button>
          <el-button size="small" plain @click="pigeonhole('newDoGuidangIsFiled')" v-show="pigeonholeFlag && historyTuijiejia && isNewGd && smoaButtonFlag">区块链归档</el-button>
          <el-button size="small" plain @click="pigeonhole('doGuidangIsFiled')" v-show="pigeonholeFlag && historyTuijiejia && smoaButtonFlag">归档</el-button>
          <el-button size="small" v-show="fromdata.curNodeCode==800&&fromdata.isSevendDays <= 7 && choseYear && smoaButtonFlag"  plain 
          @click="lcjh('doExeActivateTask',fromdata.pcsAvyId,fromdata.multiTenancyId,fromdata.processInstId,qbgl,fromdata.id)">重新流转</el-button>
          <el-button size="small" plain @click="flowScheme" v-show="smoaButtonFlag">查看流程</el-button>
          <el-button size="small" plain @click="processSheet" v-show="smoaButtonFlag">打印处理单</el-button>
          <el-button size="small" plain @click="collect('qianbao_dz',fromdata.id,fromdata.multiTenancyId, unitId)" v-show="smoaButtonFlag">收藏</el-button>
        </div>
      </el-col>
    </el-row>

    <el-dialog v-dialogDrag title="退号" width="30%" :visible.sync="handeldoBackNoFuc">
      <span>您是否要执行退号操作？</span>
    ...
    <el-dialog v-dialogDrag title="加锁" width="30%" :visible.sync="handleLockFuc">
      <span>您是否要执行加锁操作？</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleLockFuc = false">取消</el-button>
        <el-button type="primary" @click="lockHandle">确定</el-button>
      </span>
    </el-dialog>
        ...
        <div class="bank_dispach_file_part">
          ...
          >
            ...
            <div>
              <el-row>
                <el-col>
                  <el-form-item label="标题" prop="title" ref="title" v-if="smoaButtonFlag">
                    <el-input v-model="fromdata.title" placeholder="请输入正文标题" disabled></el-input>
                  </el-form-item>
                  <el-form-item label="正文标题" prop="documentTitle" ref="documentTitle" v-if="!smoaButtonFlag">
                    <el-input v-model="fromdata.documentTitle" placeholder="请输入正文标题" disabled></el-input>
                  </el-form-item>
                </el-col>
                <el-col>
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item label="联系电话" prop="phone" ref="phone">
                    <el-input v-model.number="fromdata.phone" disabled @input.native="phoneChange"></el-input>
                  </el-form-item>
                </el-col>
              ...
              <el-col>
                ...
                >
                  ...
                  <el-input
                    ...
                    :disabled="fromdata.exceedWordReason"
...
export default {
  ...
  mixins: [minixs,sevenDay,viewDraft,signquoteIndex],
  data() {
    return {
      smoaButtonFlag:true,//商密oa过来文件控制按钮显隐 只显示关闭、维护,目前只有行签报
      choseYear:true,
      nowUserId: "", //当前用户id
      interval: null,
      qbgl:this.$businessCode.qbgl,
      sequenceFlag:'',
      defaultUser:{},//完成并发送默认值
      directTitle: "中国建设银行电子签报处理单",
      //正文
      showDraft: false,
      ...
      tuijiejia:true, // 判断是否退号加锁解锁按钮显示
      historyTuijiejia:true,//判断是历史数据不显示退号、解锁、加锁、归档按钮
      finishTuihao:true,//已办结的不显示退号按钮
      //归档按钮显示
  ...
  mounted() {
    ...
    if(this.$route.query.choseYear){
      this.choseYear = false;
    }else{
       this.choseYear = true;
    }
  },
  methods: {
    ...
    async maintenance() {
      ...
      let obj1 = await this.$post
        .postData(
          "getQianbaoById",
          JSON.stringify(Object.assign(data, { function: "getQianbaoById", unitId: this.unitId })),
      ...
      let obj2 = await this.$post
        .postData(
          "selectQianbaoSysOpinion",
          JSON.stringify(
            Object.assign(data, { function: "selectQianbaoSysOpinion", unitId: this.unitId })
          ),
          this.$businessCode.qbgl
        )
        .then(res => {
        ...
        let obj3=await this.$post.postData(
            ...
            JSON.stringify(Object.assign(data,{function :"selectSysQianbaoFile",tplbsnid: 'DZQB',id: this.$route.query.id, unitId: this.unitId })),this.$businessCode.qbgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
      ...
      const routeData = this.$router.resolve({
        ...
        query: {
          ...
          type: "qb",
          sign: 1,
          selectUrl: "getToQianbaoEntityById",
    ...
    blockHandlede(){
      ...
      this.fromdata.isSystem = 1;
      this.preserve();
      this.$forceUpdate();
    },
    //加锁弹窗
    handleLock(){
      this.handleLockFuc = true;
    },
    //加锁
    lockHandle(){
      this.fromdata.isLock = true;
      this.fromdata.isLockButton = false;
      this.handleLockFuc = false;
      this.fromdata.isSystem = 1;
      this.preserve();
      this.$forceUpdate();
    },
    ...
    async judgeTaoSameBumen(self){
      ...
      await self.$post
          ...
          .then(res => {
            if (res && res.success) {
              // if(!self.isHasRedHead && (res.data && res.data.unitCode) !== userBuMen){
                
              if(!self.isHasRedHead && (res.data && res.data.unitCode) !== this.fromdata.draftDepartmentId){
                  self.$message({
                  type: "error",
                  offset: 400,
                  showClose: true,
    ...
    async processScheduling() {
      let that = this;
      if (this.isSignFinish == "0" && this.fromdata.signState) {
        //判断是否已经发了 signState
    ...
    getToQianbaoEntityById() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          //获取签报处理单主标题
          // this.directTitle = res.data && res.data.orgName + "电子签报处理单";
          if(res.data && res.data.isSpecialHead && res.data.isSpecialHead == "1"){
            this.directTitle = this.fromdata.draftDepartmentName + "电子签报处理单";
          }else{
            this.directTitle = res.data && res.data.orgName + "电子签报处理单";
          }
          // 签报依据
          ...
          this.allowBack =
            ...
            res.data.fieldStatusMap.allowBack;
          // 退文参数赋值
          ...
          // this.bank_send.curMultiTenancyId = res.data.multiTenancyId;
          // this.bank_send.pcsAvyId = res.data.res.data.pcsAvyId;
          // this.bank_send.curTplNo = res.data.tplNo;
        ...
        this.doSaveDocumentNumFuc.type = res.data.multiTenancyId;
          ...
          if (this.isSignFinish == "0" && this.fromdata.signState == "1") {
            ...
            this.tuijiejia = false;
          }
          if(res.data.curNodeName == "处理完毕"){
            this.finishTuihao = false;
          }
