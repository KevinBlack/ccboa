<template>
  <div id="signquteWaitQsh" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <!-- <el-button size="small" plain @click="tuiHXgYyHandel" v-if="isReturn">退回说明</el-button>
          <el-button size="small" plain @click="tuiHSjhHandel" v-if="isUpReturn">上级行退回说明</el-button> -->
          <el-button
            size="small"
    ...
    <el-dialog  v-dialogDrag  title="提示" :visible.sync="sjhsbVisible">
      <el-col>
        {{fsjhsbFlag}}
      </el-col>
      <span slot="footer" class="dialog-footer">
        <el-button  type="primary" @click="sjhsbVisible = false">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 修改重报 -->
    ...
    <!-- 退回说明 -->
    <el-dialog  v-dialogDrag  title="退回说明" :visible.sync="sjhThShuoMingFlag" width="60%" class="xiugaicb">
      <!-- <p class="xuanzTh">选择退回环节</p> -->
      <el-scrollbar>
        <m-table
          size="medium"
          ...
          :isPagination="false"
          :isHandle="false"
          :tableData="tableDataSjhThShuoM"
          :tableCols="tableColsSjhThShuoM"
          v-loading="tableLoading"
          class="table_TH"
        ></m-table>
      </el-scrollbar>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="sjhThShuoMingFlag = false">关 闭</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog  v-dialogDrag  title="拟稿部门意见" width="30%" :modal-append-to-body="true" :visible.sync="nigaoyjFlag">
    ...
    <el-dialog  v-dialogDrag  title="查看上级行状态" :visible.sync="cksjhzt">
      <el-table border :data="tableDataSjh" class="scroll" height="400">
        <el-table-column type="index" fixed label="序号" align="center" width="50"></el-table-column>
        <el-table-column
          v-for="item in  tableColsShj"
          :key="item.id"
          :prop="item.prop"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="big" type="primary" @click="cksjhzt = false">取 消</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog  v-dialogDrag  title="请选择上级行单位" :visible.sync="superiorsenddialog" width="60%" @close="cancelSuperiorDialog">
      <el-dialog  v-dialogDrag  title="请选择上级行单位" width="30%" :visible.sync="innerVisible" append-to-body>
      ...
      <div class="superior_send_box">
        <div class="super_send_top">
          <div class="super_send_top_left">{{ isJXJKtwoDept == 1 ? "上级单位部门" : "上级行部门" }}</div>
        ...
        <div class="super_send_bottom" v-if="getTreeState">
          <div class="super_send_bottom_left">{{ isJXJKtwoDept == 1 ? "上级单位" : "行外单位" }}</div>
          <!-- <div class="super_send_bottom_left">上级单位</div> -->
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancelSuperiorDialog">取 消</el-button>
                  ...
                  <el-form-item label="签报依据" prop="sendBasis" ref="sendBasis">
                    <el-col>
                      <el-button
                        ...
                        plain
                        @click="handleImport"
                        :disabled="fromdata.isLock == 1 || readFields.sendBasis || !readonlyQbcy"
                      >引入</el-button>
                    </el-col>
                  </el-form-item>
                  ...
                  <el-form-item label="会签部门" prop="signDepartmentName" ref="signDepartmentName">
                    ...
                    <div style="display:flex">
                      <el-input
                        ...
                        class="hqbm"
                        :disabled="fromdata.isLock == 1 || readFields.signDepartmentName || !readonlyQbcy"
                      ></el-input>
                      <button
                        @click="bankmain('hui')"
                        v-if="!sendXbSuccess"
                        type="button"
                        class="el-button el-button--default dimi_btn"
                        :disabled="readFields.signDepartmentName  || isSendUpHang == '1' || !readonlyQbcy"
                      >
                        <i class="el-icon-plus"></i>
                      </button>
                      <button
                ...
                <el-form-item label="上级单位批示意见" prop="agreementOpinion" ref="agreementOpinion">
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.agreementOpinion&&isSendUpHang == '0') 
                      || (!editedIdeaFields.agreementOpinion&&isSendUpHang == '1'&&!(fromdata.agreementOpinionList && fromdata.agreementOpinionList.length))"
                      :disabled="editedIdeaFields.agreementOpinion || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=1,fromdata.agreementOpinion&&fromdata.agreementOpinion.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.agreementOpinion && isSendUpHang == '0'"
                    >
                ...
                >
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.agreementHandleSituation&&isSendUpHang == '0') 
                      || (!editedIdeaFields.agreementHandleSituation&&isSendUpHang == '1'&&!(fromdata.agreementHandleSituationList && fromdata.agreementHandleSituationList.length))"
                      :disabled="editedIdeaFields.agreementHandleSituation || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=2,fromdata.agreementHandleSituation&&fromdata.agreementHandleSituation.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.agreementHandleSituation && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                <el-form-item label="请示单位意见" prop="consultOpinion" ref="consultOpinion">
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.consultOpinion&&isSendUpHang == '0') 
                      || (!editedIdeaFields.consultOpinion&&isSendUpHang == '1'&&!(fromdata.consultOpinionList && fromdata.consultOpinionList.length))"
                      :disabled="editedIdeaFields.consultOpinion || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=3,fromdata.consultOpinion&&fromdata.consultOpinion.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.consultOpinion && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                <el-form-item label="核稿意见" prop="hegaoOpinion" ref="hegaoOpinion">
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.hegaoOpinion&&isSendUpHang == '0') 
                      || (!editedIdeaFields.hegaoOpinion&&isSendUpHang == '1'&&!(fromdata.hegaoOpinionList && fromdata.hegaoOpinionList.length))"
                      :disabled="editedIdeaFields.hegaoOpinion || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=4,fromdata.hegaoOpinion&&fromdata.hegaoOpinion.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.hegaoOpinion && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                <el-form-item label="会签意见" prop="signOpinion" ref="signOpinion">
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.signOpinion&&isSendUpHang == '0') 
                      || (!editedIdeaFields.signOpinion&&isSendUpHang == '1'&&!(fromdata.signOpinionList && fromdata.signOpinionList.length))"
                      :disabled="editedIdeaFields.signOpinion || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=5,fromdata.signOpinion&&fromdata.signOpinion.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.signOpinion && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                >
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.draftDepartmentOpinion&&isSendUpHang == '0') 
                      || (!editedIdeaFields.draftDepartmentOpinion&&isSendUpHang == '1'&&!(fromdata.draftDepartmentOpinionList && fromdata.draftDepartmentOpinionList.length))"
                      :disabled="editedIdeaFields.draftDepartmentOpinion || isSendUpHang == '1'"
                      ></el-input>
                    <button
                      @click="writelistbox(clickTypes=6,fromdata.draftDepartmentOpinion&&fromdata.draftDepartmentOpinion.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.draftDepartmentOpinion && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                <el-form-item label="办理情况" prop="handleSituation" ref="handleSituation">
                  <div  class="fix_height">
                    <el-input
                      ...
                      :disabled="editedIdeaFields.handleSituation || isSendUpHang == '1'"
                      v-if="!(fromdata.handleSituationList && fromdata.handleSituationList.length)  || (!editedIdeaFields.handleSituation&&isSendUpHang == '0') || (!editedIdeaFields.handleSituation&&isSendUpHang == '1'&&!(fromdata.handleSituationList && fromdata.handleSituationList.length))"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=7,fromdata.handleSituation&&fromdata.handleSituation.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.handleSituation && isSendUpHang == '0'"
                      >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                <el-form-item label="传阅意见" prop="circulation" ref="circulation">
                  <div  class="fix_height">
                    <el-input
                      ...
                      || (!editedIdeaFields.circulation&&isSendUpHang == '0') 
                      || (!editedIdeaFields.circulation&&isSendUpHang == '1'&&!(fromdata.circulationList && fromdata.circulationList.length))"
                      :disabled="editedIdeaFields.circulation || isSendUpHang == '1'"
                    ></el-input>
                    <button
                      @click="writelistbox(clickTypes=8,fromdata.circulation&&fromdata.circulation.length)"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.circulation && isSendUpHang == '0'"
                    >
                      <i class="icon-font el-icon-edit select-chang"></i>
                    </button>
                  </div>
                ...
                >
                  <el-input
                    ...
                    v-model="fromdata.exceedWordReason"
                     v-if="!(fromdata.exceedWordReasonList && fromdata.exceedWordReasonList.length)  
                      || (!editedIdeaFields.exceedWordReason&&isSendUpHang == '0') 
                      || (!editedIdeaFields.exceedWordReason&&isSendUpHang == '1'&&!(fromdata.exceedWordReasonList && fromdata.exceedWordReasonList.length))"
                      :disabled="editedIdeaFields.exceedWordReason || isSendUpHang == '1'"
                  ></el-input>
                  ...
                  <el-input
                    ...
                    v-model="fromdata.remark"
                     v-if="!(fromdata.remarkList && fromdata.remarkList.length)  
                      || (!editedIdeaFields.remark&&isSendUpHang == '0') 
                      || (!editedIdeaFields.remark&&isSendUpHang == '1'&&!(fromdata.remarkList && fromdata.remarkList.length))"
                      :disabled="editedIdeaFields.remark || isSendUpHang == '1'"
                  ></el-input>
                  <opinionText 
            ...
            <div>
              <el-row  class="pr45">
                <uploadFiles
                  ref="uploadComponent"
                  @preserve="preserve"
                  @initsearch="getQianbaoKhQingshiPoById"
    ...
    <!-- 办结确认弹窗 -->
    <el-dialog  v-dialogDrag  title="办结确认" :visible.sync="banjieFlag" width="60%">
      <p class="xuanzTh">请确认是否办结?</p>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="sureBanjieFun">确 认</el-button>
        <el-button @click="banjieFlag = false">取 消</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      // 【关闭】【保存】【完成并发送】【返回上一处理人】【起草正文】【处理正文】
      // 【查看正文】【删除此文】【生成文号】【合成格式】【退号】【解锁】【清稿】【恢复清稿前正文】【传阅】
      // 【发送会签部门】【查看会签状态】【退回修改】【退回说明】【上级行退回说明】
      // 【发上级行】【查看上级行状态】【办结】【修改重报】【查看流程】【打印处理单】【收藏】【操作指南】
      buttonList: [
        ...
        { name: "退回说明", type: "btn_bhThSm", show: false }, //   办公室核稿有
        { name: "上级行退回说明", type: "btn_sjhThSm", show: false }, //signCbao 修改重报 btn_thCBao
        { name: "发上级行", type: "btn_fsjh", show: false }, //综合处理 总收发
        { name: "查看上级行状态", type: "btn_sjhzt", show: false }, //综合处理 总收发
      ...
      //input框内容
      bank_dispach: {
        function: "getQianbaoKhQingshiPoById",
        id: ""
        // id: "1"
      },
      //下拉框列表
      ...
      //提交
      saveOrUpdateQianbaoKhQingshiPo: {
        function: "saveOrUpdateQianbaoKhQingshiPo",
        id: "",
      ...
      fromdata: {
        ...
        agreementOpinionList:[],
        agreementHandleSituationList:[],
        consultOpinionList:[],
        signOpinionList:[],
      ...
      wanChengSend: false, //完成并发送状态
      chsjhztFlag: false, //查上级行状态
      banjieBtnFlag: false, //办结按钮显示隐藏
      ...
      sjhThShuoMingFlag:false,
      tableDataThShuoM: [],
      tableDataSjhThShuoM:[],
      tableColsThShuoM: ThShuoMfinshData,
      tableColsSjhThShuoM:SjhThShuoMfinshData,
      selectBackListByParentId: {
        function: "selectBackListByParentId",
        id: ""
      },
      getUpHangBackShuoming: {
        function: "getUpHangBackShuoming",
        id: ""
      },
      isReturn: "0", //判断是否退回说明按钮显示隐藏
      isUpReturn: false, // ==1显示   isLock=false解锁
      tuiWenInfo: {
        tuiwen: ""
      },
      ...
      //查看上级行状态
      getShenPiHistoryActInfoFuc: {
        function: "getShenPiHistoryActInfo",
        id: ""
      },
      //查看上级行状态弹窗
      cksjhzt: false,
      tableColsShj: lookShjzt,
      tableDataSjh: [],
  ...
  methods: {
    ...
    getIsJK() {
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            this.isJXJKtwoDept = res.data.isJXJKtwoDept
          } else {
            this.$message({
              type: "error",
              offset: 400,
    ...
    looksjhstate() {
      this.cksjhzt = true;
      this.getShenPiHistoryActInfoFuc.id = this.fromdata.id;
      this.$post
        .postData(
          "getShenPiHistoryActInfo",
          JSON.stringify(this.getShenPiHistoryActInfoFuc),
          this.$businessCode.qbgl
        )
        .then(res => {
          this.tableDataSjh = res.data;
        });
    },
    ...
    xiuGchongBdialog() {
      ...
      this.doUpdateAgainSend.curUserId = this.fromdata.currentUserId;
      this.doUpdateAgainSend.curUserName = this.fromdata.currentUser;
    ...
    //   delete parseList.handleSituationList;
    //   var obj = Object.assign(this.saveOrUpdateQianbaoKhQingshiPo, parseList);
    //   this.$post
    //     .postData(
    //       "saveOrUpdateQianbaoKhQingshiPo",
    //       JSON.stringify(obj),
    ...
    //     .then(res => {
    //       if (res && res.success) {
    //         this.saveOrUpdateQianbaoKhQingshiPo.id = getFormId(
    //           this.$route.query.id,
    //           "skhQshSignpageid",
    //           res.data.id
    //         );
    ...
    //           });
    //         }
    //         this.getQianbaoKhQingshiPoById();
    //       } else {
    //         this.$message({
    //           type: "error",
    //           offset: 400,
    ...
    //上级行退回说明
    tuiHSjhHandel(){
      this.sjhThShuoMingFlag = true;
      this.getUpHangBackShuoming.id = this.fromdata.id;
      this.$post
        .postData(
          "getUpHangBackShuoming",
          JSON.stringify(this.getUpHangBackShuoming),
          this.$businessCode.qbgl
        )
        .then(res => {
          this.tableDataSjhThShuoM = res.data;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    judgeDraf(state, judge) {
      ...
      // if (userBuMen == this.fromdata.draftDepartmentId) {
    ...
    buttonClick(params) {
      switch (params) {
        ...
        case "btn_jsuo":
          this.handledeblock();
          break;
        // { name: "退回说明", type: "btn_bhThSm", show: false },
        //退回说明
        case "btn_bhThSm":
          this.tuiHXgYyHandel();
          break;
        ...
        case "btn_sc":
          // this.shoucang();
          this.collect("qianbao_qingshi",this.fromdata.id,this.fromdata.multiTenancyId);
          break;
        // 查看流程
        case "btn_cklc":
          this.flowScheme();
    ...
    sureTuiWen() {
      ...
      // if (this.tuiWenInfo.tuiwen == "0") {
      //   this.tuiwen = "文档格式不正确";
      // } else {
      //   this.tuiwen = "文档内容不清晰";
      // }
      // this.backSureTuiWen.tuiwenyuanyin = this.tuiwen || this.tuiwenyuanyin;
      // this.backSureTuiWen.backExplain = this.tuiwen || this.tuiwenyuanyin;
      // this.backSureTuiWen.backExplain =
      //   this.tuiWenInfo.tuiwen || this.tuiwenyuanyin;
      let parseList = JSON.parse(JSON.stringify(this.fromdata));
      delete parseList.agreementOpinionList;
      ...
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            this.fromdata.cleanFilePath = "";
            this.fromdata.cleanFileName = "";
      ...
      this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            //   duration: 2000
            // });
            this.fsjhsbFlag = res.message;
            this.sjhsbVisible = true;
          }
        })
        .catch(erro => {
          this.$message({
    ...
    superiorsenddialogbox() {
      ...
      //判断是否是一级行发上级行部门下边树进行显示隐藏
      this.getCurUnitType.function = "getCurUnitType";
      this.$post
        .postData(
          "getCurUnitType",
          JSON.stringify(this.getCurUnitType),
          this.$businessCode.ggqbgl
        )
        .then(res => {
          if (res && res.success) {
            // this.getTreeState = true
            if(res && res.data == 'yjfh'){
                this.getTreeState = false;
            }else{
                this.getTreeState = true;
            }
            this.nowCheckList = [];
    ...
    cancelSuperiorDialog(){
      this.nowCheckList = [];
      this.supertreeDataList.deptNames = "";
      this.supertreeDataList.deptIds = "";
      this.superiorsenddialog = false;
      this.$refs.superTree.setCheckedKeys([]);
    },
    handleSuperiorSend() {
      if (!(this.hangWaiCheck || this.supertreeDataList.deptNames&&this.supertreeDataList.deptNames.length)) {
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
    ...
    superDeleteRow() {
      this.$refs.superTree.setChecked(this.nowCheckList[0], false);
      this.nowCheckList = [];
      this.supertreeDataList.deptNames = "";
      this.supertreeDataList.deptIds = "";
      this.$forceUpdate();
    ...
    concludematter() {
      ...
      // 上级单位批示意见fromdata.handleTypeName
      if(this.fromdata.handleTypeName == "0" && (this.fromdata.agreementOpinionList.length == 0 && !this.fromdata.agreementOpinion)){
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
          message: "上级单位批示意见未填写，无法办结",
          duration: 2000
        });
        return;
      }
    ...
    showCompDialog(data, status, type, params, dtype, radio, isShowFhqbm) {
      ...
      if (status) {
        ...
        switch (type) {
          case "hui":
            ...
            this.saveOrUpdateQianbaoKhQingshiPo.sequenceSign = this.setradio = this.fromdata.sequenceSign = radio;
            if (!ids.length) {
              this.checkIds = [];
            }
            if (dtype != "zengfa") {
      ...
      //综合环节、主表单会签部门的输入框还不能为空 同时成立才能显示发会签部门按钮 正在会签环节名称是正在会签
      if(isShowFhqbm &&  this.fromdata.curNodeName =="综合处理"  && this.fromdata.signDepartmentName){
      ...
      }else if(isShowFhqbm &&  this.fromdata.curNodeName =="综合处理"  && !this.fromdata.signDepartmentName){
        for (var i = 1; i <= this.buttonList.length ;i++) {
          if (this.buttonList[i].name == '发会签部门') {
    ...
    saveSending() {
      ...
      params.function = 'toQianbaoKhQingshiSend'
      this.$post
        .postData(
    ...
    getTcQbsignParamDicInfo(oneTip) {
      this.$post
        ...
        .then(res => {
          ...
          if(this.SqianbaoTypeKH){
            for(let val of Object.values(this.SqianbaoTypeKH)){
              if(val.label == "跨行签报"){
                this.fromdata.qianBaoTypeName = "跨行签报"
              }
            }
          }
          this.SworkSecurity =
          ...
          if(!(this.fromdata.qianbaoCode && this.fromdata.sizeLimit)){
              ...
              //  this.morenqianbao = res.data && res.data.firstQbZihao;
    ...
    //初始值 //input框内容
    getQianbaoKhQingshiPoById(oneTip,loadingFlag) {
      ...
      this.$post
        ...
        .then(res => {
          ...
          this.sendUpHang.sequenceSign = this.bank_toQianbaoSend.sequenceSign = this.setradio =
          ...
          if(res.data.secretType == "商业秘密"){
            if(this.fromdata.secretLimitType!='保密期限'){
              ...
              taoSecret = this.fromdata.security + '☆' + contInfo;
            }else{
              taoSecret = this.fromdata.security + '☆' +this.fromdata.secretLimitName+this.fromdata.secretTimeType;
            }
          ...
          if (res.data.signDepartmentId) {
            ...
            if (
              ...
              this.fromdata.signDepartmentName)||(this.fromdata.signState === "0" &&
              this.fromdata.curNodeName === "请示机构综合处理" &&
          ...
          this.isFlowUpReturn = res.data && res.data.isFlowUpReturn; //判断是否发过上级行  ==1发过 ==0未发过
          ...
          if (this.fromdata.filePath && this.fromdata.fileName) {
            ...
            if (
              ...
              //   this.tuihaoFlag = true;
              // }
              // if(this.fromdata.isLock == 1&& this.isSendUpHang != "1"){
              //   this.deblockFlag = true;
              // }
              // if(this.isFlowUpReturn == '1'){
              ...
              if (res.data.isHasRedHead  == "true") {
                ...
                this.huiFuQG = false;//恢复清稿前正文按钮和清稿按钮不能同时显示
                this.chulizw = false;

                // if (this.fromdata.cleanFilePath) {
                //   this.qinggaoFlag = false;
                //   // this.chsjhztFlag = true; //查上级行状态
                //   this.fashangjihang = true;//发上级行
                //   this.huiFuQG = true;
                // }
                ...
                if (this.fromdata.cleanFilePath) {
                  ...
                  this.fashangjihang = true;//发上级行
                  this.huiFuQG = true;
                  // 判断上级行是否发送过返回请示行的话是true  退回请示行是false    返回与退回不一样  true 隐藏   false 继续判断 
                  if(res.data.isReturnFromUpHang == "true"){
                    ...
                    if (this.isSendUpHang == "1") {
                      if (this.fromdata.consultOpinionList && this.fromdata.consultOpinionList.length) {
                        ...
                        this.huiFuQG = true;
                        this.qinggaoFlag = false;//恢复清稿前正文按钮和清稿按钮不能同时显示
                      }
                    } else {
                      this.fashangjihang = true;
                      this.huiFuQG = true;
...
// this.huiFuQG  // 恢复清稿前正文
          if (
            res.data.fieldStatusMap &&
            res.data.fieldStatusMap.isDocumentNo == "1" && this.fromdata.isLock != 1 
            //==1  退号显示    合成格式显示   处理正文显示   生成文号隐藏
            // ==0 生成文号显示     退号隐藏    合成格式隐藏  处理正文隐藏  （清稿隐藏）
          ) {
            this.chulizw = true;
          }
          // 	跨行：
          // 约束条件“已生成文号，未发上级行”，（约束条件说明：并且已经生成了文号，并且未发送上行）显示【退号】按钮。
          // 约束条件“正文已锁，未发上级行”，（约束条件说明：已经锁定正文，并且未发送上行）显示【解锁】按钮。
          // this.tuihaoFlag = true;
          // this.deblockFlag = true;//解锁按钮

              // if(res.data.fieldStatusMap &&
              //     res.data.fieldStatusMap.isDocumentNo == "1"  && this.isSendUpHang != "1"){
              //   this.tuihaoFlag = true;
              // }
              // if(this.fromdata.isLock == 1&& this.isSendUpHang != "1"){
              //   this.deblockFlag = true;
              // }
              // if(res.data.fieldStatusMap &&
              //     res.data.fieldStatusMap.isDocumentNo == "1"  && this.isFlowUpReturn == '1'){
              //   this.tuihaoFlag = false;
              // }
              ...
              if(res.data.isOutDept == "0"){
                if(res.data.fieldStatusMap &&
                    res.data.fieldStatusMap.isDocumentNo == "1"  && this.isFlowUpReturn != '1' && this.isSendUpHang != "1" && this.isSignFinish != "0"){
                  this.tuihaoFlag = true;
                }else{
                  this.tuihaoFlag = false;
                }
          ...
          if (this.isSendUpHang == "1") {
            ...
            this.chsjhztFlag = true; //查上级行状态
            this.banjieBtnFlag = false; //办结按钮显示隐藏
          } else {
            this.wanChengSend = true;
            this.chsjhztFlag = false;
            this.banjieBtnFlag = true;
          }

          this.isUpReturn = res.data && res.data.isUpReturn;   //1被退回

          //判断修改重报按钮显示隐藏
          if (this.isUpReturn == 1) {
            this.isUpReturn = true;
            this.tuihaoFlag = false;
            this.deblockFlag = false;
          } else {
            this.isUpReturn = false;
          }
          
          //判断是否退回重报查看正文进行解锁
          if (this.fromdata.isLock == "0" && res.data.isHasRedHead  == "true") {
            this.chakanzw = false;
            this.chulizw = true;
            this.hechengzw = false;
            this.qinggaoFlag = true;
            this.fashangjihang = false;//发上级行
            this.huiFuQG = false;
          }
          // if (this.isUpReturn == 1) {
          //   // this.chsjhztFlag = false; //查上级行状态
          //   this.fashangjihang = false;//发上级行
          //   this.huiFuQG = false;
          // }
          ...
          // if (this.fromdata.isLock == "1" && res.data.isHasRedHead == "true" && !this.fromdata.cleanFilePath) {
          //   this.huiFuQG = false;
          //   this.chakanzw = false;
          //   this.chulizw = true;
          // }else{
          ...
          //   this.chulizw = false;
          // }
          if (this.fromdata.filePath && this.fromdata.fileName && this.fromdata.cleanFilePath) {
            this.huiFuQG = true;
            this.qinggaoFlag = false;//恢复清稿前正文按钮和清稿按钮不能同时显示
            this.chakanzw = true;
            this.chulizw = false;
          ...
          //判断有会签部门  核稿环节，行领导秘书，行领导环节不能显示
          //
          // if(this.fromdata.signDepartmentName){
          //   this.huiQianHuanJieFlag = false
          // }
          ...
          // if(this.huiFuQG == true && this.isFlowUpReturn == '1' && this.isUpReturn == 1){
    ...
    async preserve(type,loadingFlag) {
      ...
      delete parseList.handleSituationList;
      var obj = Object.assign(this.saveOrUpdateQianbaoKhQingshiPo, parseList);
      obj.function = "saveOrUpdateQianbaoKhQingshiPo";
      ...
      //bug 清稿问题
      // if(this.isUpReturn == 1 && this.fromdata.cleanFilePath){
      //   obj.filePath = this.fromdata.cleanFilePath;
      //   obj.fileName = this.fromdata.cleanFileName;
      //   obj.cleanFilePath = "";
      //   obj.cleanFileName = "";
      // }
      ...
      await this.$post
        ...
        .then(res => {
          if (res && res.success) {
            ...
            // this.saveOrUpdateQianbaoKhQingshiPo.id = this.bank_dispach.id =
            //   res.data.id;
            // this.bank_toQianbaoSend.id = res.data.id;
            // sessionStorage.setItem("skhQshSignpageid", res.data.id);
            this.saveOrUpdateQianbaoKhQingshiPo.id = this.bank_dispach.id = this.bank_toQianbaoSend.id = getFormId(
              this.$route.query.id,
              "skhQshSignpageid",
              res.data.id
            );
          ...
          // this.wordsLimitInfo.wordsLimit = this.fromdata.sizeLimit || 0;
            ...
            this.getQianbaoKhQingshiPoById(null,loadingFlag);
            successState = true;
          } else {
            this.$message({
              type: "error",
    ...
    shoucang() {
      ...
      let draftInfo = this.fromdata.filePath
        ...
        if(this.fromdata.upAttchmentFileInfo && this.fromdata.upAttchmentFileInfo.length){
          this.fromdata.upAttchmentFileInfo.map(items => {
            draftInfo.push(items);
          });
        }
      
      // draftNewInfo = [].concat(draftInfo);
      // draftNewInfo.map(item => {
    ...
    processSheet() {
      ...
      // this.bank_dispach.id = getFormId(this.$route.query.id,"skhQshSignpageid");
      // let parseList = JSON.parse(JSON.stringify(this.fromdata));
      // delete parseList.agreementOpinionList;
      // delete parseList.agreementHandleSituationList;
      // delete parseList.consultOpinionList;
    ...
    doDraft(fileInfo,saveName,isQC) {
      ...
      if(this.draftFlag == 'frommoude'){
         let routeData = this.$router.resolve({
          ...
          query: {
            ...
            pageFlag:'signqutote',
            wenjianId:getFormId(this.$route.query.id, "skhQshSignpageid")
          }
        });
        window.open(routeData.href, "_blank");
        return;
      ...
      if (this.zhengWenState == 2) {
        localStorage.setItem("skhQshSignpagePage", "true");
      }
    ...
    //             // let transArr = [];
    //             // console.log(typeof that.fromdata.sendBasis);
    ...
    writelistvalue(data) {
      if (data) {
        if (this.clickTypes == 1) {
          ...
          // 上级单位办理情况
          this.fromdata.agreementHandleSituation = this.fromdata
            .agreementHandleSituation
            ? this.fromdata.agreementHandleSituation + data
            : data;
            if(this.fromdata.agreementHandleSituation && this.fromdata.agreementHandleSituation.length>=500){
              this.fromdata.agreementHandleSituation = this.fromdata.agreementHandleSituation.substr(0,500)
            }
          ...
          // 核稿意见
          this.fromdata.hegaoOpinion = this.fromdata.hegaoOpinion
            ? this.fromdata.hegaoOpinion + data
            : data;
            if(this.fromdata.hegaoOpinion && this.fromdata.hegaoOpinion.length>=500){
              this.fromdata.hegaoOpinion = this.fromdata.hegaoOpinion.substr(0,500)
            }
  ...
  created() {
    ...
    this.getQianbaoKhQingshiPoById("1");
