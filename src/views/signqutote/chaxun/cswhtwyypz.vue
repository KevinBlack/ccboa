...
const finshData = [
  { label: "协办日期", prop: "createTime", align: "center" },
  { label: "签报字号", prop: "documentNo", align: "center" },
  { label: "标题", prop: "title", align: "center" },
  { label: "拟稿部门", prop: "draftDepartmentName", align: "center" },
  { label: "协办部门", prop: "deptName", align: "center" },
  { label: "协办状态", prop: "curNodeName", align: "center" },
  { label: "协办办理时长", prop: "workDayIntervalTimes", align: "center" }
];
...
export default {
  ...
  methods: {
    exportExc(){
      exportTable.exportExcel("签报协办时间查询表",".el-table");
    },
    initData() {
      this.$router.push({
        query: {
    ...
    cellClick(row, column, cell, event) {
      ...
      if(row.status == 'daiban') {
        this.handleOpenPage("/xiebanWaitQbDispatch", this.idType);
    ...
    resetForm(formName) {
      ...
      this.bank_countersignature.function = "queryQianbaoXieBan";
      this.bank_countersignature.createTimeStart = "";
      this.bank_countersignature.createTimeEnd = "";
