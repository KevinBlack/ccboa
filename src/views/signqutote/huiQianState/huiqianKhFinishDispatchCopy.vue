<template>
  <!-- 中国建设银行电子签报会签意见单
    非跨行  跨行请示行  已办
   -->
  <div id="signquteFinishKhHqGly" class="bank_dispach">
    <el-row>
  ...
  data() {
    return {
      ...
      sendTackFuc: {
        ...
        // QB_DZQB  电子签报,QB_SWQB  实物签报 ,QB_KHQBQS 跨行签报请示,KHQB_SP  跨行签报审批 ,KHQB_HQ  会签 ,QB_XB 协办
      },
  ...
  methods: {
    ...
    async maintenance(){
      let data={
        ...
        let obj3=await this.$post.postData(
            ...
            JSON.stringify(Object.assign(data,{function :"selectSysQianbaoFile",tplbsnid: 'KHQB_HQ',id: this.$route.query.id, unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
    ...
    sendFinalIdea(){
      ...
      if(this.fromdata.orderType == "跨行单会签"){
        this.doFinishHQXBCB.serviceInstans = "KHQB_HQ"
      }
      let parseList = JSON.parse(JSON.stringify(this.fromdata));
      delete parseList.agreementOpinionList;
      delete parseList.agreementHandleSituationList;
...
//请求文件路径
    ...
    flowScheme() {
      ...
      this.bank_cklc.unitId = this.fromdata.unitId;
      this.$post
