<template>
  <!-- 中国建设银行电子签报会签意见单
    非跨行  跨行请示行    已办
   -->
  <div id="signquteFinishFKhHqGly" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        <p class="bank_dispach_back">
          <!-- <i class="el-icon-arrow-left"></i>中国建设银行电子签报会签意见单 -->
        </p>
<!-- 【关闭】【维护】【流程调度】【查看正文】【查看流程】【打印处理单】【收藏】 -->
        <div class="grid_comtent">
          ...
          <el-button size="small" plain @click="completeSending">选择会签人</el-button>
          ...
          <el-button size="small" plain v-show="banjiedispatch.curNodeCode != '800' && fromdata.isShowLC!=1" @click="processScheduling">流程调度</el-button>
          <el-button size="small" plain @click="drafting('0')">查看正文</el-button>
          <el-button size="small" plain @click="flowScheme">查看流程</el-button>
          <el-button size="small" plain @click="processSheet">打印处理单</el-button>
          <el-button size="small" plain @click="collect('qianbao_huiqian',fromdata.id,fromdata.multiTenancyId, unitId)">收藏</el-button>
        </div>
      </el-col>
    </el-row>
        ...
        <div class="bank_dispach_file_part">
          ...
          >
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item label="签报字号" prop="qianbaoCode" ref="qianbaoCode">
                    <el-select  v-model="fromdata.qianbaoCode+''+fromdata.documentNo" placeholder="请选择"  style="width: 100%;" disabled>
                      <el-option
                ...
                <el-col :span="8">
                  <el-form-item label="拟稿日期" prop="draftTime" ref="draftTime" disabled>
                    <el-date-picker
                      type="date"
                      placeholder="选择日期"
                ...
                <el-col>
                  <el-form-item label="会签部门">
                    <div  style="display:flex">
                      ...
                      <el-input v-model="fromdata.signDepartmentName" type="textarea" resize="none" autosize disabled
                        style="width: 100%;"
                        class="hqbm1" ></el-input>
                      <!-- <button
                        type="button"
            ...
            <el-row class="pr45">
              <el-col>
                <el-form-item label="批示意见">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.approvalOpinion" disabled v-if="!(fromdata.approvalOpinionList&&fromdata.approvalOpinionList.length)"></el-input>
              ...
              <el-col>
                <el-form-item label="审核意见">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.checkOpinion" disabled v-if="!(fromdata.checkOpinionList&&fromdata.checkOpinionList.length)"></el-input>
              ...
              <el-col>
                <el-form-item label="会签意见">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.signOpinion" disabled v-if="!(fromdata.signOpinionList&&fromdata.signOpinionList.length)"></el-input>
              ...
              <el-col>
                <el-form-item label="拟稿部门意见">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.draftDepartmentOpinion" disabled v-if="!(fromdata.draftDepartmentOpinionList&&fromdata.draftDepartmentOpinionList.length)"></el-input>
              ...
              <el-col>
                <el-form-item label="传阅意见">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.circulation" disabled v-if="!(fromdata.circulationList&&fromdata.circulationList.length)"></el-input>
              ...
              <el-col>
                <el-form-item label="备    注">
                  ...
                  <el-input type="textarea" resize="none" autosize maxlength="500" v-model="fromdata.remark" disabled v-if="!(fromdata.remarkList && fromdata.remarkList.length)"></el-input>
            ...
            <div>
              ...
              <el-row>
                <el-col>
                  <div class="tree_w" v-if="fromdata.attchmentFileInfo&&fromdata.attchmentFileInfo.length">
                    <el-scrollbar>
  ...
  methods: {
    ...
    async maintenance(){
      let data={
          ...
          title: '中国建设银行电子签报会签意见单'
        }
        let obj1=await this.$post.postData(
            "getQianbaoHuiqianById",
            JSON.stringify(Object.assign(data,{function :"getQianbaoHuiqianById", unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res&&res.data||[];
          })
          .catch(err => {
        ...
        let obj3=await this.$post.postData(
            ...
            JSON.stringify(Object.assign(data,{function :"selectSysQianbaoFile",tplbsnid: 'DZQB_HQ',id: this.$route.query.id, unitId: this.unitId})),this.$businessCode.qbgl
          ).then(res => {
            return res.data;
          })
          .catch(err => {
        ...
        // if(obj1&&obj2){
          const routeData = this.$router.resolve({
              ...
              query: {
                ...
                type:'qb',
                sign: 2,
                selectUrl: 'getToQianbaoHuiqianEntityById',
    ...
    sendFinalIdea(){
      ...
      obj.function = 'doFinishHQXBCB'
        this.$post
        .postData("doFinishHQXBCB", JSON.stringify(obj), this.$businessCode.qbgl)
        .then(res => {
          if (res && res.success) {
            this.$message({
                type: "success",
    ...
    //         // this.bank_selectConfig.curNodeId = res.data.tplNo;
    ...
    completeSending() {
      ...
      this.$post
        ...
        .then(res => {
          ...
          let parms = Object.assign(this.bank_send,this.fromdata);
            // parms.pcsAvyId = this.bank_send.pcsAvyId = res.data && res.data[0].PCSAVYID;
            parms.pcsAvyId = this.fromdata.isMutil == 1 ? this.banjiedispatch.curNodeCode : res.data[0].PCSAVYID;
            // parms.tplNo = this.bank_send.tplNo = res.data && res.data[0].PCSAVYID;
            parms.curTplNo = this.bank_send.curTplNo = res.data && res.data[0].PTPLNO;
            parms.curMultiTenancyId = this.bank_send.curMultiTenancyId =
              res.data && res.data[0].MULTITENANCYID;
            parms.unitId = this.unitId;
          this.$post
    ...
    this.bank_dispach.unitId = this.unitId;
      this.$post
        ...
        .then(res => {
          ...
          //获取签报处理单主标题
          // this.directTitle = res.data && res.data.orgName + "电子签报会签单";
          if(res.data && res.data.isSpecialHead && res.data.isSpecialHead == "1"){
            this.directTitle = this.fromdata.draftDepartmentName + "电子签报会签单";
          }else{
            this.directTitle = res.data && res.data.orgName + "电子签报会签单";
          }
          // 签报依据
          ...
          this.createSighWf.sequenceSign = this.saveOrUpdateToQianbaoHuiqianEntity.sequenceSign = this.setradio = (res.data.sequenceSign && res.data.sequenceSign.toString())||'1';
...
//已上传的文件
    ...
    async preserve(type) {
      ...
      delete parseList.remarkList;
      delete parseList.mainDeptAttachment;
      var obj = Object.assign(this.saveOrUpdateToQianbaoHuiqianEntity, parseList);
      obj.function = 'saveOrUpdateToQianbaoHuiqianEntity'
      obj.unitId = this.unitId;
       await  this.$post
        .postData(
          "saveOrUpdateToQianbaoHuiqianEntity",
 ...
 // 附件上传下载排序
    ...
    deleteRow(n,id) {
      ...
      this.bank_deleteFileById.unitId = this.unitId;
       this.$post
        .postData("deleteFileById", JSON.stringify(this.bank_deleteFileById), this.$businessCode.qbglfj)
        .then(res => {
          console.log(res);
          if (res) {
              if (res && res.success) {
    ...
    downFile(file){
      ...
      this.bank_doDownloadFile.filePath = file.filePath;
       this.$post
        .postData("doDownloadFile", JSON.stringify(this.bank_doDownloadFile), this.$businessCode.qbglfj)
        .then(res => {
          console.log(res);
          if (res) {
              if (res && res.success) {
