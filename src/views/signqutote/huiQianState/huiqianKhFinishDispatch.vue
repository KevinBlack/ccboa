<template>
  <!-- 中国建设银行电子签报会签意见单
    非跨行  跨行请示行  已办
  -->
  <div id="signquteFinishKhHq" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent" v-if="fromdata.isMutil == 1">
          ...
          <el-button size="small" plain @click="shxbr('sh')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb"  v-show="readonlyQbcy">收回多人会签</el-button>
          <el-button size="small" plain @click="shxbr('cb')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb"  v-show="readonlyQbcy">催办多人会签</el-button>
        ...
        <div class="grid_comtent" v-else>
          ...
          <el-button size="small" plain @click="shxbr('sh')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb"  v-show="readonlyQbcy">收回多人会签</el-button>
          <el-button size="small" plain @click="shxbr('cb')" v-if="fromdata.isShowSendXb=='1'&&!fromdata.isLastXb"  v-show="readonlyQbcy">催办多人会签</el-button>
          <el-button size="small" plain @click="sendTackBtn" v-if="allowBack =='1' && readFlag == '0'"  v-show="readonlyQbcy">收回</el-button>
          <el-button size="small" plain @click="hastenWork" v-if="!(fromdata.isShowSendXb=='1'&&!fromdata.isLastXb)"  v-show="readonlyQbcy">催办</el-button>
          <el-button size="small" plain @click="flowScheme">查看流程</el-button>
          <el-button size="small" plain @click="processSheet">打印处理单</el-button>
          <el-button size="small" plain @click="collect('qianbao_huiqian',fromdata.id,fromdata.multiTenancyId)">收藏</el-button>
          <el-button size="small" plain @click="instructions"  v-show="readonlyQbcy">操作指南</el-button>
                ...
                <el-form-item label="会签部门最终意见">
                  ...
                  <el-input
                      ...
                      v-model="fromdata.finalOpinion"
                       disabled
                      v-if="!(fromdata.finalOpinionList && fromdata.finalOpinionList.length) || fromdata.finalOpinion"
                    ></el-input>
                    <opinionText 
...
export default {
  ...
  methods: {
   ...
   //表单校验滚动  mimi
    ...
    //       case "CN000": //发文
    //         this.$businessCode.ggqbgl = "A08462001";
    //         break;
    //       case "CN001": //收文
    //         this.$businessCode.ggqbgl = "A08462003";
    //         break;
    //       case "CN002": //签报
    //         this.$businessCode.ggqbgl = "A08462005";
    //         break;
    //     }
    ...
    //           //   that.showImport = false;
    //           //   that.showImportData = true;
    //           //   that.$businessCode.ggqbgl = "A08462001";
