<template>
  ...
  <div style="background: white;margin: 0 auto;" class="conferDetail">
    ...
    >
      <el-form ref="elForm" :rules="rules" :model="formData" label-width="120px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="会议地点" prop="hyAdress">
              <el-input v-model="formData.hyAdress" placeholder="请选择" @focus="focusEvent" />
        ...
        <el-form-item label="出席">
          <el-row :span="24" style="margin-bottom:20px">
            ...
            <el-col :span="22">
              <div class="leaders" prop="cxLd">
                <el-input class="left" v-model="formData.cxLd" style="width:90%"></el-input>
                <el-button
                  class="right"
          ...
          <el-row :span="24">
            ...
            <el-col :span="22">
              <div class="leaders">
                <el-input class="left" v-model="formData.cxGg" style="width:90%"></el-input>
                <el-button class="right" @click="selectPerson('高管')" type="primary" size="medium">选择</el-button>
              </div>
            </el-col>
          </el-row>
        </el-form-item>
        <el-row :span="24" style="margin-bottom:20px">
          <div style="float:right;">
            ...
            >添加</el-button>
            <el-button class="right" type="primary" size="medium" @click="delBank">删除</el-button>
          </div>
        </el-row>
                  ...
                  <el-row :span="24" type="flex" justify="space-between">
                    <el-col :span="10">
                      <el-row type="flex" justify="space-around">
                        <el-col :span="1">
                          <el-checkbox v-model="item.checked" @change="clickMe(item)"></el-checkbox>
                        </el-col>
                        <el-col :span="2">
                          <div class="tooltip">
                            <span>领导</span>
                        ...
                        <el-col :span="19">
                          <el-input v-model="item.ldName" class="input-with-select">
                            <el-button
                              slot="append"
                    ...
                    <el-col :span="13">
                      <el-row type="flex" justify="space-around">
                        ...
                        <el-col :span="20">
                          <el-input class="left" v-model="item.LdqxReason" style="width:100%"></el-input>
                        </el-col>
                      </el-row>
                    </el-col>
                  </el-row>
        ...
        <el-row :span="24" style="margin-bottom:20px">
          <div style="float:right;">
            <el-button
              ...
              style="margin-left:10px"
            >添加</el-button>
            <el-button class="right" type="primary" size="medium" @click="delBanky">删除</el-button>
          </div>
        </el-row>
                ...
                <el-col :span="22">
                  <el-row :span="24" type="flex" justify="space-between">
                    <el-col :span="10">
                      <el-row type="flex" justify="space-around">
                        <el-col :span="1">
                          <el-checkbox v-model="item.checked" @change="clickHld(item)"></el-checkbox>
                    ...
                    <el-col :span="13">
                      <el-row type="flex" justify="space-around">
                        ...
                        <el-col :span="20">
                          <el-input class="left" v-model="item.GgqxReason" style="width:100%"></el-input>
                        </el-col>
                      </el-row>
                    </el-col>
                  </el-row>
        ...
        <el-form-item>
          <div class="right">
            ...
            <el-button
              ...
              size="mini"
              plain
              @click="adviseDownMove"
              v-if="!isShowBmButton"
            >下移</el-button>
            <el-button type="primary" size="mini" plain @click="adviseDel" v-if="!isShowBmButton">删除</el-button>
            <el-button
              type="primary"
              size="mini"
              plain
              ...
              v-if="!isShowChButton"
            >查看参会部门流程</el-button>
            <el-button type="primary" size="mini" plain @click="back" v-if="!isShowBmButton">退回</el-button>
            <el-button
              type="primary"
              size="mini"
              plain
        ...
        <el-form-item label="报名信息">
          ...
          <el-row style="margin-bottom:17px">
            ...
            <el-col :span="23">
              <el-input v-model="formData.lxJs" placeholder="请输入监事姓名" />
            </el-col>
          </el-row>
          <!-- </div> -->
          <!-- 报名信息表格 -->
      ...
      <el-dialog title="发送结果" :visible.sync="dialogVisibleSend" width="40%">
      ...
      <el-dialog title="发送结果" :visible.sync="dialogVisibleEnd" width="40%">
        <div v-if="this.msgAry.length">
          <div style="color:green">
            <p style="line-height: 30px;">{{msgAry[0]}}</p>
          </div>
          <div>
            <p style="line-height: 30px;">{{msgAry[1]}}</p>
          </div>
        </div>
        <div v-else>
          <div style="color:green">
            <p style="line-height: 30px;">{{msgDataAry}}</p>
          </div>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="dialogVisibleEnd = false">确 定</el-button>
        </span>
      </el-dialog>
      <!-- 查看报名状态 -->
      <el-dialog title="查看报名状态" :visible.sync="dialogVisibleType">
        <m-table
          size="medium"
          :tableData="bmTypeData"
          :tableCols="bmTypeCol"
          :isPagination="false"
          :sortable="false"
        ></m-table>
...
export default {
  ...
  data() {
    return {
      ...
      msgArr: [], //发送成功之后的返回直
      msgDataAry: "",
      msgAry: [], ////正式通知发送成功之后的返回直
      npmYt: {},
      isShowBmButton: false, //会议内容按钮显示隐藏
      isShowChButton: false, //参会流程按钮显示隐藏
      ...
      dialogVisibleSend: false, //报名通知发送弹框
      dialogVisibleEnd: false, //正式通知发送弹框
      ...
      bmTypeCol: [
        ...
        { label: "报名状态", prop: "bmFlag" },
        { label: "处理人", prop: "currentUser" },
        { label: "结束时间", prop: "bmTimeStr" }
      ],
      choosetableData: [
      ...
      buttonList: [
        ...
        { name: "查看报名状态", type: "ckbmzt", show: false },
        ...
        { name: "生成会议单", type: "shengchenghuiyidan", show: true },
         { name: "处理会议单", type: "chulihuiyidan", show: false },
  ...
  methods: {
    ...
    //查看报名状态
    seeBmzt_btn() {
      this.$api.secreatary.meetings.ckBmType({ id: this.id }).then(res => {
        this.bmTypeData = res.data;
      ...
      if (ytLength == 0) {
        ...
        this.$api.secreatary.meetings
          ...
          .then(res => {
            ...
            //   type: "success"
            // });
            var mesg = res.msg.split("#");
            // this.msgArr = mesg;
            if (mesg) {
              this.msgAry = mesg;
            } else {
              this.msgDataAry = re.msg;
            }
        ...
        this.$post
          ...
          .then(res => {
            if (res && res.success) {
              ...
              this.depart = "scqjd";
              this.doDraftOne(res); //使用金格控件打开申请
            } else {
              this.$message({
                type: "error",
                showClose: true,
      ...
      return new Promise((resolve, reject) => {
        this.$api.secreatary.meetings
          ...
          .then(res => {
            ...
            console.log("this.bankList, AbsenceList", this.bankList);
            this.LDbankList = res.data.map(item => item.ldName).join(",");
            resolve(res);
          });
      });
    },
    ...
    getSelected() {
      ...
      // sessionStorage.setItem("hyType", this.hyType);
      ...
      if (this.hyType == "预订成功" || this.hyType == "审批中") {
        this.$message({
          message:this.multipleSelection[0].returnExplain,
          type: "warning"
        });
      }
    ...
    saveMethod() {
      ...
      //重新保存会议单子段格式
      try{
        this.formData.tzAttachTitle = JSON.parse(this.formData.tzAttach).data.fileName;
       this.formData.tzAttach = JSON.parse(this.formData.tzAttach).data.filePath;
     }catch(e){
    ...
    send() {
      ...
      this.objKow.meetingName =
        this.formData.title + "会议单" ? this.formData.title + "会议单" : "";
     ...
     //tzAttach字段不是json格式直接取值
     try{
        bank_doDownloadFile.fileName = JSON.parse(this.formData.tzAttach).data.fileName;
       bank_doDownloadFile.filePath = JSON.parse(this.formData.tzAttach).data.filePath;
     }catch(e){
        ...
        this.$api.secreatary.meetings
          ...
          .then(res => {
            ...
            this.meetingTime = res.data.hyTimeStr
              .replace("时", ":")
              .replace("分", "");
            // console.log(this.meetingTime, res.data.hyTimeStr);
            ...
            this.ifView = 1;
            // 查看报名状态显示隐藏 isShowBmButton 为 'true' 显示 ，否则隐藏
            if (this.formData.isShowZbdButton === "false") {
              this.isShowBmButton = true;
            } else {
              this.isShowBmButton = false;
            }
            // 查看参会流程显示隐藏 isShowChButton 为 'true' 显示 ，否则隐藏
            if (this.formData.isShowChButton === "false") {
              this.isShowChButton = false;
            } else {
              this.isShowChButton = true;
            }
            // 查看报名状态显示隐藏 isShowBmButton 为 'true' 显示 ，否则隐藏
            if (this.formData.isShowBmButton === "true") {
              let ckbmztIndex = undefined;
              const ckbmztObj = this.buttonList.find((item, index) => {
                if (item.type === "ckbmzt") {
                  ckbmztIndex = index;
                  return true;
                }
              ...
              this.$set(this.buttonList, ckbmztIndex, {
                ...ckbmztObj,
                show: true
              });
            } else {
            ...
            //生成过会议单则显示处理会议单按钮
            if(this.formData.tzAttachModified==='1'){
        ...
        .then(res => {
          ...
          this.bmTableData.map((item, index) => {
            ...
            this.newLxData.lxList.push(
              item.lxDeptName + " " + item.chName + "列席" + "\n"
            );
          });
          // }
        });
