<template>
  <div id="receiveElectronDispatch" class="receive_dispatch">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          >关闭</el-button>
          <!-- <el-button plain class="bank_grid_comtent_active" size="small" @click="handleGg">公告</el-button> -->
          <el-button
            size="small"
            plain
    ...
    <el-dialog title="收回传阅人" v-dialogDrag :visible.sync="shdialog1" width="50%">
      <el-card>
        <el-checkbox
    ...
    <!-- 自动分办弹窗 -->
    <el-dialog title="自动分办" :visible.sync="zdfbdialog" width="30%" :before-close="zdfbdialogFalse">
      <el-card v-if="zdfbList&&zdfbType == 3">
        <div v-for="item in zdfbList" :key="item" style="padding-top:20px;">
          <span style="margin-right:20px;display:inline-block;min-width:70px;">{{item.key}}</span>
          <el-radio-group v-model="checkedZdfb" v-for="item1 in item.value" :key="item1.id">
            <el-radio :label="item1" style="margin-right:16px;">{{item1.name}}</el-radio>
          </el-radio-group>
        </div>
        <!-- <el-radio-group v-model="checkedZdfb" v-for="item in zdfbList" :key="item.id">
          <el-radio :label="item" style="margin-right:16px;">{{item.name}}</el-radio>
        </el-radio-group>-->
      </el-card>
      <el-card v-if="zdfbList.success&&zdfbType == 2">
        <el-col>
          <div style="font-size: 15px;line-height: 22px;">
            是否自动选择主办部门：
            <br />
            {{zdfbList.undertakeDeptName}}
          </div>
        </el-col>
      </el-card>
      <el-card v-if="zdfbType==1">
        <el-col>
          <div style="font-size: 15px;line-height: 22px;">{{zdfbList.message}}</div>
        </el-col>
      </el-card>

      <span slot="footer" class="dialog-footer">
        <el-button @click="zdfbdialogFalse()">取 消</el-button>
        <el-button v-if="zdfbType == 2" type="primary" @click="zdfbqd('4')">确 定</el-button>
        <el-button v-if="zdfbType == 3" type="primary" @click="zdfbqd('3')">确 定</el-button>
      </span>
    </el-dialog>
    <!-- //收回弹窗-->
    <el-dialog title="退文原因" :visible.sync="shouhuidialog" width="30%">
      <el-card>
        <el-checkbox-group v-model="checkedtw">
          <el-checkbox
            v-for="(item,index) in twData"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="tuiwenqueding">确 定</el-button>
      </span>
    </el-dialog>
    ...
    <header class="bank_dispach_header">
      ...
      <h3 class="bank_dispach_tit" v-else>中国建设银行电子收文处理单</h3>DOCUMENT_TITLE-->
      ...
      <div class="bank_dispach_file">
        <div class="bank_dispach_file_part">
          ...
          >
              ...
              <el-row>
                ...
                <el-col :span="10">
                  <el-form-item label="是否阅件:" style="float: right;padding-right: 20px;">
                    <el-radio v-model="fromdata.isRead" :label="1" :disabled="readFields.isRead">是</el-radio>
                    <el-radio v-model="fromdata.isRead" :label="0" :disabled="readFields.isRead">否</el-radio>
                  </el-form-item>
                </el-col>
                <el-col
                  v-if="isSupervise == 1"
              ...
              <el-row>
                ...
                <el-col>
                  <el-form-item label="正文标题">
                    <el-input placeholder="正文标题" v-model="fromdata.title" disabled></el-input>
                  </el-form-item>
                </el-col>
                <el-col>
                ...
                >
                  <el-col
                    :span="8"
                    v-if="fromdata.secretType == '商业秘密'||fromdata.secretType == '国家秘密'"
                  >
                  ...
                  <el-col
                    ...
                    v-if="fromdata.secretType == '商业秘密'||fromdata.secretType == '国家秘密'"
                  >
                    <el-form-item label="密级和保密期限" :span="8">
                      <el-input v-model="fromdata.secretFlagType" disabled style="width:40%"></el-input>
                      <span v-if="fromdata.secretType == '商业秘密'">
                        <i class="el-icon-star-off pad_10"></i>
                      </span>
                      <span v-if="fromdata.secretType == '国家秘密'">
                        <i class="el-icon-star-on pad_10"></i>
                      </span>
                ...
                <div v-if="fromdata.fromWhere == '实物收文'">
                  ...
                  <el-col :span="4" class="linecenter">
                    <el-form-item label="/" label-width="20px">
                      <el-input :value="fromdata.secretLimitType" disabled></el-input>
                    </el-form-item>
                  </el-col>
                </div>
                ...
                <div v-if="isSupervise == 1">
                  ...
                  <el-col :span="8">
                    <el-form-item label="督办日期" prop="superviseStartTime" ref="superviseStartTime">
                      <el-input v-model="fromdata.superviseStartTime" disabled></el-input>
                    </el-form-item>
                  </el-col>
                </div>
                ...
                >
                  <el-col>
                    <el-form-item
                      ...
                      prop="isForword"
                      ref="isForword"
                      v-if="!(( fromdata.fromWhere == '电子发文' || fromdata.fromWhere == '实物发文' )   && fromdata.isDrect == '1' )"
                    >
                      <el-radio-group
                        :disabled="fromdata.isSendSub == 1||fromdata.isInner != 1 || readFields.isForword"
                  ...
                  <el-col :span="8" v-show="(fromdata.isForword==1)||(fromdata.isDirect==1)">
                    <el-form-item label="直发文件" prop="isDirect" ref="isDirect">
                      <el-radio-group
                        :disabled="fromdata.isSendSub == 1||(!fromdata.isForword&&fromdata.isDirect==1)||fromdata.isInner != 1 || readFields.isDirect"
                  ...
                  <el-col :span="8">
                    <el-form-item
                      ...
                      prop="directRange"
                      ref="directRange"
                      v-show="(fromdata.isDirect==1&&fromdata.isForword==1)||fromdata.isDirect==1"
                    >
                      <el-select
                        :disabled="fromdata.isSendSub == 1||(!fromdata.isForword&&fromdata.isDirect==1) || readFields.directRange"
                  ...
                  <el-col :span="8">
                    ...
                    >
                      <el-select
                        :disabled="fromdata.isSendSub == 1||(!fromdata.isForword&&fromdata.isDirect==1) || readFields.directType"
                  ...
                  <el-col>
                    <el-form-item label="保管期限" prop="keepingLimitName" ref="keepingLimitName">
                      <el-input v-model="fromdata.keepingLimitName" disabled></el-input>
                    </el-form-item>
                  </el-col>
                </div>
                <el-col>
                  <el-form-item label="主办部门" prop="undertakeDeptName" ref="undertakeDeptName">
                    ...
                    >
                    <el-button slot="append" :disabled="readFields.undertakeDeptName" @click="getTreeData('chen')" icon="el-icon-plus"></el-button>
                    </el-input>-->
                    <div style="display:flex">
                      <el-input
                        ...
                        disabled
                        placeholder="请选择主办部门"
                        style="flex:1;color:black !important;"
                        class="hqbm"
                      ></el-input>
                      <button
                ...
                <el-col :span="12">
                  <el-form-item label="协办部门" prop="assistDeptName" ref="assistDeptName">
                    <!-- <div style="display:flex">
                    <button @click="getTreeData('xie')" v-if="fromdata.isHasXB != 1" type="button" class="el-button el-button--default dimi_btn" > <i class="el-icon-plus"></i></button>-->
                    <!-- <button @click="getTreeData('xie',1)" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1" >增发</button>
                      <button @click="takeBackSign('shouhui','xie')" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1" >收回</button>
                    <button @click="takeBackSign('cuiban','xie')" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1">催办</button>-->
                    <!-- <div v-show="!readFields.assistDeptName">
                        <button @click="getTreeData('xie',1)" :disabled="readFields.assistDeptName" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1" >增发</button>
                        <button @click="takeBackSign('shouhui','xie')" :disabled="readFields.assistDeptName" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1" >收回</button>
                        <button @click="takeBackSign('cuiban','xie')" :disabled="readFields.assistDeptName" v-if="fromdata.isHasXB == 1" type="button" class="el-button el-button--default dimi_btn1">催办</button>
                      </div>
                    </div>-->
            ...
            <el-row class="pr45">
              <el-col>
                <el-form-item label="批示意见">
                  <div class="fix_height" style="display:flex">
                    <el-input
                      type="textarea"
                      maxlength="500"
                      resize="none"
                    ...
                    <button
                      ...
                      type="button"
                      class="el-button el-button--default dimi_btn"
                      v-if="!editedIdeaFields.pishiOpinion"
                    >
                ...
                <el-form-item label="办公室批办意见" prop="officeOpinion" ref="officeOpinion">
                  <div class="fix_height" style="display:flex">
                    ...
                    <button
                      ...
                      v-if="!editedIdeaFields.officeOpinion"
                      type="button"
                      class="el-button el-button--default dimi_btn"
                    >
                      <i class="el-icon-edit"></i>
            ...
            <!-- 问与答 -->
            <template v-if="isAskShow">
              <h6 class="bank_dispach_file_tit">问与答</h6>
              <el-row class="pr45">
                <el-col>
                  <el-form-item label="问与答">
                    <el-card class="box-card" v-if="listShow">
                      ...
                      <div v-for="(item,index) in ListWD" :key="index" class="text item">
                        <span
                          style="color: #f00;"
                          v-if="item.question.renyuanType==1"
                        >{{'【问】' +item.question.questionUserName+"向拟稿人 ("+item.question.answerUserName+") 提问："}}</span>
                        <span
                          style="color: #f00;"
                          v-if="item.question.renyuanType==2"
                        >{{'【问】' +item.question.questionUserName+"向发送人 ("+item.question.answerUserName+") 提问："}}</span>
                        <span style="color: #000;">{{item.question.questionContent}}</span>
                        <span>{{"("+item.question.createTime+")"}}</span>
                        <br />
                        <span v-if="item.answer">
                          <span style="color: #0000FF;">{{'【答】' +item.answer.answerUserName }}</span>
                ...
                <el-form-item label>
                  ...
                  <el-col :span="2" style="text-align: center;margin-top:40px">
                    <el-button @click="dialogVisibleGBWT = true">提问</el-button>
                  </el-col>
                </el-form-item>
              </el-row>
            </template>
    ...
    <!-- wenda弹窗 -->
    <el-dialog title="请选择提问对象" :visible.sync="dialogVisibleGBWT" width="15% !important">
      <span>
        <div class="demo-input-suffix" style="padding-left: 10%;padding-bottom: 5%;">
          拟稿人：
          <!-- item.question.answerUserName -->
          <el-radio v-model="straightdelivery" label="1" class="alternative">{{radioWD1}}</el-radio>

          <!-- <el-radio v-model="radioWD1"></el-radio><div>{{radioWD2}}</div> -->
        </div>
        <div class="demo-input-suffix" style="padding-left: 10%;">
          发送人：
          <el-radio v-model="straightdelivery" label="2" class="alternative">{{radioWD2}}</el-radio>
          <!-- <el-radio v-model="radioWD1" ></el-radio><div>{{radioWD1}}</div> -->
          <!-- <el-radio v-model="radioWD2" label="222">备选项</el-radio> -->
        </div>
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisibleGBWT = false">取 消</el-button>
        <el-button type="primary" @click="wenDa">确 定</el-button>
      </span>
    </el-dialog>
    <!-- wenda弹窗END -->
...
export default {
  ...
  data() {
    return {
      ...
      qzshIsIndeterminate: false, //强制收回全选按钮不确定性属性
      qzshCheckAll: false,
      isAskShow: "",
      closeHandler: false,
      //qzsh
      ...
      ggjyCode: this.$businessCode.swgl,
      //完成并发送弹窗下拉框
      bank_selectConfig: {
      ...
      buttonList: [
        ...
        { name: "自动分办", type: "btn_zdfb", show: false }, //[16]
        { name: "退文", type: "btn_tw", show: false }, //[3]
        ...
        { name: "办结", type: "btn_banjie", show: false, index: 4 }, //[10]
        { name: "传阅", type: "btn_cy", show: false }, //[8]
        { name: "发送主办部门", type: "btn_fscbbm", show: false }, //[6]
        { name: "发送协办部门", type: "btn_fsxbbm", show: false }, //[7]
        { name: "删除", type: "btn_sccw", show: false }, //[4]
        { name: "查看承办状态", type: "btn_ckcbzt", show: false }, //[18]
        { name: "查看协办状态", type: "btn_ckxbzt", show: false }, //[19]
        { name: "查看流程", type: "btn_cklc", show: false }, //[17]
        { name: "打印处理单", type: "btn_dycld", show: false }, //[20]
        { name: "收藏", type: "btn_sc", show: false }, //[9]
        { name: "操作指南", type: "btn_czzn", show: false } //[21]
      ],
      offenDevices: [],
      titleType: "",
      tableData1: [],
  ...
  methods: {
    ...
    wenDa(data, status, type, params, dtype) {
      let stringType = "";
      this.dialogVisibleGBWT = false;

      if (this.straightdelivery == "1") {
        this.straightdelivery = this.nigaorenId;
        this.renyuanType = "1";
      } else if (this.straightdelivery == "2") {
        this.straightdelivery = this.fasongrenId;
        this.renyuanType = "2";
      }
      if (this.renyuanType == "1") {
        stringType = this.curUser + "向拟稿人" + this.radioWD1 + "提问";
      } else {
        stringType = this.curUser + "向发送人" + this.radioWD2 + "提问";
      }
      if (this.wenti == "") {
        this.$message({
          type: "error",
      ...
      let shform = {
        ...
        receiveWenhao: this.fromdata.documentCode, //收文字号
        questionUserId: JSON.parse(localStorage.getItem("userInfo")).humanId, //当前处理人ID
        questionUserName: this.curUser, //当前处理人
        answerUserName: this.curUser,
        renyuanType: this.renyuanType,
        answerUserId: this.straightdelivery, //this.fromdata.userId,//回答人user_id
        questionContent: this.wenti, //问题内容
        documentTitle: this.fromdata.title, //标题
        sendOrgName: this.fromdata.sendOrgName, //来文单位
        draftUserId: this.nigaorenId,
        stringType: stringType
      };
      this.$post
        .postData("saveQuestion", JSON.stringify(shform), this.swglCode)
        .then(res => {
          if (res && res.success) {
            if (this.renyuanType == "1") {
              this.$message({
              ...
              this.$message({
                ...
                showClose: true,
                offset: 400,
                message: "问题已提交给" + this.radioWD2,
                duration: 1500
              });
            }
            this.straightdelivery = "";
            this.wenti = "";
            this.wendaList();
          } else {
            this.$message({
    ...
    getTreeDataForCy(n, m) {
      ...
      let judgmentCondition1 = transSecretLimitType.includes(
        that.fromdata.secretLimitType
      );
      let judgmentCondition2 = transSecretLimitType.includes(
      ...
      if (
        ...
        if (
          !(
            (that.fromdata.secretType == "商业秘密" &&
              (judgmentCondition1 || judgmentCondition2)) ||
    ...
    //this.lowerBankFlag == '否'||this.fromdata.isSendSub==1
    buttonClick(params) {
      switch (params) {
        case "btn_bc":
          this.preserve("save");
    ...
    setConfig() {
      ...
      if (this.huanJieMessage.visibleButtons.btn_send) {
        if (
          ...
          ((this.fromdata.lowerBankFlag == "上级发送下级行" &&
            this.fromdata.isDirect == 1) ||
            this.fromdata.isForword == 1) &&
            (this.fromdata.isInner == "" || this.fromdata.isInner == "1")
        ) {
      ...
      //fromdata.fromWhere == '电子收文'
      if (
        this.buttonList[4].show &&
        (this.fromdata.lowerBankFlag == "下级上报上级行" ||
          this.fromdata.lowerBankFlag == "同级发送") &&
        this.fromdata.isSendSub != 1 &&
        this.fromdata.fromWhere &&
        this.fromdata.fromWhere.includes("发")
      ) {
    ...
    setFormConfig() {
      this.$nextTick(() => {
        this.rules = {
          assistDeptName: [
            {
              required: this.requiredFields.assistDeptName,
              trigger: "change",
              message: "请选择协办部门"
            }
          ],
          ...
          undertakeOrgOpinion: [
            {
              required: this.requiredIdeaFields.undertakeOrgOpinion,
              message: "不能为空",
              trigger: "blur"
            },
            { min: 0, max: 500, message: "输入内容超过500字限制,请修改!" }
          ...
          officeOpinion: [
            {
              required:
                this.requiredIdeaFields.officeOpinion &&
                !(
        ...
        if (
          this.huanJieMessage.requiredFields.limitDate &&
          this.isSuperviseRuqer()
        ) {
    ...
    //发送
    fasongBtn() {
      if (!this.fromdata.forwordRequirements && this.fromdata.isForword == 1) {
        this.$message({
          type: "error",
          offset: 400,
          showClose: true,
    ...
    //退文
    tuiwen() {
      /*if(this.fromdata.lowerBankFlag != '下级上报上级行'||this.fromdata.isSendSub==1){
        this.$message({
          type: "error",
          showClose: true,
          offset: 400,
    ...
    tuiwenqueding() {
      if (this.checkedtw.length > 0 || this.twyy != "") {
        if (this.checkedtw.length > 0) {
          this.checkedtw = this.checkedtw.join(";") + ";" + this.twyy;
        } else {
          this.checkedtw = this.twyy;
        }
        this.$post
          .postData(
            "doBackSendOrder",
            JSON.stringify({
              function: "doBackSendOrder",
              processInstId: this.fromdata.processInstId,
    ...
    async shouwenduban() {
      ...
      await this.$post
        .postData("superviseReceiveDZ", JSON.stringify(obj), this.swglCode)
        .then(res => {
          if (res && res.success) {
            this.$message({
              type: "success",
    ...
    unshouwenduban() {
      let gobackform = {
        function: "cancelSuperviseReceiveDZ",
        id: this.receiveselect.id
      };
      let copyFrom = JSON.parse(JSON.stringify(this.fromdata));
      delete copyFrom.remarksList;
    ...
    shQueding() {
      ...
      let shform = {
        ...
        id: this.receiveselect.id,
        takeBackDeptNames: cbList
      };
      if (this.shdialogTypeUnder == true) {
        Object.assign(shform, { undertake: "" });
      } else {
        if (shform.undertake) {
      ...
      this.$post
        ...
        .then(res => {
          if (res.data) {
            ...
            if (res.data.sendUserName)
              this.nigaoren = res.data.sendUserName.split(";")[1];
            if (res.data.sendUserName)
              this.fasongren = res.data.sendUserName.split(";")[0];
            if (res.data.sendUserId)
              this.nigaorenId = res.data.sendUserId.split(";")[1];
            if (res.data.sendUserId)
              this.fasongrenId = res.data.sendUserId.split(";")[0];
    ...
    clearOpinion() {
      ...
      this.fromdata.undertakeOrgOpinion = ""; //主办部门办理情况
    ...
    receiveselectlist() {
      this.uploadConfig.id = this.receiveselect.id = this.receiveselect.id
        ...
        : sessionStorage.getItem("pageSWDZid") || "";
      this.getDataOne();
      this.getDataTwo();
      this.getDataThree();
      this.getDataFour();
    ...
    zdfbdialogFalse() {
      this.zdfbdialog = false;
      this.checkedZdfb = [];
    },
    zdfbqd(type) {
      if (type == "3") {
        if (!this.checkedZdfb.name) {
          this.$message({
            type: "error",
            showClose: true,
            offset: 400,
        ...
        this.fromdata.undertakeDeptName = this.checkedZdfb.name;
        this.fromdata.undertakeDeptId = this.checkedZdfb.id;
        this.preserve("1").then(res => {
          ...
          this.$post
            .postData("bmfb", JSON.stringify(httpObj), this.swglCode)
            .then(data => {
      ...
      if (type == "4") {
        this.fromdata.undertakeDeptName = this.zdfbList.undertakeDeptName;
        this.fromdata.undertakeDeptId = this.zdfbList.undertakeDeptId;
        this.preserve("1").then(res => {
          let httpObj1 = {
            function: "bmfb",
            ...this.fromdata
          ...
          this.$post
            .postData("bmfb", JSON.stringify(httpObj1), this.swglCode)
            .then(data => {
              if (data && data.success) {
                this.$message({
                  type: "success",
    ...
    async preserve(type) {
      this.cantDisabled = true;
      let syccessfalg = false;
      this.checkedZdfb = [];
      this.saveOrUpdateSW.id =
        this.receiveselect.id || sessionStorage.getItem("pageSWDZid") || "";
      ...
      delete copyFrom.feedbackAttachmentMap;
      delete copyFrom.finalOpinionList;
      delete copyFrom.opinionMap;
      delete copyFrom.attchmentFileInfo;
      let obj = Object.assign(this.saveOrUpdateSW, copyFrom);
      await this.$post
        .postData("saveOrUpdate", JSON.stringify(obj), this.swglCode)
        .then(res => {
          this.cantDisabled = false;
          if (res && res.success) {
            sessionStorage.setItem("pageSWDZid", res.data.id);
            if (type && type != "uploadfile" && type != "1") {
              this.$message({
                type: "success",
                showClose: true,
                offset: 400,
        ...
        this.$confirm("是否返回给：上一处理人?", "提示", {
          ...
          callback: action => {
            if (action == "confirm") {
              ...
              this.preserve("1").then(() => {
                if (this.buttonList[9].show) {
    ...
    backDialogData(type) {
      ...
      switch (type) {
        ...
        case "xie":
          ...
          break;
        case "fasong":
          this.checkIds = this.fromdata.checkedCities1
            ? this.fromdata.checkedCities1.split(";")
            : [];
    ...
    flowScheme() {
      ...
      this.$post
        .postData("getWfHistory", JSON.stringify(this.bank_cklc), this.swglCode)
        .then(res => {
          if (res) {
    ...
    // deleteFromNode(copyFrom,deleteFromArr){
    ...
    zidongfenban() {
      this.$post
        .postData(
          "selectByCurBankId",
          JSON.stringify({
            function: "selectByCurBankId",
            bussinessTypeName: this.fromdata.bussinessTypeName || "",
            // bussinessTypeName:"人力资源,会计,董事会,公司业务,国际业务",
            id: this.fromdata.id
          }),
          this.$businessCode.swgl
        )
        .then(res => {
          this.zdfbList = res.data;
          //一种业务分类也不自动选择
          // if (
          //   this.zdfbList.undertakeDeptName &&
          //   this.zdfbList.undertakeDeptName.indexOf(",") < 0
          // ) {
          //   this.zdfbType = 2;
          // }
          if (res.success && this.zdfbList) {
            this.zdfbType = 3;
            let newArr = [];
            for (let key in this.zdfbList) {
              let newObj = {};
              let newA = [];
              let idObj = this.zdfbList[key].undertakeDeptId.split(",");
              let nameObj = this.zdfbList[key].undertakeDeptName.split(",");
              for (let index = 0; index < idObj.length; index++) {
                newA.push({
                  id: idObj[index],
                  name: nameObj[index]
                });
              }
              newArr.push({ key: key, value: newA });
            }
            this.zdfbList = newArr;
            this.$forceUpdate();
            this.zdfbdialog = true;
          } else {
            this.$message({
              type: "error",
              showClose: true,
          ...
          // if (this.zdfbList.success && this.zdfbList.undertakeDeptName) {
          //   this.zdfbType = 3;
          //   let idObj = this.zdfbList.undertakeDeptId.split(",");
          //   let nameObj = this.zdfbList.undertakeDeptName.split(",");
          //   let newA = [];
          //   idObj = this.zdfbList.undertakeDeptId.split(",");
          //   for (let index = 0; index < idObj.length; index++) {
          //     newA.push({
          //       id: idObj[index],
          //       name: nameObj[index]
    ...
    handleSend() {
      if (
        ...
        this.fromdata.secretType == "请选择" ||
        this.fromdata.secretTimeType == "" ||
        this.fromdata.secretTimeType.includes("公布前")
      ) {
        this.mmlx = true;
      } else {
        this.mmlx = false;
