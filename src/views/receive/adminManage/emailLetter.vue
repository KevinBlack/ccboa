  ...
  data() {
    return {
      ...
      bank_countersignature: {
        // choseYear:"",
        function: "selectReceiveXDHJByPage",
        page: 1, //当前页数
        limit: 20, //每页条数
        fileCode: "",
        documentCode: "",
      ...
      if(this.tableSelectList.length==0){
        ...
        this.$confirm("是否确认删除？", "提示", {
          ...
          callback: action => {
            if (action == "confirm") {
              let httpobj={
                function:'deleteXdhjByManager',
                ids:this.deletIds,
                unitName:this.bank_countersignature.unitName,
                unitId:this.bank_countersignature.unitId
              };
              this.$post.postData("deleteXdhjByManager", JSON.stringify(httpobj), this.$businessCode.swgl)
                .then(res=>{
                  if(res&& res.success){
                    this.tableSelectList=[];
                    this.deletIds="";
