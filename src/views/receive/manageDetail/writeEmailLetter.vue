<template>
  <div id="app" class="bank_dispach">
    <el-row>
      <el-col class="bank_dispack_side">
        ...
        <div class="grid_comtent">
          ...
          <!-- <el-button size="small" plain @click="processSheet('/xdhjdjd')">打印处理单</el-button> -->
          <!-- <el-button size="small" plain @click="preserve('save')">保存</el-button>
          <el-button size="small" plain @click="bcbdj">保存并登记</el-button>
          <el-button size="small" plain @click="processSheet('/xdhjdjd')">打印处理单</el-button>
    ...
    <header class="bank_dispach_header">
      <h3 class="bank_dispach_tit">中国建设银行信电函件处理单</h3>
    </header>
        ...
        <div class="bank_dispach_file_part">
          ...
          >
            ...
            <div style="margin-top:40px;display: inline-block;">
              ...
              <el-col :span="8">
                <el-form-item label="收件日期" prop="receiveTime" ref="receiveTime">
                  <el-date-picker
                    type="date"
                    placeholder="收件日期"
              ...
              <el-col :span="8">
                <el-form-item label="来件渠道" prop="fromType" ref="fromType">
                  <el-select
                    v-model="fromdata.fromType"
                    placeholder="请选择"
                    style="width:100%"
                    disabled
                  >
                    <el-option
                      v-for="sendTypeitem in fromMod"
                      :key="sendTypeitem.id"
                      :label="sendTypeitem.value"
                      :value="sendTypeitem.value"
                    ></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="登记人" ref="recorder" prop="recorder">
                  <el-input v-model="fromdata.recorder" disabled></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="收件类型" :span="8" prop="fileTypeForXd" ref="fileTypeForXd">
                  <el-select
                    v-model="fromdata.fileTypeForXd"
                    placeholder="请选择"
                    style="width:100%"
                    disabled
                  >
                    <el-option
                      ...
                      :key="emergencyLevelitem.id"
                      :label="emergencyLevelitem.value"
                      :value="emergencyLevelitem.value"
                    ></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="密级" :span="8" prop="security" ref="security">
                  <el-select v-model="fromdata.security" placeholder="请选择" style="width:100%" disabled>
                    <el-option
              ...
              <el-col :span="8">
                <el-form-item label="缓急" :span="8" prop="emergencyName" ref="emergencyName">
                  <el-select v-model="fromdata.emergencyName" placeholder="请选择" style="width:100%" disabled>
                    <el-option
                      v-for="textTypeitem in StextType"
                      :key="textTypeitem.id"
                      :label="textTypeitem.value"
                      :value="textTypeitem.value"
                    ></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="收件编号" prop="fileCode" ref="fileCode">
                  <el-input v-model="fromdata.fileCode" disabled></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="数量" prop="count" ref="count">
                  <el-input-number v-model="fromdata.count" :min="0" disabled></el-input-number>
                </el-form-item>
              </el-col>
              <!-- <el-col :span="24">
                <el-form-item label="来文单位"  prop="sendDept" ref="sendDept">
                    <el-select  multiple v-model="fromdata.sendDeptId" style="width:100%">
                      <el-option
                      v-for="item in SendDept"
                      :key="item.id"
                      :value="item.value"
                      :label="item.value"
                    ></el-option>
              ...
              <el-col :span="24">
                <el-form-item label="来件单位" prop="sendOrgName" ref="sendOrgName" style="overflow:hidden;height: 40px;">
                  <el-input
                    ...
                    disabled
                  ></el-input>
                  <el-select v-model="fromdata.sendOrgName" placeholder="请选择"
                             style="width: 100%;z-index: 1;position:absolute;left: 0;" disabled>
                    <el-option
                      v-for="item in SendDept"
                      :key="item.value"
                      :label="item.value"
              ...
              <el-col>
                <el-form-item label="承办部门" prop="undertakeDeptName" ref="undertakeDeptName">
                  <el-input
                    ...
                    disabled
                  >
                    <el-button slot="append" @click="completeSending('hui')" icon="el-icon-plus"></el-button>
                  </el-input>
                </el-form-item>
                <el-col>
                  <el-form-item label="备    注">
                    <el-input v-model="fromdata.remark" type="textarea" resize="none" maxlength='500'
                              autosize placeholder="填写备注" disabled></el-input>
...
export default {
  ...
  data() {
    return {
      ...
      //下拉框列表
      bank_dispachfile: {
        function: "getXDHJDrawdownDataSource",
        // organizationId: "2ec81dcb-9ed5-443a-a7e7-314ae0537478"
      },
      //提交
      bank_saveOrUpdateXDHJInfo: {
        function: "saveOrUpdateXDHJInfo",
      },
  ...
  methods: {
    ...
    async maintenance() {
      ...
      let obj1 = await this.$post.postData(
        "selectReceiveXDHJById",
        JSON.stringify(Object.assign(data, {function: "selectReceiveXDHJById",})), this.$businessCode.swgl
      ).then(res => {
        return res.data;
      })
        .catch(err => {
      ...
      //     "getReceiveXdhjSuggestInfoById",
      //     JSON.stringify(Object.assign(data,{function :"getReceiveXdhjSuggestInfoById",})),this.$businessCode.swgl
      //   ).then(res => {
      //     return res.data;
      //   })
      //   .catch(err => {
    ...
    //时间格式转换
    timeChange(type) {
      this.fromdata.receiveTime = dateFormate.date(Date.parse(this.fromdata.receiveTime))
    },
    //选择人员后 确定完成并发送
    saveSending() {
      ...
      this.$post
        .postData("doSend", JSON.stringify(params), this.$businessCode.swgl)
        .then(res => {
          if (res && res.success) {
            this.$message({
              type: "success",
    ...
    getXDHJDrawdownDataSource() {
      ...
      this.$post
        ...
        .then(res => {
          obj.fromMod = res.data.xd_from_mode;
          obj.SemergencyLevel = res.data.xd_type;
          obj.SbusinessType = res.data.xd_slevel;
          obj.StextType = res.data.xd_hlevel;
          obj.SendDept = res.data.send_dept_name;

          let dom = res.data.under_take_dept;
          for (i in dom) {
            obj.treeData[name] = i[value]
          }
    ...
    async preserve(type) {
      ...
      let obj = Object.assign({function: "saveOrUpdateXDHJInfo"}, this.fromdata);
      await this.$post
        .postData("saveOrUpdateXDHJInfo", JSON.stringify(obj), this.$businessCode.swgl)
        .then(res => {
          sessionStorage.setItem("XDHJId", res.data.id);
          this.bank_dispach.id = res.data.id;
          if (res && res.success) {
            this.$message({
              type: "success",
              showClose: true,
  ...
  created() {
    ...
    this.toEdit();
    this.getXDHJDrawdownDataSource();
  },
  destroyed() {
