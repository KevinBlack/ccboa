...
const finshData = [
  ...
  { label: "承办部门", prop: "undertakeDeptName", align: "center",width:"150" }
];
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        function: "selectLowerLevelRepeat",
        state: 1,
        page: 1, //当前页数
        limit: 20, //每页条数
        totalCount: 0,
  ...
  methods: {
    ...
    resetForm(formName) {
      ...
      (this.receiveTime = ""), //收文日期值
        (this.ruleForm.function = "selectLowerLevelRepeat"),
    ...
    //导出
    exportExc(){
      exportTable.exportExcel("下级行重发文查询统计表",".el-table");
    },
    routeYear(){
      this.$router.push({
        query: {
