<template>
  <!-- 8.	公文督查办单
  同收文单（行外电子收文（总行）、行外电子收文（一级行）、行内电子收文
  、实物收文处理单）。在电子收文单最上方添加“办理期限”和“办理进度”两个字段
   -->
  <div class="tablePrint">
    <el-row>
      <h1 style="" class="title-h1">中国建设银行电子实物收文处理单</h1>
    ...
    <table cellspacing="0" cellpadding="0" class="printTableList" style="font-family: 宋体;font-size: 19px;"
      ...
      <tr style="height: 32px;">
        <td style="width:10%;text-align:center">直发文件</td>
        <td style="width:26%;padding-left: 10px;">{{ fromdata.isDirect }}</td>
        <td style="text-align:center">直发范围</td>
        <td style="padding-left: 10px;">{{ fromdata.directRange }}</td>
        <td style="text-align:center" class="biaozhi" id="mjbs">直发文种类</td>
        <td style="padding-left: 10px;" class="biaozhi">{{ fromdata.isDirect }}</td>
      </tr>
      <tr style="height: 32px;">
