<template>
  <div>
    <el-row>
      ...
      <el-col style="margin: 20px 0;" class="el-table">
        <table class="table_box table_box_hade">
          <tr style>
            <th>单位</th>
            <th>数量</th>
          </tr>
        </table>
        <table class="table_box" v-for="(value,key) in tableData" :key="key">
          <tr style="background: #e9ecf2;">
            <td colspan="2">{{ key}}</td>
          </tr>
          <tr v-for="(item,index) in value" :key="index">
            <td style="width:50%">{{item.unitName}}</td>
            <td style="width:50%">{{item.totleCount}}</td>
          </tr>
        </table>
      </el-col>
        ...
        <table class="stcs_table">
          <tr>
            <td>
              <br />
              <span style="font-weight: 700;">说明：</span>
              <br />1.统计范围：各机构辖内所有分支机构总收发和部门综合用户未登记收文数量，包括网点。
            </td>
          </tr>
        </table>
      </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        function: "statisticsUnregistered",
        state: 1,
        startTime: "", //   统计起始日期
        endTime: "" //    统计结束日期
      },
  ...
  methods: {
    exportExc(){
      exportTable.exportExcel("收文未登记情况统计表",".el-table");
    },
    initData() {
      this.$post
        .postData(
          "statisticsUnregistered",
