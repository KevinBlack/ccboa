...
export default {
  ...
  methods: {
    ...
    postFile (param, onProgress) {
      ...
      // url, data, fwTranId, pagination, needLoading
      return this.$post.postData('doFileUploade', JSON.stringify(this.bank_uplod), this.$businessCode.fawglfj, '', false, formData, config).then(rs => rs.data)
    },
    // 文件校验方法
    validateFile (file) {
      return this.$post.postData('doFileUploade', JSON.stringify(this.bank_uplod), this.$businessCode.fawglfj, '', false,  null, null, file).then(rs => rs.data)
    }
  }
}
</script>