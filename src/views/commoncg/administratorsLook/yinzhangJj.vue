...
' @FilePath: \OA1\src\views\sealManage\statistical\introductionStatistical.vue 保管交接
'
<template>
  <div>
    <div class="draft">
      ...
      <el-form ref="statisticalForm" :model="form" label-width="80px">
        <el-row>
          ...
          <!-- 流转显示拟稿日期 -->
          <el-col :span="6" v-if="activeTab=='transfer'">
...
export default {
  ...
  data() {
    return {
      ...
      // 区分调用接口 2已完结 4流转中
      tableCols: [
        { label: "拟稿日期", prop: "cdate" , width: 150},
        { label: "印章名称", prop: "title" , width: 150},
    ...
    activeTab: {
      handler(val) {
        ...
        if (val === "transfer") {
          this.processType = 4;
          this.tableCols = [
            { label: "拟稿日期", prop: "cdate" },
            { label: "印章名称", prop: "title" },
            { label: "编号", prop: "documentCode" },
            { label: "移交人", prop: "creator" },
  ...
  methods: {
    getNumYearList() {
      return new Promise((resolve, reject) => {
        this.$api.numYear.getNumYearList({tbParam:'yzjjAdmin'}).then(res => {
    ...
    selectYear(val) {
      ...
      if (val == "近两年") {
        ...
        // this.form.documentCode = "〔" + new Date().getFullYear() + "〕"
      } else {
        this.form.numYear = val;
        this.form.documentCode = "〔" + val + "〕";
      }
    },
    select(row) {
      let dataArry = [];
    ...
    deleteForms() {
      ...
      this.$confirm(
        ...
        this.$api.sealManage.sealCommon
            .deleteSealKeeperChange(jsonData)
          .then(res => {
            if (res.code == "SUCCESS") {
              this.deleteArry = "";
    ...
    query() {
      ...
      this.formList.documentCode = this.form.documentCode;
      this.formList.numYear = this.form.numYear;
      this.loadData();
    },
    // 重置按钮
    ...
    handleOpenPage(path, id, readType, flag,draftOrganId) {
      ...
      const routeData = this.$router.resolve({
        path: path,
        query: {id:id, draftOrganId:draftOrganId,type: this.processType,numYear:this.form.numYear}
      });
    ...
    cellClick(row) {
      ...
      //   path,
      //   query: {
      //     draftOrganId: row.draftOrganId, // 机构id
      //     id: row.id,
      ...
      // });
      // 不允许打开同一页面
      this.handleOpenPage(path, row.id,'','',row.draftOrganId)
    }
  },
  created() {
