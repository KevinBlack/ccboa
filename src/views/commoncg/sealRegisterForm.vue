  ...
  <div class="sealRegisterForm">
    ...
    <div class="process_content" id="bank_dispach_content">
      ...
      >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="印章类别" prop="yzType">
              <el-select v-model="form.yzType" placeholder="请选择" style="width: 100%">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="印章状态" prop="sealStatus">
              <el-radio-group v-model="form.sealStatus" @change="selectStmpStCd">
                <el-radio
          ...
          <el-col :span="12">
            <el-form-item label="序号" prop="markNo">
              <el-input v-model="form.markNo" @change="stmpBsnClIDChg"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="领用人" prop="getSealP" :rules="[{ required: ifJkunit, message: '请填写领用人', trigger: 'blur' }]">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="启用日期" prop="useDate" :rules="[{ required: !ifJkunit, message: '请选择启用日期', trigger: 'change' }]">
              <el-date-picker
                type="date"
                placeholder="请选择日期"
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="印章设备编号" v-if="EqmtMftrNm_isShow">
              <el-input v-model="form.deviceNoBak2" :readonly="true"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="24" v-if="StorePlcCd_isShow">
            <el-form-item label="印章存放地点">
              <el-input v-model="form.speace"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
                ...
                <el-form-item>
                  <el-col :span="24">
                    ...
                    >
                      <el-scrollbar>
                        <div
                          v-for="item in form.attchmentFileInfo"
                          :key="item.id"
                          class="comp_list"
                        >
                          <template>
...
export default {
  ...
  mixins: [sealDeviceMixins, openOrDownLoadFiles,minixs,draftMuban,viewDraft,dispatchIndex,isNowUser],
  data: function data() {
    return {
      ifJkunit: false, // 判断当前用户是否为金科， true 是， false 否
      //单位名称
      unitName: JSON.parse(localStorage.userInfo).unitName,
      headerBtnList: ["关闭", "保存", "收藏"],
      //印章种类
      sealTypes: [],
      attTitleID: [],
  ...
  methods: {
    ...
    saveData: function() {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          // 判断是否存放到用印机器中
          if (this.form.isLocate == '0') {
            console.log(this.form, '获取否的情况')
            this.form.deviceNameBak = ''
            this.form.deviceSpnoBak = ''
            this.form.deviceNoBak2 = ''
            this.form.deviceNoBak = ''
            this.form.storeMark = ''
            this.form.rpidFlag = ''
          }
          if (!this.id) {
    ...
    isCurrentUserJXJK () {
      this.$api.sealManage.sealRegisterForm.isIFJXJK({}).then(response => {
        console.log("response----", response)
        if (response.code === "SUCCESS") {
          this.ifJkunit = response.ifJkunit;
        }
      })
    },
  },
