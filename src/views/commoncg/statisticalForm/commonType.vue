  ...
  <div>
    ...
    <el-form :model="form" class="statistical" ref="elForm" :rules="rules" label-width="80px">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="印章类别" prop="yyType">
            <el-select v-model="form.yyType" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in YzTypeList" :key="item.index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="用印时间" prop="draftTime">
            <el-date-picker
  ...
  created() {
    this.$api.sealManage
      ...
      .then(res => {
        if (res.code == "SUCCESS") {
          console.log(res);
          this.YzTypeList = res.data.scUnit.replace(/，/g, ",").split(",");
          console.log(this.YzTypeList);
        }
      })
      .catch(e => {});
    // this.loadData();
