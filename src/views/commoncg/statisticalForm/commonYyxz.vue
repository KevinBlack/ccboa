  ...
  <div>
    ...
    <el-form :model="form" class="statistical" ref="elForm" :rules="rules" label-width="80px">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="用印性质" prop="type">
            <el-select v-model="form.type" placeholder="请选择" style="width: 100%">
              <el-option v-for="item in YyxzList" :key="item.index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
...
export default {
  ...
  created() {
    this.$api.sealManage
      ...
      .then(res => {
        if (res.code == "SUCCESS") {
          console.log(res);
          this.YyxzList = res.data.scFiletypeHw.replace(/，/g, ",").split(",");
          console.log(this.YyxzList);
        }
      })
      .catch(e => {});
    // this.loadData();
  ...
  methods: {
    ...
    //导出
    exportData() {
      exportTable.exportExcel("用印性质统计");
    },
    // 重置按钮
    reset() {
      this.form.type = "";
