  ...
  <div>
    ...
    <el-form :model="form" class="statistical" ref="elForm" :rules="rules" label-width="80px">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="用印部门" prop="draftDepartment">
...
export default {
  ...
  data() {
    return {
      ...
      tableCols: [
        { label: "用印部门", prop: "adn" },
        { label: "本级行申请单个数", prop: "bhSQ" },
        { label: "本级行用印数量", prop: "bhYY" },
        { label: "下级行申请单个数", prop: "xjhSQ" },
        { label: "下级行用印数量", prop: "xjhYY" }
      ],
      pagination: {
        pageNum: 1,
        pageSize: 10
  ...
  methods: {
    ...
    //导出
    exportData() {
      exportTable.exportExcel("用印部门统计");
    },
    // 重置按钮
    reset() {
