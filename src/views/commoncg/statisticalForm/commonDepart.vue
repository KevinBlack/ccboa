      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="发往单位类型" prop="sendType">
            <el-select v-model="form.sendType" placeholder="请选择" style="width: 90%">
              <el-option v-for="(item,index) in typeList" :key="item.index" :label="item" :value="index"></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
...
export default {
  ...
  methods: {
    ...
    //导出
    exportData() {
      exportTable.exportExcel("发往单位类型统计");
    },
    // 重置按钮
    reset() {
      this.form.sendType = "";
    ...
    loadData() {
      ...
      this.$api.sealManage.sealStatistical
        .statisticalUnitsType({ ...this.form })
        .then(res => {
