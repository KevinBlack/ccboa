...
export default {
  ...
  methods: {
    ...
    //导出
    exportData() {
      exportTable.exportExcel("印章状态统计", "#table-tj");
    },
    // 重置按钮
    reset() {
      this.tableData = [];
    ...
    loadData() {
      this.$api.sealManage.sealStatistical
        .statisDocStuas({ ...this.form })
        .then(res => {
          // res.list.map(item => {
          //   this.$set(
          //     item,
          //     "hj",
          //     Number(item.bumenneishezhang) +
          //       Number(item.dangweizhang) +
          //       Number(item.hetongzhuanyongzhang) +
          //       Number(item.lingdaorenyuanmingzhang) +
          //       Number(item.xingzhang) +
          //       Number(item.ceshizhang)
          //   );
          // });
...
.statisHead .is-group tr:nth-of-type(2) {
  display: none;
}
// .statistical {
//   width: 80%;
