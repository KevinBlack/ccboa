  ...
  methods: {
    getNumYearList() {
      return new Promise((resolve, reject) => {
        this.$api.numYear.getNumYearList({tbParam:'cxbmyy'}).then(res => {
      ...
      this.$api.sealManage.sealStatistical
        ...
        .then(res => {
          ...
          this.tableData = res.list.map(item => {
            if (item.rtfAssetList1) {
              if (item.markNum == "0") {
                let rtfAssetList1 = JSON.parse(item.rtfAssetList1);
                item.markNum = rtfAssetList1.reduce((total, rtItem) => {
                  const sealNum = rtItem.sealNum ? Number(rtItem.sealNum) : 0;
                  return total + sealNum;
                }, 0);
              }
            }
            return item;
    ...
    handleOpenPage(path, id, readType, flag) {
      ...
      const routeData = this.$router.resolve({
        path: path,
        query: { id: id, markOrderType: "4", numYear: this.formData.numYear }
      });
    ...
    exportTables() {
      ...
      //   return;
      // }
      exportTable.exportExcel("部门用印查询结果");
    }
  }
};
</script>
