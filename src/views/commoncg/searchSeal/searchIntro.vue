  ...
  methods: {
    getNumYearList() {
      return new Promise((resolve, reject) => {
        this.$api.numYear.getNumYearList({tbParam:'cxjsxyy'}).then(res => {
    ...
    changeYear(val) {
      ...
      if (val == null) {
        this.formData.lidTime = "";
        this.formData.invalidTimeStart = "";
        this.formData.invalidTimeEnd = "";
      }
    },
    reset() {
      this.formData = {
    ...
    loadData() {
      ...
      if (this.formData.lidTime) {
        this.formData.invalidTimeStart = this.formData.lidTime[0];
        this.formData.invalidTimeEnd = this.formData.lidTime[1];
      }
      this.$api.sealManage.sealStatistical
        ...
        .then(res => {
          ...
          this.tableData = res.list.map(item => {
            const rtfAssetList1 = item.rtfAssetList1 ? JSON.parse(item.rtfAssetList1) : [];
            item.markNum = rtfAssetList1.reduce((total, item) => total + (parseInt(item.sealNum)), 0)
            return item;
          });
        });
    },
    ...
    exportTables() {
      ...
      //   return;
      // }
      exportTable.exportExcel("介绍信用印查询结果");
    }
  }
};
</script>
