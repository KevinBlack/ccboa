 ...
 * @LastEditTime: 2020-09-23 19:26:38
...
export default {
  ...
  data() {
    return {
      ...
      theSelect: [],
      tableCols: [
        { label: "业务名称", prop: "appCat" },
        { label: "业务分类简称", prop: "appCatShortName" },
        {
          label: "是否启用",
          prop: "isAvailable",
          formatter: (row) => {
            return beanList.yesOrNo[row.isAvailable];
          },
        },
      ],
      tableData: [],
  ...
  methods: {
    ...
    cellClick(row, column, cell, event) {
      let it = this.tableData[row.index].appCatShortName;
      this.$router.push({
    ...
    btnDel() {
      ...
      if (this.theSelect.length) {
        this.$confirm("此操作将永久删除该条业务, 是否继续?").then(() => {
          this.$api.designDictionary
            .businessClassifyListDelete(this.theSelect)
            .then((res) => {
              this.$message({
                type: "success",
                message: "删除成功",
    ...
    loadData() {
      this.$api.designDictionary
        .businessClassifyList(this.searchData, {
