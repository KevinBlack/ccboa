      ...
      <el-col>
        ...
        >
          <el-row>
            ...
            <el-col :span="12">
              <el-form-item label="协办部门" prop="mainOrganNames">
                <el-input v-model="ruleForm.mainOrganNames" style="width:73%;">
                  <el-button
                    slot="append"
...
export default {
  ...
  data() {
    return {
      bank_countersignature: {
        function: "detailsReceiveAssistQs"
      },
      pickerOptions: {
        shortcuts: [
          {
      ...
      tableData: [],
      tableCols: [
        { label: "协办开始时间", prop: "startTime", align: "center", width: "150px" },
        { label: "标题", prop: "title", align: "left", minwidth:"40%" },
        { label: "文号", prop: "documentNo", align: "center", width: "220px" },
        { label: "来文单位", prop: "orgName", align: "center", width: "220px" },
        { label: "办理时常（天）", prop: "wDay", align: "center", width: "220px" },
        { label: "协办部门", prop: "deptName", align: "center", width: "80px" },
        { label: "办结时间", prop: "finishTime", align: "center", width: "80px" },
        { label: "状态", prop: "state", align: "center", width: "220px",
          formatter(row) {
            return (row.state=="0"?"已办结":"未办结");
          }
        },
        // { label: "数据来源", prop: "DOCUMENTCODE", align: "left", width: "220px" }
      ],
      pagination: {
        page: 1, //第几页
        total: 0, //共有条数
  ...
  methods: {
    // 页面初始化加载数据
    handleLoadData() {
      this.$post.postData('detailsReceiveAssistQs', JSON.stringify(Object.assign(this.bank_countersignature, this.ruleForm, this.pagination)), this.$businessCode.xnkb).then((res) => {
    ...
    timeChange(val) {
      if (val=="1") {
        ...
        this.ruleForm.dateStart1 = dateFormate.date(Date.parse(this.draftTime1[0]));
        this.ruleForm.dataEnd1 = dateFormate.date(Date.parse(this.draftTime1[1]));
      }
    },
    submitForm(formName) {
    ...
    toExcel(){
      ...
      setTimeout(()=>{
        exportTable.exportExcel("请示类收文协办明细统计");
